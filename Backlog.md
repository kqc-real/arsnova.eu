...
- **Story 8.9c (Optionale generative Moderationszusammenfassung):** 🟢 Als Lehrperson oder Moderator:in möchte ich optional eine kurz formulierte, quellengebundene Zusammenfassung aus dem Moderationskompass erhalten, damit ich die Lage schneller verbalisieren kann, ohne dass ein LLM Aktionen ausführt oder ungeprüfte Aussagen als Wahrheit erscheinen.
  ...
  - **Stand (2026-08-19):** Slices 1–3 im Repo (Shared-Vertrag, Host-Kompass-Button, ephemere Queue, privater HTTP-Adapter). Kill-Switch `QA_SUMMARY_ENABLED` bleibt default aus; ohne `QA_SUMMARY_INFERENCE_URL` kein Cloud-Fallback. Echtes LLM folgt mit Story 1.14c. Kanonisch: [docs/features/qa-summary.md](docs/features/qa-summary.md). Die Story ist nicht fertig, solange Slice 4 (Modell) fehlt.
