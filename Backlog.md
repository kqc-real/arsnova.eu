...
> **Repo-Abgleich (Codebase 2026-07-09):** Die weiterhin **offenen bzw. laufenden** Stories sind durch den Stand im Monorepo begründet: u. a. **kein** Fragentyp numerische Schätzung; `SHORT_TEXT` ist inzwischen als **eigener bewertbarer Typ** in `QuestionTypeEnum` (`libs/shared-types`) vorhanden, offen bleiben dort aber der **weiterführende deterministische Ausbau** für Schlüsselwort-Gruppen, token-basierte Mehrwortbewertung und der restliche UX-Abschluss; noch **kein** asynchroner Quizmodus mit teilnehmendenindividuellem Fortschritt, Feedback-Strategie und Host-/Presenter-Dashboard; Q&A-`moderatorView` ist weiterhin an Host-Authentifizierung gebunden, **kein** eigener Moderator-Token/-Rollenpfad. **Teilweise umgesetzt** ist **0.7** (Pipeline-Stand 2026-07-09): unter `scripts/load/` liegen k6-Skripte (`k6-trpc-health-50vu.js`, `k6-trpc-session-50vu.js`, `k6-session-hotpaths-500vu.js`), Node-Smokes (`concurrent-50-http.mjs`, `session-participants-50.mjs`, `ws-status-subscribers.mjs`, `host-vote-progress-200.mjs`, `vote-timer-fairness-600.mjs`, vier Classroom-30er), Artillery-500 (`load:artillery:500`, Unified Session Quiz+Q&A+Blitzlicht, HTTP+WebSocket+Host-Monitor); in CI laufen auf PR/Push `classroom-smokes` (4 Szenarien × 30 TN) und Playwright-E2E, schwere Last (`artillery-500`, k6-Produktion) nur Schedule/Manuell. Offen für volle Story-Erfüllung: Reconnect-Welle, Freitext-/Word-Cloud-Last, Q&A-Moderation unter Spitzenlast, Yjs-Sync-Last, formalisierte Laststufen, Laufvergleich/Regression, produktionsnaher Voll-Lauf (Vote/Status auf Hetzner). **Umgesetzt** sind jetzt u. a. **0.4a** (Session-Tagesrekord-Verlauf im Server-Status-Hilfedialog mit `DailyStatistic`, `dailyHighscores` und Chart im Hilfe-Dialog), **1.2e** (`SHORT_TEXT` mit deterministischer Textbewertung, Teilpunkten und Editor-/Host-Flows), **1.2ea** (Textbewertung 2.0 mit transpositionssensitiver Distanz, stabiler Bewertungs-Pipeline, Vorschau und lernfreundlichem Ergebnisfeedback), **1.2eb** (gemeinsame numerische Bewertungsbasis mit Zahl, Toleranz und Einheit), **1.14** (interaktive Freitext-Wortwolke mit `d3-cloud`-Layout und Export), **2.1c** (Host-/Presenter-Härtung via Host-Token und `hostProcedure`), **5.4a** (Foyer-Einflug im Preset `Spielerisch` mit Host-/Team-Arrival-Layer, Teilnehmenden-Ankunftsmoment, `enableRewardEffects`-Gate und `prefers-reduced-motion`-Fallback), **8.6/8.7** (Q&A-Sortiermodi `Top` / `Beste Fragen` / `Umstritten` inklusive Wilson-/Kontroversitätsberechnung im Router), **8.8** (Tempo-Blitzlicht als `quickFeedback`-Template mit mutablem Redis-Hotpath, Tendenzmodus und Spotlight-UX) sowie die besitzgebundene Quiz-Historie per `accessProof` ohne eigene Story-ID. Die **✅-Einträge** wurden stichprobenartig nicht widerlegt. _Ohne eigene Story-ID:_ Rekord **max. Teilnehmende pro Session** in `health.stats` / Hilfe-Seite (`PlatformStatistic`, u. a. Migration `platform_statistic_max_participants`).
>
...
>
> **Statistik:** 🔴 Must: 34 · 🟡 Should: 67 · 🟢 Could: 11 = **112 Stories gesamt** (**90** ✅ Fertig · **1** 🔨 In Arbeit · **21** ⬜ Offen)
...
- **Story 0.7 (Last- & Performance-Tests mit E2E-Szenarien):** 🟡 Als Entwickler möchte ich reproduzierbare Last- und Performance-Tests mit realitätsnahen E2E-Szenarien haben, damit Engpässe in Live-Sessions, Join-Flows, Freitext, Q&A und Synchronisierung früh erkannt und vor Releases messbar bewertet werden können.
  ...
  - **Aktueller Implementierungsstand (2026-07-09, abgeglichen mit [ADR-0013](docs/architecture/decisions/0013-use-k6-and-artillery-for-load-and-performance-testing.md) und [CI-WORKFLOW.md](docs/CI-WORKFLOW.md)):**
    - **k6 (protokollnah):** `k6-trpc-health-50vu.js`, `k6-trpc-session-50vu.js`, `k6-session-hotpaths-500vu.js` (Modi `join-wave`, `active-question`, `vote-spike`). CI-Job `load-test` gegen Produktion (Schedule/Manuell).
    - **Artillery (Realtime/E2E-nahe Last):** `load:artillery:500` — Unified Session (Quiz + Q&A + Blitzlicht), HTTP + WebSocket + Host-Monitor (`revealResults`, Vote-Progress); CI-Job `artillery-500` (Schedule/Manuell, Standard 100 TN auf Runner).
    - **Node-Smokes / Simulationen:** `concurrent-50-http.mjs`, `session-participants-50.mjs`, `ws-status-subscribers.mjs`, `host-vote-progress-200.mjs`, `vote-timer-fairness-600.mjs`; Classroom-30er: `blitzlicht-classroom-30.mjs`, `qa-classroom-30.mjs`, `demo-quiz-classroom-30.mjs`, `ws-vote-progress-classroom-30.mjs`, `ws-reconnect-wave-classroom-30.mjs` — Root-NPM: `load:simulate:*`, `load:smoke:*`, `load:smoke:*-classroom-30`.
    - **CI auf PR/Push:** Job `classroom-smokes` (fünf Unterrichts-Szenarien, je 30 TN, JSON-Artefakte); Job `e2e` (Playwright `smoke:unified-session`, funktionale Referenz, kein Mehrnutzer-Lastersatz).
    - **Frontend (Playwright, funktional):** u. a. `smoke:unified-session`, `smoke:quiz-sync`, `benchmark:word-cloud` — Referenz-Flows, kein Ersatz für Mehrnutzer-Last.
    - **Dokumentation & Ergebnisse:** [`docs/TESTING.md`](docs/TESTING.md), [`docs/CI-WORKFLOW.md`](docs/CI-WORKFLOW.md), [`docs/implementation/LASTTEST-500-TEILNEHMENDE.md`](docs/implementation/LASTTEST-500-TEILNEHMENDE.md), [`docs/implementation/LASTTEST-ARSNOVA-ARCHITEKTUR-ARBEITSAUFTRAG.md`](docs/implementation/LASTTEST-ARSNOVA-ARCHITEKTUR-ARBEITSAUFTRAG.md), Ergebnisberichte vom 2026-05-09; Praktikums-Einstieg: [`docs/praktikum/HANDOUT-LAST-UND-PERFORMANCE-TESTS.md`](docs/praktikum/HANDOUT-LAST-UND-PERFORMANCE-TESTS.md).
    - **Abgedeckt (Hotpaths):** Join-Welle, aktive Frage, Vote-Spike, Status-Fan-out (WS), Host-Vote-Fortschritt, Timer-Fairness, Q&A-Einreichung, Blitzlicht-Tempo, Demo-Quiz (9 Fragen), Artillery-Live-Session bis 500 TN lokal, Reconnect-Welle (30 TN Classroom-Smoke + **Artillery bis 500 TN**).
    - **Akzeptanzkriterien — Erfüllungsgrad (Kurz):**
      - Setup lokal + CI/dediziert: **teilweise** (PR-Smokes + schwere Jobs getrennt).
      - Vollständige E2E mit Angular-FE unter Last: **offen** (Artillery/Node ohne Browser-FE).
      - Szenarien Join/Quiz/Vote/Q&A-Einreichung/Blitzlicht/Reconnect (Classroom-30 + Artillery-500): **ja**; Freitext-Word-Cloud, Sync unter Last, k6-Reconnect-Hochlast: **offen**.
      - Laststufen formal + Metriken einheitlich + Laufvergleich: **offen** (einzelne JSON/k6-Thresholds, kein Regression-Store).
      - PR-leichter Smoke + Trennung schwerer Last: **ja**.
      - ADR-0013-Rollentrennung: **ja**.
    - **Offen für volle Story-Erfüllung:** Freitext-/Word-Cloud-Last, Q&A-Moderation unter Spitzenlast, Yjs-Sync-Last, k6-Reconnect-Hochlast, formalisierte Laststufen (klein/mittel/hoch), vereinheitlichte maschinenlesbare Summary mit Laufvergleich, k6-Threshold als optionales PR-Gate; produktionsnaher Voll-Lauf (Vote/Status auf Hetzner) noch nicht abgeschlossen.
