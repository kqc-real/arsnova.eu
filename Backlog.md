...
- **Story 1.14b (Word Cloud 2.6 – optionale spaCy-Glättung auf dem Single Host):** 🟡 ✅ **Fertig.** Als Lehrperson möchte ich in der Freitext- und Q&A-Wortwolke auf Wunsch unterschiedliche Sprachformen zu stabilen, lesbaren Begriffen zusammenführen, damit Flexionen die inhaltliche Verdichtung nicht künstlich aufsplitten und die bestehende lexikalische Wortwolke vollständig erklärbar bleibt.
  - **Umsetzungsnachweis (August 2026, Analyseversion `1.14b.7`):** Optionaler spaCy-Sidecar hinter dem Backend (Unix-Socket, Compose-Profil `nlp`, Default `NLP_ENABLED=false`). Host-Aktion `Sprachformen glätten` in Freitext (In-Place-Maximize derselben `app-word-cloud`) und Q&A-Dialog; Snapshot-Analyse mit hartem lexikalischem Fallback; Redis-Text- und Snapshot-Cache; kuratierte `de`/`en`-Fixtures. Neue eingehende Daten markieren den Stand als veraltet. Host-Wechsel der Freitext-Ansicht oder der Q&A-Sortierung bei aktiver Glättung analysiert denselben Stand mit der neuen N-Gramm-Länge bzw. Metrik neu. Produkt- und Betriebsdokumentation: [`docs/features/word-cloud-spacy.md`](docs/features/word-cloud-spacy.md).
