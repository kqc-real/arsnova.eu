...
- **Story 8.9d (Selbstgehosteter Open-Weight-LLM-Server: Runtime-Baustein für 1.14c/8.9c):** 🟢 Als Lehrperson oder Moderator:in möchte ich, dass echte, generative Themenlabel (1.14c) und Moderations-Kurzfassungen (8.9c) auf einem selbstgehosteten Open-Weight-Modell laufen können, damit ich von besseren KI-Vorschlägen profitiere, ohne dass Fragen- oder Antworttexte an einen kommerziellen SaaS-Anbieter gehen.
  ...
  - **Reihenfolge:** Runtime-Baustein dieser Story zuerst (Image, Profil, Flags, gemeinsames Inflight, Health, Tests ohne Modell-Download) → 1.14c Stufe 2 (kurze Labels) → 8.9c Slice 4 erst nach Prefill-Messung auf der echten 8-vCPU-Box.
  - **Akzeptanzkriterien:**
    ...
    - Ein schmaler Node-Client übersetzt die bestehenden App-Verträge nach Chat-Completions; `json_schema`/GBNF **pro Request** (Label ≠ Summary). Kein globales `--json-schema` am Server.
    ...
    - Ein **gemeinsames** Inflight über Label- und Summary-Adapter. Belegter Slot: `GET /slots?fail_on_no_slot=1` und sofort Fallback, keine interne llama.cpp-Warteschlange. Ohne `--slots` liefert der Server 501 statt 503; das App-Inflight bleibt die erste Sperre. Client-Abort gibt den Slot frei. Timeouts liegen unter den jeweiligen Backend-Budgets und sind nicht die Gemini-8-s-Kopie. Circuit Breaker analog Encoder-Client.
  ...
  - **Tests / Verifikation:**
    ...
    - Gleichzeitige Label- und Summary-Anfrage: zweiter Job Fallback, kein gegenseitiges Timeout hinter einer Server-Queue.
    - Vertragstest: OpenAI-Rohantwort ohne Übersetzung wird vom 8.9c-Adapter abgelehnt; gültige Übersetzung bindet nur Snapshot-Quellen.
    - Allowlist-Test: öffentliche DNS-Namen und SaaS-Hosts abgelehnt; Loopback und nach Auflösung private Adressen erlaubt.
  - **Nicht-Ziele:**
    - Kein eigener fachlicher Vertrag, kein eigenes UI-Element; das bleibt bei 1.14c und 8.9c.
