...
- **Story 8.9a (Deterministischer Live-Moderationskompass):** 🟡 Als Lehrperson oder Moderator:in möchte ich während einer Live-Session eine **regelbasierte, quellenbelegte Lageeinschätzung** aus bereits vorhandenen Session-Signalen erhalten, damit ich den nächsten Moderationsschritt bewusst wählen kann, ohne neue Inferenz- oder Datenschutzrisiken in den Live-Pfad zu ziehen.
  ...
  - **Umsetzungsstand Stand 2026-08-17:** Host-Button **Moderation** neben der Live-Leiste, Dialog mit regelbasierten Karten (`Themen`, `Klärung`, `Reibung`, `Tempo`, `Nächster Schritt`), klickbare Quellen (Kanalwechsel, Forum-Highlight, Wortwolken-Sprung inkl. Sortierung/Ansicht), **Zurück**, ruhige Zustände für fehlende Signale, regelbasierte Basis und deaktivierte Analyse. Quiz-Fakten erst nach `RESULTS`, inkl. Histogramm-Spitze außerhalb des Bands. Q&A-Ranking nutzt `score`/`bestScore`/`controversyScore` und den aktiven Sortiermodus; angepinnte Fragen als Themenquelle, Archiv/gelöscht ohne Reibung. Present/Vote und `FINISHED` ohne Kompass. Keine automatischen Pin-/Archiv-/Phasenaktionen, kein neuer globaler Poller, kein Teilnehmer-Fan-out. Moderatorzugang bleibt an Story **8.5** gebunden. i18n in `de`, `en`, `fr`, `es` und `it`.
