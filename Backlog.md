...
- **Story 8.9d (Selbstgehosteter Open-Weight-LLM-Server: Runtime-Baustein für 1.14c/8.9c):** 🟢 Als Lehrperson oder Moderator:in möchte ich, dass echte, generative Themenlabel (1.14c) und Moderations-Kurzfassungen (8.9c) auf einem selbstgehosteten Open-Weight-Modell laufen können, damit ich von besseren KI-Vorschlägen profitiere, ohne dass Fragen- oder Antworttexte an einen kommerziellen SaaS-Anbieter gehen.
  ...
  - **Akzeptanzkriterien:**
    ...
    - Cluster-Label-Aufträge (1.14c) und Summary-Bullet-Aufträge (8.9c) dürfen sich den Prozess teilen, behalten aber getrennte Prompts, Zod-Verträge, Queues und Cooldowns; ein hängender Auftrag blockiert den anderen nicht dauerhaft.
    - `deploy.sh` startet den Server nicht automatisch; Aktivierung ist ein bewusster, dokumentierter Betreiberschritt.
    - Live-Hotpfade (`qa.submit`, Join, Vote, WebSocket) warten nie auf diesen Server.
  - **Tests / Verifikation:**
    - Sidecar-Tests ohne Modell-Download (Compose-Profil, kein öffentlicher Port, Ressourcenlimits), analog `npm run test:spacy-sidecar` / `test:wordcloud-encoder`.
    - Hotpath-Isolation: Join/Vote/`qa.submit`/WS unverändert bei totem, langsamem oder überlastetem LLM-Server.
    - Last-/Fehlertest für gleichzeitige Cluster-Label- und Summary-Anfragen (Serialisierung, kein gegenseitiges Blockieren).
  - **Nicht-Ziele:**
    - Kein eigener fachlicher Vertrag, kein eigenes UI-Element; das bleibt bei 1.14c und 8.9c.
    - Kein Ersatz für die extraktiven bzw. labellosen Fallback-Pfade von 1.14c/8.9c.
  ...
  - **Abhängigkeiten:** Story 1.14c (Stufe 2), Story 8.9c (Slice 4), [ADR-0032](docs/architecture/decisions/0032-optional-nlp-cascade-for-qa-moderation-signals.md), [ADR-0035](docs/architecture/decisions/0035-self-hosted-llm-runtime-llama-cpp-over-ollama.md), ADR-0025, ADR-0026.
