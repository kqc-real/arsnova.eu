...
| 8    | 8.8   | Tempo-Blitzlicht als Host-Option                                                   | 🟡   | ⬜ Offen     |
...
>
> **Statistik:** 🔴 Must: 34 · 🟡 Should: 62 · 🟢 Could: 11 = **107 Stories gesamt** (**85** ✅ Fertig · **1** 🔨 In Arbeit · **21** ⬜ Offen)
...
> **Verifizierung Epic 8 (2026-05-13):** Der bisherige Q&A-Kernumfang 8.1–8.4 sowie die Host-Sortiermodi 8.6/8.7 sind umgesetzt. Offen: Story 8.5 (delegierbare Moderation) und Story 8.8 (Tempo-Blitzlicht als Host-Option). Details zu 8.6/8.7: [`docs/features/controversy-score.md`](docs/features/controversy-score.md). Zielmodell fuer 8.8: [`docs/architecture/decisions/0029-tempo-as-predefined-blitzlicht-template.md`](docs/architecture/decisions/0029-tempo-as-predefined-blitzlicht-template.md).  
...
- **Story 8.8 (Tempo-Blitzlicht als Host-Option):** 🟡 Als Lehrperson möchte ich im bestehenden Blitzlicht ein vordefiniertes Tempo-Blitzlicht starten können, sowohl in der Session als auch im Standalone-Blitzlicht, damit Teilnehmende während eines Vortrags jederzeit anonym signalisieren können, ob sie folgen, ob es schneller gehen darf, ob es zu schnell ist oder ob sie abgehängt sind.
  - **Akzeptanzkriterien:**
    ...
    - Der Tendenzindikator wird erst **aktiv**, wenn genügend Tempo-Stimmen relativ zur **Gesamtzahl der aktuell teilnehmenden Personen im jeweiligen Blitzlicht-Kontext** vorliegen; die Bewertung bezieht sich dabei nicht nur auf die abgegebenen Tempo-Stimmen, sondern auf die gesamte aktive Teilnehmendenbasis.
  ...
  - **Empfohlene Berechnungslogik fuer die Tendenz:**
    ...
    - **Beispiel-Schwellen auf Basis von `activeParticipants`:** `lost >= 0.12` => `Mehrere Teilnehmende sind abgehängt.`; `slow_down + lost >= 0.22` => `Das Tempo wirkt zu hoch.`; `speed_up >= 0.22` bei gleichzeitig niedrigem `slow_down + lost` => `Die Gruppe signalisiert Unterforderung.`; `following >= 0.50` bei niedrigen Problemanteilen => `Die Mehrheit kann folgen.`; knappe Abstände zwischen mehreren Gruppen oder hohe Streuung => `Die Gruppe ist heterogen.`
