...
- Touch-Targets grundsätzlich ≥ 44 × 44 px als Projektstandard; mindestens
  WCAG 2.2 AA, Erfolgskriterium 2.5.8, einschließlich zulässiger Ausnahmen.
...
- Kein fehlgeschlagenes gewichtetes Lighthouse-Accessibility-Einzelaudit;
  zusätzlich bleiben Score, axe-, Template-Lint- und Reflow-Gates grün.
...
- **Story 0.7 (Last- & Performance-Tests mit E2E-Szenarien):** ✅ Als Entwickler möchte ich reproduzierbare Last- und Performance-Tests mit realitätsnahen E2E-Szenarien haben, damit Engpässe in Live-Sessions, Join-Flows, Freitext, Q&A und Synchronisierung früh erkannt und vor Releases messbar bewertet werden können.
  ...
  - **Aktueller Implementierungsstand (2026-07-12, abgeglichen mit [ADR-0013](docs/architecture/decisions/0013-use-k6-and-artillery-for-load-and-performance-testing.md), [PERFORMANCE-TESTING.md](docs/PERFORMANCE-TESTING.md) und [CI-WORKFLOW.md](docs/CI-WORKFLOW.md)):**
    ...
    - **Artillery (Realtime/E2E-nahe Last):** Unified Session und Reconnect bis 500 TN; Prozessfehler und Host-Subscription-Fehler sind harte Gates. Kapazitätsnachweise lokal und per CI-Runner (`artillery-500`, `artillery-reconnect-500`).
    ...
    - **Metriken & Reports:** gemeinsame atomische JSON-/JUnit-Reports, Budgetvergleich gegen geprüfte Baselines sowie Backend-Prozess-, Redis- und PostgreSQL-Probes im Soak.
    ...
    - **Akzeptanzkriterien — Erfüllungsgrad (Kurz):**
      ...
      - Szenarien Join/Quiz/Vote/Q&A/Blitzlicht/Reconnect/Freitext-Word-Cloud/Yjs-Sync: **ja**.
      - Laststufen + einheitliche Metriken + Laufvergleich: **ja** (Baseline freigegeben).
  ...
  - **Abhängigkeiten:** Story 0.2 (tRPC WebSocket-Adapter), Story 0.5 (Rate-Limiting), Story 0.6 (CI/CD), Story 2.1a (Session-Start), Story 2.2 (Lobby), Story 3.1 (Join), Story 3.3b (Abstimmung), Story 4.5 (Freitext-Auswertung), Story 8.1–8.4 (Q&A), optional Story 1.6/1.6a/1.6b (Sync), ADR-0013.

- **Story 0.8 (Komplexitätsabbau / McCabe-Refactor):** 🚫 **Geschlossen – wird nicht umgesetzt.** _(menschliche Metrik, keine Relevanz für Code-Agenten)_ — Als Entwickler möchte ich überhöhte zyklomatische Komplexität in priorisierten Hotspots systematisch reduzieren, damit Wartbarkeit, Änderbarkeit und Fehlersicherheit steigen, ohne funktionale Regressionen einzuführen.
...
> **Verifizierung Epic 1 (2026-03-09, ergänzt bis 2026-08-21):** Die umgesetzten Kern-Stories in Epic 1 sind auf **✅ Fertig** gesetzt — siehe Übersichtstabelle. **Offen** bleiben aktuell **1.2ec–1.2ed**, **1.14c** und **1.14d** (dort ⬜). **1.14a** ist mit den produktiven Ausbaustufen 2.1 bis 2.5 abgeschlossen; **1.14b** ist mit der optionalen spaCy-Glättung auf dem Single Host abgeschlossen; **1.14c** ist die getrennte semantische Q&A-Ausbaustufe und das Studienobjekt des Cloud-Computing-Kurses; **1.14d** hängt den Freitext-Themenmodus an denselben Encoder. **1.6c** ist technisch fertig; der automatische UUID-only-Legacy-Cutoff am **1. Oktober 2026** ist nur noch ein betrieblicher Termin. **1.6d** ist mangels nachgewiesenem Performanceproblem geschlossen und nicht geplant. **Story 1.2f** ist aus A11y-Gründen **🚫 geschlossen**. **Story 1.2d**, **Story 1.2e**, **Story 1.2ea**, **Story 1.2eb**, **Story 1.2g**, **Story 1.2h**, **Story 1.2i**, **Story 1.2j**, **Story 1.7a** und **Story 1.7b** sind umgesetzt und auf **✅ Fertig** gesetzt.
...
- **Story 1.2d (Numerische Schätzfrage – eigener Fragentyp mit Zwei-Runden-Flow und Statistik):** 🟡 Als Lehrperson möchte ich **numerische Schätzfragen** stellen können, bei denen ich **Zahlentyp** (ganzzahlig / Dezimal mit maximaler Nachkommastellenanzahl), **Referenzwert** und **Toleranzband** für die Wertung „richtig“ vorgebe; optional mit **zwei Abstimmungsrunden** inkl. Diskussionsphase (analog Peer Instruction). Als Host möchte ich nach Freigabe der Ergebnisse eine **beamer-taugliche Visualisierung** und eine **detaillierte statistische Auswertung beider Runden** sehen – **ohne** vorherigen Herdeneffekt durch Live-Verteilungen.
  - **Akzeptanzkriterien:**
    ...
    - **Scoring & Scoreboards:** Der Schätzfrage-Typ `NUMERIC_ESTIMATE` ist ein **bewertbarer Fragetyp** und muss überall mitzählen, wo Quiz-Scoreboards Punkte auswerten: persönliche Scorecard, finales Leaderboard, Top-5-Zwischenleaderboard auf Host und Clients sowie Team-Leaderboard im Team-Modus. Die Scoreboard-Gates dürfen diesen Typ nicht wie `FREETEXT`, `SURVEY` oder `RATING` ausblenden. Die Implementierung muss die gemeinsame Scoring- und Effective-Vote-Regel aus [ADR-0028](docs/architecture/decisions/0028-quiz-scoring-and-effective-vote-rule.md) einhalten.
  ...
  - **Implementierungsstatus 2026-06-17:** ✅ Umgesetzt als eigener Typ `NUMERIC_ESTIMATE` inkl. Shared Types/Zod, Prisma-Feldern, Quiz-Editor, arsnova.click-Import, Demo-Quiz in fünf Sprachen, Host-/Vote-Flow, zwei Runden, Data-Stripping, Statistik-Histogramm, Scoreboards, Nähe-Scoring im Toleranzband und Smoke-Test. Detaildoku: [`docs/features/numeric-estimate.md`](docs/features/numeric-estimate.md).
...
- **Story 1.2ec (Kurzantwort: gemeinsame Schlüsselwort-Bewertungsbasis für Gruppen, Teilpunkte und Erklärtexte):** 🟡 Als Lehrperson möchte ich **Kurzantworten über hinterlegte Schlüsselwort-Gruppen** teilweise bewerten können, damit kurze offene Fachantworten **ohne KI** nachvollziehbar und fairer bepunktet werden.
  - **Lehrkraft-Perspektive / Defizit:** Lehrkräfte können aktuell zwar Musterlösungen für Kurzantworten hinterlegen, aber keine **inhaltlichen Bausteine** mit eigenen Teilpunkten modellieren. Bei Antworten wie „Photosynthese nutzt Lichtenergie, Wasser und CO₂ zur Bildung von Glucose und Sauerstoff“ lässt sich nicht sauber festlegen, dass z. B. `Lichtenergie`, `Wasser`, `CO₂` und `Glucose/Sauerstoff` jeweils getrennt Punkte bringen. Stattdessen müssten viele komplette Musterlösungsvarianten gepflegt werden, oder fachlich gute Teilantworten werden zu hart bewertet. Die Story soll deshalb eine nachvollziehbare, nicht-KI-basierte Bewertung nach erwarteten Schlüsselbegriffen bzw. Begriffgruppen ermöglichen.
  - **Akzeptanzkriterien:**
    ...
    - **Abgrenzung arsnova.click-Import:** `arsnova.click`-Tags bzw. dortige Keyword-/Tag-Metadaten werden **nicht** als arsnova.eu-Schlüsselwort-Gruppen übernommen, unterstützt oder ausgewertet. Der Import bleibt hier bewusst Best-Effort: solche Metadaten werden ignoriert bzw. mit Warnhinweis dokumentiert, aber nicht in fachliche Bewertung übersetzt.
...
- **Story 1.2ed (Kurzantwort: gemeinsame Token-Bewertungsbasis für Mehrwortantworten und UX-Abschluss):** 🟡 Als Lehrperson möchte ich **Mehrwort-Kurzantworten mit optional weniger strenger Wortreihenfolge** bewerten können und eine **konsistente, didaktisch verständliche UI** für alle deterministischen Kurzantwort-Modi erhalten.
  - **Lehrkraft-Perspektive / Defizit:** Die heutige Kurzantwortbewertung ist für kurze Begriffe, Zahlen und kleinere Schreibfehler geeignet, bleibt bei **Mehrwortantworten** aber stark an die konkrete Zeichenfolge gebunden. Fachlich naheliegende Antworten wie „Rückkopplung negativ“ statt „negative Rückkopplung“ oder „Integration kontinuierlich“ statt „kontinuierliche Integration“ werden ohne zusätzliche Musterlösung nicht zuverlässig als passend erkannt. Lehrkräfte müssen dadurch viele Wortstellungsvarianten manuell eintragen oder riskieren, dass fachlich richtige Antworten unfair abgelehnt werden. Die Story soll einen expliziten, deterministischen Schalter für weniger strenge Wortreihenfolge schaffen, ohne verstecktes Semantik- oder KI-Matching einzuführen.
  - **Akzeptanzkriterien:**
...
- **Story 1.2f (Fragentyp: Hotspot auf Bild):** 🚫 **Geschlossen – wird nicht umgesetzt.** Die visuelle und positionsabhängige Interaktion kann für blinde und motorisch eingeschränkte Nutzer:innen nicht gleichwertig zugänglich gestaltet werden und passt daher nicht zum Anspruch von arsnova.eu auf WCAG-2.2-AA-konforme, inklusive Fragetypen.
  - **Historische Spezifikation (nicht zur Umsetzung):** Als Lehrperson möchte ich **Hotspot-Fragen auf Bildern** stellen können, damit Teilnehmende eine **Position oder Region auf einem Bild** markieren können, z. B. in Karten, Diagrammen, anatomischen Abbildungen, Fotos oder technischen Skizzen. Als Host möchte ich nach der Ergebnisfreigabe eine **aggregierte, beamer-taugliche Visualisierung** der Klicks/Taps sehen, um typische Fehlwahrnehmungen und Cluster schnell zu erkennen – **ohne** dass während der laufenden Antwortphase ein Herdeneffekt entsteht.
  - **Ehemalige Akzeptanzkriterien:**
...
- **Story 1.2j (Fragentyp: Gruppierung / Kategorisierung):** 🟡 Als Lehrperson möchte ich **Kategorisierungsfragen** erstellen können, bei denen Teilnehmende mehrere Elemente (Begriffe, Aussagen, Beispiele, Formeln) vorgegebenen Kategorien (2 bis 4 Zielgruppen/Klassen) zuordnen, damit Abgrenzungs- und Strukturierungskompetenzen geprüft werden können. Als Host möchte ich nach der Ergebnisfreigabe sehen, welche Elemente welchen Kategorien zugeordnet wurden und wo typische Misskonzeptionen / Fehlkategorisierungen aufgetreten sind.
  - **Akzeptanzkriterien:**
    - **Neuer Fragentyp:** Neuer Typ `CATEGORIZATION` (oder fachlich gleichwertige Benennung) in `@arsnova/shared-types`.
    - **Editor (Lehrperson):** Im Quiz-Editor kann die Lehrperson
      - **2 bis 4 Kategorien** und **4 bis 12 Elemente** definieren,
      - pro Kategorie und Element kurze Texte bzw. kurze Markdown-/KaTeX-Fragmente hinterlegen,
      - jedem Element genau eine Zielkategorie zuweisen (1:N-Zuordnung),
      - optional die Reihenfolge der Elemente pro Teilnehmenden zufällig mischen lassen.
    - **Teilnehmenden-UI:** Zu jedem Element steht eine eindeutig beschriftete Kategorie-Auswahl (z. B. Kombinationsfeld / native/Material-Select oder Segmentgruppe) bereit. Die vollständige Aufgabe ist ohne Drag-and-Drop und ohne Maus ausschließlich per Tastatur, Touch, Spracheingabe und Screenreader lösbar.
    - **Validierung:** Jedes Element muss genau einer Kategorie zugeordnet sein; unvollständige Zuordnungen können nicht abgesendet werden. Der Fehlerfokus führt zum ersten unvollständigen Auswahlfeld.
    - **Wertung:** Im ersten Schritt ist die Frage **voll korrekt** oder **nicht voll korrekt** für das Scoring; zusätzlich werden intern pro Element und Kategorie Trefferquoten für die Host-Analyse ausgewertet.
    - **Scoring:** Kategorisierungsfragen nehmen regulär am Quiz-Scoring teil. Teilpunkte sind im ersten Schritt **kein Pflichtumfang** dieser Story.
    - **Host-/Presenter-Visualisierung nach Freigabe:** Die Auswertung zeigt mindestens
      - die Zuordnung der Elemente zu den Soll-Kategorien,
      - den Anteil vollständig korrekter Antworten,
      - die Verteilung der gewählten Kategorien je Element (Matrix/Diagramm),
      - eine Übersicht häufiger Fehlkategorisierungen,
      - eine kompakte, beamer-taugliche Darstellung ohne alleinige Farbcodierung.
    - **Kein Herdeneffekt:** Während `ACTIVE` gibt es keine öffentliche Zwischenanzeige der Kategorisierungen.
    - **Data Stripping:** Während `ACTIVE` werden Soll-Kategorien und Bewertung nicht an Teilnehmende übertragen.
    - **Import/Export:** Kategorien, Elemente und Zuordnungen sind vollständig serialisierbar; der JSON-Import validiert Kardinalität, Eindeutigkeit und Referenzen.
    - **Tests:** Es gibt Tests für Editor-Validierung, korrekte/falsche Kategorisierung, stabile Shuffle-Logik, Data-Stripping sowie Import/Export.
    - **i18n (ADR-0008):** Alle neuen UI-Strings in **de, en, fr, es, it**.
  - **Abhängigkeiten:** Story **1.3** (Bewertungslogik), Story **1.7** (Markdown/KaTeX), Story **2.4** (Data-Stripping), Story **3.3a/b** (Teilnehmenden-UI), Story **4.1** / **4.4** (Scoring & Ergebnisvisualisierung), Story **4.7** (Ergebnis-Export), Story **6.5** (WCAG 2.2 AA).
  - **Synergie- & Umsetzungsstrategie (Stories 1.2g, 1.2h, 1.2j):**
    - **Shared Infrastructure (`libs/shared-types`):** Generischer, deterministischer `stableShuffleWithContext`-Helper für Ansichten ohne Lösungsleak & wiederverwendbares Data-Stripping-Pattern für den Zustand `ACTIVE`.
    - **Baugleiche Frontend-Komponenten (`apps/frontend`):** `MATCHING` und `CATEGORIZATION` teilen sich dieselbe barrierefreie `ItemSelectionRowComponent` (Label + `MatSelect` + ARIA-Status + automatischer Fehlerfokus).
    - **Einheitliches Host-Widget (`apps/frontend`):** Wiederverwendbare `PresenterDistributionMatrixComponent` für die fehler-fokussierte Visualisierung von Verwechslungen, Inversionen und Fehlkategorisierungen nach Ergebnisfreigabe.
    - **Empfohlene Entwicklungsreihenfolge:** `1.2h (ORDERING)` → `1.2g (MATCHING)` → `1.2j (CATEGORIZATION)` reduziert den Gesamtaufwand der Folgestories um ca. 50–60 %.
...
- **Story 1.6c (Sync-Sicherheit härten):** 🔴 🗓️ **Technisch fertig – automatischer Cutover am 1. Oktober 2026.** Als Lehrperson möchte ich, dass geteilte Quiz-Sammlungen sicherer geöffnet werden können, damit ein versehentlich weitergegebener oder missverstandener Sync-Zugang nicht stillschweigend Vollzugriff gewährt.
  - **Stand 2026-08-07:** UI-Kommunikation, Relay-Caps, Rate-/Payload-/Verbindungsgrenzen, signierte langlebige Share-Tokens, serverseitig erzeugte sichere Räume, Generationen und manuelle Link-Rotation sind umgesetzt. Es gibt kein weiteres Implementierungsticket. Die UUID-only-Legacy-Grace endet automatisch über `YJS_SHARE_LEGACY_UUID_CUTOFF_AT` (Default `2026-10-01T00:00:00.000Z`); danach ist nur noch die reguläre betriebliche Beobachtung vorgesehen.
  - **Akzeptanzkriterien:**
    ...
    - [ ] Betrieblicher Termin: Nach dem automatischen Cutoff am **1. Oktober 2026** wird im regulären Betrieb bestätigt, dass UUID-only-Legacy-Zugriffe deaktiviert sind; dafür ist kein eigener Implementierungs-PR vorgesehen.
- **Story 1.6d (Sync-Performance & Skalierung optimieren):** 🚫 **Geschlossen – nicht geplant.** Als Lehrperson möchte ich, dass die Synchronisierung meiner Quiz-Sammlung auch bei größeren Sammlungen und mehreren Geräten flüssig bleibt, damit Bearbeitung und Gerätewechsel nicht durch spürbare Verzögerungen ausgebremst werden.
  - **Status 2026-08-07:** Der erfolgreiche 500-Client-Last- und Reconnect-Nachweis zeigt kein konkretes Performanceproblem. Weitere Optimierungen werden deshalb nicht vorsorglich umgesetzt. Die folgenden Kriterien bleiben nur als historische Option für den Fall erhalten, dass spätere Messwerte einen realen Engpass belegen.
  - **Historische Akzeptanzkriterien:**
...
- **Story 1.14a (Word Cloud 2.0 – echtes Layout + Premium-UX):** 🟡 ✅ **Fertig.** Als Lehrperson möchte ich Freitext-Antworten in einer echten, dichten und präsentationstauglichen Word-Cloud sehen, damit häufige Begriffe im Livebetrieb auf Smartphone, Host-Ansicht und Beamer ruhig, lesbar, interaktiv und exportierbar dargestellt werden.
  - **Umsetzungsnachweis (seit Mai 2026 produktiv):** Die Ausbaustufen Word Cloud 2.1 bis 2.5 liefern das gekapselte `d3-cloud`-Layout, responsive Wortlimits, dokumenthäufigkeitsbasierte Gewichtung, Phrasen- und Q&A-Modi, quellenbezogene Interaktion, Host-/Presenter-Vollbild und Freeze, textuelle Alternative sowie geordnete PNG-/CSV-Exporte. Implementierung und Tests liegen in [`apps/frontend/src/app/features/session/session-present`](apps/frontend/src/app/features/session/session-present); Strategie, produktiver Stand und Qualitätsproben sind in [`WORD-CLOUD-2.1-LEMMA-STRATEGY.md`](docs/implementation/WORD-CLOUD-2.1-LEMMA-STRATEGY.md), [`WORD-CLOUD-THESIS-REVIEW-2026-05.md`](docs/implementation/WORD-CLOUD-THESIS-REVIEW-2026-05.md) und den [`Word-Cloud-Screenshots`](docs/screenshots/README.md) dokumentiert.
  - **Akzeptanzkriterien:**
    ...
    - **Produktqualitäts-Abnahme:** Die Standarddarstellung ist mit realistischen und synthetischen Datensätzen auf Smartphone, Laptop und Projektionsfläche hinsichtlich kollisionsarmer Platzierung, visueller Hierarchie, Dichte, Responsiveness, quellenbezogener Interaktion und geordnetem Export geprüft.
    - **Nachgewiesene Produktstärken:** Deterministisches Layout, quellenbezogene Filterung und Tooltips, responsive Wortlimits, textuelle barrierefreie Alternative sowie geordnete PNG-/CSV-Exporte sind implementiert und durch Tests beziehungsweise dokumentierte Qualitätsproben abgesichert.
    ...
    - **Nachrangiger UX-Restpunkt:** Eine positionsstetige Animation zwischen zwei `d3-cloud`-Layouts ist kein Abschlusskriterium der produktiven Story. Debouncing, deterministisches Layout und Host-Freeze sind die bewusst gewählten Stabilitätsmechanismen. Eine Animation wird nur bei belastbarem UX-Nutzen ergänzt und muss `prefers-reduced-motion` respektieren.
    ...
    - **Bewusste Rotationsentscheidung:** Die produktive Darstellung verwendet fest 0° für ruhige Lesbarkeit auf Mobilgerät und Beamer. Die technische Rotationsschnittstelle von `d3-cloud` bleibt intern verfügbar; eine nutzerseitige Konfiguration wird erst bei vergleichender UX-Evidenz ergänzt.
    ...
    - **Sprachliche Normalisierung entkoppelt:** Story 1.14a hält Cleaning, Tokenisierung, Stopwortlogik und Layout erweiterbar, implementiert aber keine neue NLP-Laufzeit. Die optionale spaCy-Glättung mit eigenem Fallback-, Ressourcen- und Qualitätsvertrag ist in Story 1.14b umgesetzt.
  ...
  - **Performance-Hinweis:** Die Story ist nach [ADR-0025](docs/architecture/decisions/0025-treat-future-extensions-as-performance-critical-until-proven-otherwise.md) performance-kritisch, weil Live-Layout, Re-Rendering, Aggregation und Exportqualität im Presenter-/Host-Pfad liegen. Die Ressourcen- und Degradationsverträge der spaCy-Glättung liegen in Story 1.14b, die der semantischen Inferenz in Story 1.14c.
...
- **Story 1.14b (Word Cloud 2.6 – optionale spaCy-Glättung auf dem Single Host):** 🟡 ✅ **Fertig.** Als Lehrperson möchte ich in der Freitext- und Q&A-Wortwolke auf Wunsch unterschiedliche Sprachformen zu stabilen, lesbaren Begriffen zusammenführen, damit Flexionen die inhaltliche Verdichtung nicht künstlich aufsplitten und die bestehende lexikalische Wortwolke vollständig erklärbar bleibt.
  - **Umsetzungsnachweis (August 2026, Analyseversion `1.14b.8`):** Optionaler spaCy-Sidecar hinter dem Backend (Unix-Socket, Compose-Profil `nlp`, Default `NLP_ENABLED=false`). Host-Aktion `Sprachformen glätten` in Freitext (In-Place-Maximize derselben `app-word-cloud`) und Q&A-Dialog; explizite Wolkensprache am Glätten-Button (Default Host-UI, Override `de`/`en`/`fr`/`es`); Snapshot-Analyse mit hartem lexikalischem Fallback; Redis-Text- und Snapshot-Cache; kuratierte `de`/`en`/`fr`/`es`-Fixtures. Neue eingehende Daten markieren den Stand als veraltet. Host-Wechsel der Freitext-Ansicht, der Q&A-Sortierung oder der Wolkensprache bei aktiver Glättung analysiert denselben Stand neu. Produkt- und Betriebsdokumentation: [`docs/features/word-cloud-spacy.md`](docs/features/word-cloud-spacy.md).
  - **Produktgrenze:** spaCy liefert ausschließlich sprachliche Glättung. Die Story erzeugt keine Synonym-, Intent- oder Themencluster und darf in UI, Tooltip oder Dokumentation nicht als semantische Analyse bezeichnet werden. Der semantische Themenpfad ist Story 1.14c (Q&A) und Story 1.14d (Freitext).
  - **Lizenz- und Auslieferungsgrenze:** arsnova.eu bleibt MIT. Es gibt keine App-Lizenz, unter der alle fünf offiziellen spaCy-Kernmodelle als ein Open-Source-Paket mitgeliefert werden dürften. Die Modelle bleiben Drittwerk mit eigener Lizenz; Hinweise gehören in `NOTICE` beziehungsweise eine Drittlizenzseite, nicht ins Impressum oder in die Datenschutzerklärung.
    - `de` (`de_core_news_sm`, MIT) und `en` (`en_core_web_sm`, MIT) dürfen im optionalen Standard-Sidecar mitgeliefert werden.
    - `fr` (`fr_core_news_sm`, LGPL-LR) darf mitgeliefert werden, sofern Namensnennung, Lizenztext und Ersetzbarkeit des Modells dokumentiert sind.
    - `es` (`es_core_news_sm`, GPL-3.0) darf mitgeliefert werden, muss aber als GPL-Drittteil gekennzeichnet bleiben; Sidecar-Image und Compose dürfen nicht als reines MIT ausgewiesen werden.
    - `it` (`it_core_news_sm`, CC BY-NC-SA 3.0) gehört nicht in den MIT-Default (Repo-Compose, Standard-Image, Fork-Auslieferung). Kommerzielle oder unbeschränkt weitergegebene Installationen fallen für `it` lexikalisch zurück. Ein optionales, klar getrenntes Extra-Image nur für den eigenen altruistischen Betrieb ist zulässig, aber kein Produktstandard.
    - Die erste Qualitätsstufe bleibt `de`/`en` mit Fixtures. `fr`/`es` folgen erst nach eigenen Fixtures und NOTICE-/GPL-Artefakten. `it` folgt im verteilten Default nur mit einem lizenzkompatiblen Alternativmodell, nicht mit dem offiziellen NC-Modell.
  - **Akzeptanzkriterien – Auslösung und Zustände:**
    - Die bestehende lexikalische Wortwolke bleibt in Freitext und Q&A der sofort verfügbare Standard und funktioniert auch bei vollständig deaktivierter oder nicht erreichbarer spaCy-Komponente unverändert weiter.
    - Nur der Host kann im jeweiligen Wortwolken-Dialog die sekundäre Aktion `Sprachformen glätten` auslösen; es gibt keinen Participant-Umschalter und keine automatische Analyse bei jeder Antwort, Frage oder Abstimmung.
    - Die Analyse verarbeitet einen unveränderlichen Snapshot der aktuell sichtbaren Daten. Neue Fragen oder Antworten starten keine Neuberechnung, sondern markieren den geglätteten Stand als veraltet und bieten `Neu analysieren` an. Ein Host-Wechsel der Freitext-Ansicht (`Einzelwörter` / `Wörter & Phrasen`) oder der Q&A-Sortierung bei aktiver `LEXICAL`-Glättung analysiert denselben Stand mit der neuen N-Gramm-Länge bzw. Metrik neu.
    - Die UI bildet mindestens `verfügbar`, `läuft`, `aktiv`, `veraltet`, `nicht verfügbar` und `fehlgeschlagen` ruhig und verständlich ab. Während der Analyse bleibt die lexikalische Wolke sichtbar und bedienbar.
    - Benutzertexte vermeiden Technikjargon wie `spaCy`, `NLP` oder `Lemmatisierung` und sind in `de`, `en`, `fr`, `es` und `it` idiomatisch gepflegt; technische Modell- und Versionsangaben dürfen in Diagnose beziehungsweise Export erscheinen.
  - **Akzeptanzkriterien – linguistische Qualität:**
    - Die gemeinsame Pipeline bereinigt zuerst Rauschen und schützt Fachbegriffe; danach liefert spaCy nur Token, Lemma, Wortart und optional Entity-Typ an die bestehende lexikalische Aggregation.
    - Die erste produktive Qualitätsstufe ist auf `de` und `en` begrenzt. `fr` und `es` fallen bis zu eigenen Fixtures und Lizenzartefakten lexikalisch zurück. `it` fällt im verteilten Default dauerhaft lexikalisch zurück, solange nur das offizielle CC-BY-NC-SA-Modell verfügbar ist.
    - Gruppiert werden zunächst `NOUN`, `PROPN`, geschützte technische Begriffe und kuratierte, durch Fixtures freigegebene Verb-/Adjektivfamilien. Damit bleiben fachlich gewünschte Zusammenführungen wie `validieren`, `validiert` und `Validierung` erhalten; eine unbeschränkte Lemmatisierung sämtlicher Verben und Adjektive ist ausdrücklich nicht Teil der ersten Produktstufe.
    - Eigennamen werden nicht blind lemmatisiert, deutsche Komposita nicht aggressiv zerlegt und Mehrwortbegriffe nicht unkontrolliert expandiert.
    - Gruppierungsschlüssel und sichtbares Label bleiben getrennt: Normalisierte Schlüssel dürfen natürliche, häufige Oberflächenformen wie `Fragen` nicht durch unlesbare Modellformen ersetzen.
    - Geschützte Formen wie `C++`, `C#`, `npm install`, `docker compose` und `HTTP 404` bleiben unverändert und dürfen durch Tokenisierung oder Lemmatisierung nicht beschädigt werden.
    - Kuratierte deutsche und englische Fixtures belegen mindestens Flexion, Singular/Plural, Verb-/Nominalvarianten, Eigennamen, Komposita, technische Begriffe, Sprachmischung, Tippfehler sowie Fälle, die bewusst nicht zusammengeführt werden dürfen.
  - **Technisches Zielbild – bestehender Single Host:**
    - spaCy läuft als optionaler, ausschließlich intern erreichbarer Sidecar hinter dem Backend auf der bestehenden One-Server-Infrastruktur; es wird weder in das Angular-Frontend noch als Python-Laufzeit in den Node-App-Container eingebaut. Das Standard-Sidecar-Image enthält `de`/`en`; `fr`/`es` nur mit NOTICE- beziehungsweise GPL-Kennzeichnung; `it_core_news_sm` nicht.
    - Die Pipeline bleibt `Daten holen → bereinigen und Fachbegriffe schützen → optional glätten → lexikalisch aggregieren → rendern → Tooltip/Export`. Der Renderer analysiert weiterhin keine Rohtexte.
    - Der schema-first Analysevertrag unterscheidet mindestens angeforderten und tatsächlich angewandten Normalisierungsmodus, Locale/Fallback-Locale, Modell-/Analyseversion, Snapshot-Hash, Erzeugungszeitpunkt und Fallbackstatus.
    - Ein Text-Cache nach Sprache, Text-Hash und Analyseversion sowie ein Snapshot-Cache nach Session, Kanal, Sortiermetrik, Normalisierungsmodus und Snapshot-Hash verhindern doppelte Arbeit. TTL und Invalidierung sind dokumentiert.
    - Sidecar und Client besitzen Feature-Flag/Kill-Switch, Health-Check, hartes Timeout, begrenzte Parallelität, Backpressure und CPU-/RAM-Limits. spaCy darf Join, Vote, Q&A, WebSocket/Yjs, PDF-Erzeugung oder die lexikalische Wortwolke nicht verdrängen.
    - Der Sidecar besitzt keinen öffentlichen Port. Eingaben, Analyseantworten und Modellzwischenstände werden nicht dauerhaft gespeichert oder in normalen Logs ausgeschrieben.
  - **Performance-Steckbrief nach ADR-0025:**
    - **Lastklasse:** performance-kritisch wegen einer neuen CPU-/RAM-Komponente auf dem Produktionshost.
    - **Pfadtyp:** ausschließlich host-ausgelöster Hintergrund-/Snapshotpfad, niemals Participant-, Vote-, Q&A-Submit- oder WebSocket-Hotpath.
    - **Worst Case:** viele Hosts fordern nahezu gleichzeitig große Freitext- oder Q&A-Snapshots an, während der normale Livebetrieb ausgelastet ist.
    - **Entlastung:** Caching, Deduplizierung über Snapshot-Hash, kleine Queue, Parallelitätsgrenze, Timeout, Drop-/Retry-Regel, Ressourcengrenzen, Feature-Flag und harter lexikalischer Fallback.
    - **Messung:** Analyse- und Queue-Latenz, CPU/RAM, Cache-Hit-Rate, Fehler-/Timeout-/Fallback-Rate sowie unveränderte Join-, Vote- und Q&A-Baselines mit deaktiviertem und künstlich langsamem Sidecar.
  - **Tests und Verifikation:**
    - Shared-Zod-Vertrag sowie Backend-Contract-Tests decken Erfolg, Modellnichtverfügbarkeit, Timeout, ungültige Antwort, leeres Ergebnis und Fallback ab.
    - Frontend-Tests decken den vollständigen Ablauf `Auslösen → läuft → aktiv → durch neue Daten veraltet → neu analysieren` sowie Fehler und Abbruch ab; bestehende Freeze-, Sortier-, Tooltip-, Filter- und Exportpfade bleiben regressionsfrei.
    - Ein Test mit künstlich langsamem Sidecar weist nach, dass weder Q&A-Einreichung noch Vote-, Join- oder Realtime-Pfade auf die Analyse warten.
    - Tooltips, CSV und textuelle barrierefreie Alternative bleiben quellenbezogen und zeigen verständlich, welche Oberflächenformen zusammengeführt wurden.
    - Vor Produktivfreigabe sind Single-Host-Ressourcenbudget, Containerlimits, Fallback und Deaktivierung auf einer isolierten Zielumgebung nachgewiesen. Mitgelieferte Modelle sind in `NOTICE` beziehungsweise der Drittlizenzseite mit Name, Version, Lizenz und Quelle ausgewiesen.
  - **Nicht-Ziele:** semantische Paraphrasencluster, Embeddings, generative Labels, Teilnehmeranalyse, Sentimentanalyse, automatische Dauerneuberechnung, neue Layout-Engine, externer SaaS-NLP-Dienst, Relizenzierung von arsnova.eu zugunsten der Modelllizenzen oder Mitlieferung von `it_core_news_sm` im MIT-Default.
  - **Abhängigkeiten und kanonische Quellen:** Story 1.14/1.14a, Story 4.5, Story 6.5, ADR-0012, ADR-0013, ADR-0025, ADR-0026, [`word-cloud-spacy.md`](docs/features/word-cloud-spacy.md), [`WORD-CLOUD-SPACY-GLAETTUNG-ZIELBILD.md`](docs/implementation/WORD-CLOUD-SPACY-GLAETTUNG-ZIELBILD.md), [`WORD-CLOUD-SPACY-GLAETTUNG-IMPLEMENTATION-PLAN.md`](docs/implementation/WORD-CLOUD-SPACY-GLAETTUNG-IMPLEMENTATION-PLAN.md) und [`WORD-CLOUD-THESIS-REVIEW-2026-05.md`](docs/implementation/WORD-CLOUD-THESIS-REVIEW-2026-05.md).
- **Story 1.14c (Word Cloud 3.0 – semantischer Q&A-Themenmodus mit eigenem Open-Weight-Inferenzserver):** 🟡 Als Lehrperson möchte ich in der Host-Q&A-Wortwolke zusätzlich zu Einzelwörtern und Begriffen/Phrasen einen erklärbaren semantischen Themenmodus aufrufen, damit sinngleiche Fragen und Paraphrasen als gemeinsame Themen sichtbar werden, ohne den Q&A-Livebetrieb von KI-Inferenz abhängig zu machen.
  - **Stand (2026-08-21, Leitplanken 2026-08-22):** Stufe 0 (Vertrag/UI) und Stufe 1 (privater Encoder + Clustering, extraktive Labels) im Repo. Kill-Switch `WORD_CLOUD_SEMANTIC_ENABLED` bleibt default aus; ohne privaten Socket/URL kein SaaS-Fallback. Stufe 2 (LLM-Labels), Presenter-Themenmodus, 8.9c Slice 4 und die Produktivfreigabe bleiben offen. Die Runtime für Stufe 2 ist mit **Story 8.9d** / [ADR-0035](docs/architecture/decisions/0035-self-hosted-llm-runtime-llama-cpp-over-ollama.md) vorentschieden (`llama.cpp`/`llama-server`, nicht Ollama); LLM-Ausfall darf nur das Label verlieren, nicht auf lexikalisch 2.x zurückfallen. Die Umsetzung von Stufe 2 selbst bleibt offen. Encoder-Clustering für Quiz-Freitext ist **Story 1.14d**, nicht mehr Teil von 1.14c. Kanonisch: [docs/features/word-cloud-semantic.md](docs/features/word-cloud-semantic.md).
  - **Umsetzungs- und Lehrabsicht:** Die Story wird als durchgehendes Studienobjekt im Cloud-Computing-Kurs demonstriert und in den Übungen schrittweise umgesetzt. Studierende erstellen technische Messungen und Wirtschaftlichkeitsanalysen, dokumentieren eine verantwortete Architekturentscheidung und vertreten diese im Referat. Eine Produktivaktivierung folgt daraus nicht automatisch, sondern erst nach den unten definierten Qualitäts-, Sicherheits-, Datenschutz-, Last- und Betriebsgates.
  - **Produktzuschnitt:**
    - Der erste Rollout ist host-first und ausschließlich für Q&A. Presenter-Q&A bleibt bis zu einer eigenen Folgestory auf dem stabilen 2.x-Pfad. Quiz-Freitext-Themen sind Story 1.14d.
    - Der Host kann orthogonal zur Gewichtung zwischen `Einzelwörter`, `Begriffe & Phrasen` und `Themen` wechseln. `Meist unterstützt`, `Beste Fragen` und `Umstritten` bleiben als Gewichtungsmodi unverändert wirksam.
    - Analysiert werden nur die im aktuellen Host-Snapshot sichtbaren `PINNED`-/`ACTIVE`-Fragen. Archivierte, nicht sichtbare oder nicht hostseitig freigegebene Inhalte gelangen nicht in den Analyseauftrag.
    - Ein Thema ist nur zulässig, wenn seine Mitgliedschaft erklärbar ist. Tooltip, Fokus-/Textalternative und CSV zeigen mindestens Label, gewichteten Wert, Metrik, Konfidenz beziehungsweise Unsicherheit, Beispiel-/Mitgliedsfragen, Modell-/Analyseversion und Erzeugungszeitpunkt.
    - Findet die semantische Pipeline keine belastbaren Cluster oder ist sie deaktiviert, überlastet, zu langsam oder fehlerhaft, bleibt die lexikalische Wolke ohne leere Zwischenkarte vollständig nutzbar.
    - Deutsch und Englisch sind für die erste Produktstufe verpflichtend zu evaluieren. Weitere App-Locales werden erst nach eigenen Fixtures und Qualitätsnachweisen aktiviert und fallen bis dahin kontrolliert lexikalisch zurück.
  - **Semantische Kaskade statt LLM-Monolith:**
    - Die messbare Referenzkaskade lautet `lexikalischer 2.x-Fallback → optionale spaCy-Glättung aus 1.14b → mehrsprachige Embeddings → deterministisches Clustering → optionales Open-Weight-LLM für lesbare Themenlabels oder kurze quellengebundene Zusammenfassungen`.
    - Embeddings und deterministisches Clustering bilden die semantische Mitgliedschaft. Das LLM darf Cluster nicht frei erfinden, sondern nur aus belegten Mitgliedsfragen verbalisieren; eine Variante ohne generatives LLM bleibt als Kosten-, Qualitäts- und Ausfallbaseline erhalten.
    - Jede Modellstufe besitzt einen klaren Early-Exit, Timeout und Fallback. Die größte beziehungsweise teuerste Modellstufe wird nur ausgeführt, wenn günstigere Stufen den Auftrag nicht ausreichend erfüllen.
    - Strukturierte Ausgaben werden serverseitig gegen einen Shared-Zod-Vertrag validiert; nicht schemafähige, quellenlose oder widersprüchliche Modellantworten werden verworfen und niemals ungeprüft dargestellt.
  - **Zielarchitektur und Infrastrukturgrenze:**
    - Der heutige belegte Baselinebetrieb bleibt Single Host: Nginx, App, PostgreSQL und Redis laufen in getrennten Diensten/Containern auf demselben Debian-Host. Diese Story darf PostgreSQL nicht fälschlich als bereits physisch getrennten Produktionsserver dokumentieren.
    - Open-Weight-LLM und Embedding-Modell laufen produktionsnah auf einem eigenen, vom Live-App-Host getrennten Inferenzserver im privaten Netz. Die Komponente ist analog zu einem separat deploybaren Datenbankdienst eine eigenständig versionierte, abgesicherte und beobachtbare Serverrolle.
    - Das Lehrlabor vergleicht mindestens die Single-Host-Baseline mit der getrennten Inferenzarchitektur; ein zusätzlich physisch getrennter PostgreSQL-Server darf als bewusste Drei-Server-Variante untersucht werden, ist aber keine stillschweigende Voraussetzung dieser Story.
    - Der Inferenzserver besitzt keinen öffentlichen Modellport. Nur der autorisierte Backend-/Workerpfad darf über private Netzregeln und einen rotierbaren Service-Credential- beziehungsweise mTLS-Vertrag zugreifen; Browser und Teilnehmende sprechen den Server nie direkt an.
    - Modell, Runtime, Quantisierung, Kontextgrenze, Container-/Image-Digest, Modell-Hash, Model Card, Lizenz und Deploymentkonfiguration sind versioniert. `Open Weight` wird nicht mit `Open Source` gleichgesetzt; Nutzung, Veränderung, Weitergabe und kommerzieller Betrieb werden vor Freigabe lizenzrechtlich geprüft.
    - Produktion, Echtdaten und Produktionscredentials bleiben aus studentischen Übungen ausgeschlossen. Die Übungen verwenden isolierte, rücksetzbare Zielserver, synthetische oder ausdrücklich freigegebene Q&A-Daten, kurzlebige Credentials, Quotas, Budgets und automatischen Cleanup.
  - **Asynchroner Ablauf und Degradation:**
    - Q&A-Fragen werden immer zuerst normal persistiert und ausgeliefert. Weder `qa.submit`, `qa.list`, Realtime-Subscriptions noch Votes warten auf Embedding- oder LLM-Inferenz.
    - Analyse startet ausschließlich hostseitig on demand oder mit einer später ausdrücklich begründeten niedrigen Aktualisierungsfrequenz, niemals automatisch pro Eingabe, Vote oder Tastendruck.
    - Ein unveränderlicher, datensparsamer Snapshot wird über einen Hash dedupliziert. Pro Session ist höchstens ein aktiver Analyseauftrag zulässig; neue Daten markieren das Ergebnis als veraltet und lösen keine verdeckte Dauerberechnung aus.
    - Queue/Worker, harte Kontext- und Batchgrenzen, Timeout, begrenzte Parallelität, Backpressure, Circuit Breaker, Drop-/Skip-/Retry-Regel, Feature-Flag und Kill-Switch sind konfiguriert und dokumentiert.
    - Der Ergebnisvertrag bildet mindestens `pending`, `ready`, `uncertain`, `stale`, `disabled`, `failed` und `fallback` ab. App- und Inferenzserver können unabhängig ausfallen und wiederhergestellt werden, ohne dass der Q&A-Kernpfad blockiert.
  - **Datenschutz, Sicherheit und Datenlebenszyklus:**
    - Der Analysesnapshot enthält keine Host-/Admin-Tokens, IP-Adressen, Participant-IDs, Nicknames oder sonstigen nicht erforderlichen Metadaten. Für semantische Mitgliedschaft werden nur die erforderlichen sichtbaren Fragetexte und anonyme Quellschlüssel übertragen.
    - Prompts, Snapshots, Embeddings, Zwischenzustände und Modellantworten werden standardmäßig nicht dauerhaft gespeichert und nicht in normalen Logs ausgeschrieben. Ein Audit-/Debugmodus erfordert eine bewusste Betreiberkonfiguration, klare Kennzeichnung, Zugriffsschutz, TTL und Löschkonzept.
    - Ergebnis-Caches sind kurzlebig, an Session-/Snapshot-Hash und Analyseversion gebunden und folgen der bestehenden Session-Retention. Eine Modell- oder Pipelineänderung invalidiert unvereinbare Ergebnisse.
    - Bedrohungsmodell und Tests behandeln mindestens Modell-/Prompt-Injection durch Fragetexte, überlange Eingaben, Ressourcenerschöpfung, manipulierte Modellantworten, Supply-Chain-/Modellrisiken, unzulässigen Netzwerkzugriff und Datenabfluss.
  - **Verbindliches Messprogramm:**
    - **Produktqualität:** kuratiertes, DSGVO-sicheres Q&A-Seed-Set mit deutschen und englischen Paraphrasen, Code-Switching, Tippfehlern, Mehrdeutigkeit, Fachbegriffen, Über- und Unterbündelung; Vergleich gegen lexikalische, spaCy-, Embedding-/Clustering- und LLM-Varianten.
    - **Qualitätsmetriken:** mindestens Pairwise Precision/Recall/F1 oder eine begründete vergleichbare Clustermetrik, Fallback-/Uncertain-Rate sowie eine nachvollziehbare menschliche Rubrik für Labeltreue, Lesbarkeit und Quellenbindung.
    - **Betrieb:** Queue-Wartezeit, Ende-zu-Ende- und Modelllatenz, Durchsatz, Timeout-/Fehler-/Fallback-Rate, Early-Exit-Anteil, Cache-Hit-Rate sowie CPU, RAM, GPU/VRAM, Netzwerk und Energie soweit messbar.
    - **Live-Regressionsschutz:** Q&A-Submit, Join, Vote, WebSocket/Reconnect und lexikalische Wortwolke werden bei deaktiviertem, langsamem, überlastetem und ausgefallenem Inferenzserver gegen die bestehende Baseline geprüft.
    - **Skalierung:** die Lehrprofile `100 parallele Classrooms × 50 Teilnehmende` und `eine Konferenz-Session × 5.000 Teilnehmende` werden als getrennte, ungetestete Szenarien modelliert; Aussagen gelten nur für die tatsächlich ausgeführte Umgebung und Last.
    - **Wirtschaftlichkeit:** Infrastruktur-, GPU-/CPU-, Energie-, Speicher-, Netzwerk-, Betriebs-, Monitoring-, Security-, Backup-, Modellpflege-, Personal-, Migrations- und Exitkosten; Kosten pro Analyse, Session und 1.000 Fragen; Auslastung/Leerlauf; Best-/Base-/Worst-Case, Sensitivität und Break-even.
    - **Entscheidungsvergleich:** self-hosted Open-Weight-Modell gegen kleinere Encoder-/CPU-Lösung und, nur als dokumentierte Marktalternative ohne stillen Produktfallback, geeignete Managed-/SaaS-Angebote; Build/Buy, Datenschutz, Lock-in, Verfügbarkeit und Exit werden aus derselben technischen Evidenz bewertet.
  - **Lehrprodukte im Cloud-Computing-Kurs:**
    - versionierte IaC-/Deploymentartefakte für App-/Baseline- und separaten Inferenzserver sowie optional die Drei-Server-Variante mit getrenntem PostgreSQL;
    - Agentenvertrag, Rechte-/Netz-/Datenflussdiagramm, Bedrohungsmodell, Modell-/Lizenzmanifest und ausgeführter Degradations-/Cleanup-Nachweis;
    - reproduzierbarer Qualitäts-, Last-, Ressourcen- und Resilienzbericht mit Rohdaten, Umgebung, Modellversion und Gültigkeitsgrenze;
    - TCO-/FinOps-Modell mit Unit Economics, Sensitivität, Build/Buy, Risiko und Exit;
    - Architecture Decision Record mit stärkster verworfener Alternative sowie ein individuelles Referat, das technische Messung und wirtschaftliche Entscheidung fachlich verteidigt.
  - **Stufenweise Freigabe und Definition of Done:**
    - Stufe 1: reproduzierbarer Offline-/Notebookvergleich auf synthetischem Seed-Set; Stufe 2: isolierter Zwei-Server-Laborpfad; Stufe 3: produktionsnahe Last-, Fehler-, Security-, Privacy- und Kostenprüfung; erst danach gesonderte Entscheidung über produktive Aktivierung.
    - Die Story ist nicht durch eine attraktive Demo abgeschlossen. Erforderlich sind schema-first Verträge, erklärbare UI/CSV/Textalternative, harter Fallback, vollständige Telemetrie, dokumentierte Budgets, Lizenzprüfung, automatisiertes Deployment/Cleanup und nachvollziehbare Mess- und Kostenberichte.
    - Relevante Backend-, Shared-Type-, Frontend-, Contract-, Security- und Lasttests sind automatisiert; jede Kursdurchführung verwendet denselben festgeschriebenen Repo-, Modell-, Daten- und Laborstand.
  - **Nicht-Ziele:** LLM im Participant- oder synchronen Live-Hotpath, automatische Moderationsentscheidung, personenbezogene Sentiment-/Aufmerksamkeitsanalyse, autonomes Grading, frei erfundene Themen, externer proprietärer Fallback, öffentliche Modell-API, sofortiger Rollout für Presenter oder ungeprüftes Kapazitätsversprechen. Encoder-Clustering für Quiz-Freitext ist Story 1.14d.
  - **Bezug zu 8.9b/8.9c:** Story 1.14c besitzt ausschließlich den host-ausgelösten Snapshot-/Clustervertrag mit den Zuständen `pending`, `ready`, `uncertain`, `stale`, `disabled`, `failed` und `fallback`. Story 8.9b behält davon getrennt ihren per persistierter Frage asynchronen Klassifikationsvertrag nach ADR-0032 einschließlich `classified`/`unclassified`; sie konsumiert weder 1.14c-Snapshots noch deren Zustände. Beide Pfade dürfen dieselbe abgesicherte und versionierte Inferenzserver-Rolle sowie gemeinsame Betriebsbausteine nutzen, bleiben aber fachlich, schema-seitig, in Cache/Queue und im Modelllebenszyklus getrennt. Story 8.9c darf dieselbe Serverrolle für einen dritten, quellengebundenen Auftrag nutzen und besitzt weiterhin nur den Moderationszusammenfassungsvertrag.
  - **Abhängigkeiten und kanonische Quellen:** Story 0.7, Story 1.14/1.14a/1.14b, Story 4.5, Story 6.5, Story 8.6/8.7/8.9a–d, ADR-0012, ADR-0013, ADR-0025, ADR-0026, ADR-0032, ADR-0035, [`WORD-CLOUD-3.0-STORY-VORSCHLAG.md`](docs/implementation/WORD-CLOUD-3.0-STORY-VORSCHLAG.md), [`PRAKTIKUM-DATA-ANALYTICS.md`](docs/praktikum/PRAKTIKUM-DATA-ANALYTICS.md), [`Technische_Quellen_ARSnova.md`](docs/didaktik/Modulkonzept%20Cloud%20Computing/Technische_Quellen_ARSnova.md), [`CLOUD-COMPUTING-AGENTIC-LEHRLABOR.md`](docs/didaktik/CLOUD-COMPUTING-AGENTIC-LEHRLABOR.md) und [`Themenblockplan_Cloud_Computing_12_Themenbloecke.md`](docs/didaktik/Modulkonzept%20Cloud%20Computing/Themenblockplan_Cloud_Computing_12_Themenbloecke.md).
- **Story 1.14d (Word Cloud 3.1 – semantischer Freitext-Themenmodus auf dem 1.14c-Encoder):** 🟡 Als Lehrperson möchte ich in der Host-Freitext-Wortwolke denselben erklärbaren Themenmodus wie in Q&A aufrufen, damit sinngleiche Antworten — vom Einzelwort bis zum Satz, etwa bei „Schreibt, wie ihr euch gerade fühlt“ — als gemeinsame Themen sichtbar werden, ohne den Live-Abstimmungspfad von Inferenz abhängig zu machen.
  - **Stand (2026-08-21):** Offen. Die Stufe-0-UI (Toggle **Themen**) ist vorhanden und fällt in 1.14c Stufe 1 kontrolliert auf 2.x (`channel: 'FREETEXT'` → `status: fallback`). Diese Story hebt den Fallback auf, sobald der Encoder von 1.14c Stufe 1 läuft.
  - **Warum eigene Story:** Freitext ist kein Einwort-Kanal. Impulsfragen erzeugen oft Sätze und Mischformen. Das ist ein anderer Snapshot als Q&A-Paraphrasen (andere IDs, Längen, Gewichtung, Qualität). Derselbe Sidecar, anderer Auftrag.
  - **Produktzuschnitt:**
    - Gilt für die **aktuelle** Host-Freitextfrage und deren sichtbare Antworten. Andere Fragen, Q&A und Presenter bleiben unverändert.
    - Der vorhandene Toggle `Einzelwörter` / `Wörter & Phrasen` / `Themen` bleibt. Ohne Kill-Switch, ohne Sidecar, bei Timeout oder ohne belastbares Cluster bleibt die lexikalische Wolke vollständig nutzbar; keine leere Karte.
    - Analysiert werden die im Host-Snapshot enthaltenen Antworttexte. Snapshot-Items tragen `channel: 'FREETEXT'` und stabile anonyme Quellschlüssel; die Heuristik `response-\d+` darf Encoder-Clustering nicht mehr pauschal unterbinden.
    - Ein Thema ist nur zulässig bei erklärbarer Mitgliedschaft. Tooltip, Fokus-/Textalternative und CSV zeigen Label, Gewichtung, Mitgliedertexte, Konfidenz und Modell-/Analyseversion wie in 1.14c.
    - Mindestclustergröße 2. Eine einzelne Antwort oder nur Singletons ergeben `fallback` (Hinweis **Themen sind gerade nicht belastbar**), nicht `uncertain` mit „Mitgliedsfragen prüfen“.
    - Pflichtlocales der ersten Freitext-Stufe: `de` und `en`. `fr`/`es`/`it` fallen kontrolliert lexikalisch zurück, bis eigene Fixtures vorliegen.
    - `SEMANTIC + LEMMA` bleibt `MODE_UNSUPPORTED`; die Glättung bleibt im Themenmodus ausgeblendet.
  - **Qualität und Schwelle:**
    - Seed-Set mit drei Familien: (1) Stimmungssätze zu einem Impuls wie „Wie fühlt ihr euch?“, (2) Einwort-Synonyme, (3) lange, fachliche Sätze; plus Gegenbeispiele, die nicht zusammenfallen dürfen.
    - Startpunkt ist Complete-Linkage und Kosinus 0,87 aus 1.14c.2. Eine abweichende Freitext-Schwelle braucht eine neue Analyseversion und Fixtures, keinen zweiten Kill-Switch.
    - Derselbe Encoder (`WORD_CLOUD_SEMANTIC_ENABLED`, Compose-Profil `encoder`). Kein neues Image, keine öffentliche URL, kein LLM-Label (bleibt 1.14c Stufe 2).
  - **Live-Pfad:** `session.submitFreetext` / Vote / Join / WebSocket warten nicht auf Inferenz. Analyse nur hostseitig on demand; neue Antworten markieren `stale` plus **Neu analysieren**.
  - **Akzeptanzkriterien:**
    - Bei aktivem Encoder und Locale `de`/`en` liefert `wordCloud.analyze` mit `mode: SEMANTIC` und `channel: FREETEXT` belastbare Cluster statt hartem `fallback`.
    - Die Stimmungs-Familie im Seed fällt zusammen; fachlich fremde Antworten bleiben getrennt.
    - Eine einzelne Nonsense-Antwort zeigt den bestehenden Fallback-Hinweis in `de`/`en`/`fr`/`es`/`it`, keinen Unsicher-Hinweis.
    - Q&A-Themen, lexikalische Freitext-Wolke und Presenter regressieren nicht.
    - Hotpath-Isolation und Contract-Tests analog 1.14c; Produktdoku [`word-cloud-semantic.md`](docs/features/word-cloud-semantic.md) um den Freitext-Kanal ergänzt.
  - **Nicht-Ziele:** Presenter-Freitext-Themen, Q&A-Pipeline umbauen, LLM-Labels, neuer Sidecar oder Kill-Switch, Sentiment-Scoring einzelner Teilnehmender, automatische Neuberechnung pro Tastendruck, SaaS-Fallback.
  - **Abhängigkeiten und kanonische Quellen:** Story 1.14/1.14a/1.14b/1.14c Stufe 1, Story 4.5, Story 6.5, ADR-0012, ADR-0013, ADR-0025, [`word-cloud-semantic.md`](docs/features/word-cloud-semantic.md), [`WORD-CLOUD-3.0-STORY-VORSCHLAG.md`](docs/implementation/WORD-CLOUD-3.0-STORY-VORSCHLAG.md).
...
- **Story 2.5 (Passive Presenter-/Beamer-Ansicht):** 🔴 Als Lehrperson projiziere ich Quiz, Q&A oder Blitzlicht in einer ruhigen, rein anzeigenden Ansicht, während die Host-Steuerung auf meinem Gerät bleibt.
  ...
  - **Akzeptanzkriterien:**
    ...
    - **End-Phase (`FINISHED`):** finales Leaderboard bei vorhandener Beteiligung (Story 4.1), Belohnungseffekte (Story 5.4). Ohne Stimmen bleibt nur der zentrierte Hinweis „Die Session ist beendet.“ sichtbar. Verlässt die Lehrperson die Host-Ergebnisseite („Zur Startseite“) oder beendet die Session ohne Ergebnisseite, wechselt die Presenter-Ansicht auf dasselbe Exit-Branding. Die passive Projektion bietet in allen Fällen keine Navigation zur Startseite an und kann während der anschließenden Lehrveranstaltung geöffnet bleiben.
...
- **Story 2.9 (Asynchrone Quiz-Modi und Feedback-Strategien):** 🔴 ⬜ Offen · **noch nicht beauftragt** — Als Lehrperson möchte ich beim Start einer Quiz-Instanz festlegen können, ob das Quiz **host-gesteuert synchron** oder **teilnehmendenindividuell asynchron** läuft und wann Rückmeldungen sichtbar werden, damit dieselben Quiz-Inhalte sowohl im Plenum als auch im eigenen Tempo bearbeitet werden können.
  ...
  - **Akzeptanzkriterien:**
    - **Pacing-Konfiguration:** Beim Start einer Quiz-Instanz wählt die Lehrperson genau einen Pacing-Modus: `Synchron (host-gesteuert)`, `Asynchron manuell (Self-Paced)` oder `Asynchron zeitgesteuert (timeboxed)`. Der bestehende synchrone Modus bleibt Default und muss ohne Verhaltensänderung weiter funktionieren.
    - **Feedback-Strategie:** Für asynchrone Modi ist zusätzlich eine Feedback-Strategie Pflicht: `Sofort nach jeder Frage` oder `Verzögert erst nach Quizabschluss`. Im synchronen Modus gilt weiterhin die bestehende Auflösung durch die Lehrperson; es entsteht kein zweiter widersprüchlicher Feedback-Schalter.
    - **Persistenz und DTOs:** Pacing-Modus und Feedback-Strategie werden in Quiz-/Session-Konfiguration, Zod-Schemas, Prisma-Modell und Import-/Exportformat eindeutig serialisiert. Alte Quizdaten ohne diese Felder werden migriert bzw. beim Laden auf `Synchron (host-gesteuert)` normalisiert.
    - **Synchroner Legacy-Flow:** Im Modus `Synchron` bleiben Statusmodell, Host-Steuerung, Presenter-Ansicht, Countdown, Lesephase, Peer-Instruction-Flow und Ergebnisfreigabe kompatibel zu Stories 2.3, 2.6, 2.7, 3.3a/b, 3.4 und 4.4.
    - **Asynchron manuell:** Teilnehmende sehen ihren eigenen Quizfortschritt, beantworten Frage für Frage und können nach einer gültig gespeicherten Antwort selbst zur nächsten Frage wechseln. Zurücknavigation ist nur lesend erlaubt, solange dadurch keine bereits gespeicherte Antwort verändert oder doppelt gesendet wird.
    - **Asynchron zeitgesteuert:** Pro Frage gilt der konfigurierte Timer. Nach Ablauf wird die Eingabe für die jeweilige teilnehmende Person gesperrt und der Client wechselt automatisch zur nächsten Frage bzw. zum Abschluss. Die maßgebliche Deadline kommt serverseitig bzw. aus einem serverzeitbasierten Zeitmodell; Client-Drift darf keine zusätzlichen Antwortfenster öffnen.
    - **Unbeantwortete Fragen:** Läuft eine timeboxed Frage ohne gültige Antwort ab, wird ein neutraler `keine Antwort`-/Timeout-Zustand gespeichert, der mit 0 Punkten in Scorecard, Leaderboard, Export und Dashboard eingeht. Leere oder ungültige Eingaben erzeugen keinen fachlich gültigen Vote.
    - **Sofortiges Feedback:** Bei `Sofort nach jeder Frage` sieht die teilnehmende Person nach Antwortabgabe bzw. Timeout die eigene Korrektheit, erreichte Punkte, richtige Lösung und optionalen Auflösungstext. Diese Rückmeldung ist persönlich und darf keine Rohdaten anderer Teilnehmender enthalten.
    - **Verzögertes Feedback:** Bei `Verzögert erst nach Quizabschluss` bestätigt der Client nach jeder Antwort nur den Eingang. Richtige Lösungen, Korrektheit, Punkte und Auflösungstexte bleiben bis zum persönlichen Abschluss verborgen. Data-Stripping gilt auch in asynchronen Zwischenzuständen.
    - **Abschlussansicht:** Nach Abschluss des gesamten Quiz sieht die teilnehmende Person eine persönliche Scorecard mit Ergebnis je Frage, Gesamtpunktzahl und Rang, sofern Ranglisten aktiviert sind. Bei verzögertem Feedback wird die vollständige fachliche Rückmeldung erst hier sichtbar.
    - **Host-/Presenter-Dashboard:** Für aktive asynchrone Sessions erhalten Host und Presenter ein Live-Dashboard mit aggregiertem Fortschritt pro Frage: Anteil begonnen, beantwortet, offen, Timeout und abgeschlossen. Eine optionale Korrektheits-/Fehlerquote pro Frage wird nur aggregiert angezeigt und darf keine korrekte Antwort oder einzelne Teilnehmende verraten.
    - **Beamer-Schutz:** Wenn die Presenter-Ansicht für Teilnehmende sichtbar ist und verzögertes Feedback aktiv ist, zeigt sie standardmäßig Fortschritt ohne Lösungsdetails. Eine Korrektheits-Heatmap ist als Host-/Presenter-Option möglich, muss aber klar als potenziell lösungsnah gekennzeichnet sein und darf nie Antworttexte als richtig/falsch markieren, bevor die Feedback-Strategie dies erlaubt.
    - **Rangliste und Bonuscodes:** Nach Session-Ende bzw. Abschluss aller relevanten Teilnahmen erzeugt das System eine Rangliste nach bestehender Scoring-Regel. Bonuscodes werden analog Story 4.6 generiert und ausschließlich auf der persönlichen finalen Scorecard der berechtigten Teilnehmenden angezeigt; Host-Export und Bonuslisten bleiben geschützt durch Host-Token.
    - **Kompatibilität mit Modi:** Anonymer Modus, freie Nicknames, Team-Modus, Presets, Lesephase, Kurzantwort, numerische Schätzfragen, Rating, Survey und Freitext werden explizit behandelt. Nicht unterstützte Kombinationen müssen im Startdialog verständlich deaktiviert oder erklärt werden, statt zur Laufzeit zu brechen.
    - **Session-Lifecycle:** Asynchrone Quiz-Modi sind keine langfristigen Hausaufgaben-Sessions. Sie folgen demselben Session-Lebenszyklus, Cleanup und Datenschutzmodell wie andere Live-Sessions, sofern keine eigene spätere Retention-Story etwas anderes entscheidet.
    - **Export:** Ergebnisexporte enthalten Pacing-Modus, Feedback-Strategie, Abschlussstatus und aggregierte Fortschrittsdaten. Nickname-/Ranglisten-Exporte folgen den bestehenden Datenschutz- und Host-Token-Regeln.
    - **Performance:** Fortschritts- und Korrektheitsaggregate werden so voraggregiert, gecacht oder gebatcht, dass asynchrone Sessions mit vielen Teilnehmenden nicht pro Teilnehmeraktion teure Vollabfragen über alle Votes auslösen.
...
- **Story 2.10 (Smartphone oder vertrauenswürdige Person als zweiten Host verbinden / Paired Host; UX- und Sicherheits-Erweiterung für Presenter-Start und Smartphone-Pairing):** 🟡 ⬜ Offen – Als Lehrperson möchte ich mein Smartphone oder das Gerät einer vertrauenswürdigen Tutor:in beziehungsweise Moderator:in mit einer bereits laufenden Veranstaltung als **zweiten vollwertigen Host** verbinden, damit die Veranstaltung parallel gesteuert und moderiert werden kann, während der Laptop oder ein anderer Bildschirm die Presenter-Ansicht zeigt.
  ...
  - **Vertrauensmodell und Einsatz als Moderator:in:**
    ...
    - Vor der Freigabe weist die UI unmissverständlich darauf hin, dass der Paired Host neben Q&A und Moderationskompass auch Quizphasen, Ergebnisse, Blitzlicht, Presenter-Flächen und das Session-Ende steuern kann.
    - Der Quiz-Export schützt die wiederverwendbare Quizdefinition, macht Fehlbedienung oder Missbrauch in einer laufenden Session aber nicht folgenlos: Vorzeitig gezeigte Lösungen, übersprungene Phasen und veränderte Live-Ergebnisse bleiben mögliche Auswirkungen.
    - Benötigt eine Organisation Zugänge für nicht vollständig vertrauenswürdige Personen oder echte Least-Privilege-Rechte, darf sie keinen Paired Host koppeln; ein solches Rollenmodell ist nicht Bestandteil dieser Story.
  ...
  - **Delegation als vollwertiger Host:**
    - Pairing-Austausch, gehashte Multi-Token-Registry, Token-ID, TTL, Widerruf, Geräteanzeige sowie HTTP-/WebSocket-Prüfung bilden den einzigen vorgesehenen Unterbau für delegierte Session-Zugänge.
    - `PAIRED_HOST` darf die bestehende Host-Shell verwenden und wird bei regulären Host-Aktionen serverseitig wie ein Host autorisiert.
    - Es gibt keine separate Moderator-Route, kein Moderator-Token und keine zweite eingeschränkte Moderationsoberfläche.
...
> **Tag:** `epic-5` · UX-Verbesserungen (Phasen-Labels, Lesephase-Banner, Letzte-Frage-Hinweis, Richtig-Badge, KaTeX-Scrollbar, Join/Diskussion, Foyer-Einflug) sind in den Session-Features integriert.
...
- **Story 5.4a (Foyer-Einflug im Preset Spielerisch):** 🟡 Als Teilnehmende:r möchte ich im Preset `Spielerisch` während der Connecting-Phase sehen, wie bunte Teilnehmenden-Chips in das Foyer einfliegen, damit der Einstieg lebendig, motivierend und unverwechselbar wirkt.
  ...
  - **Umsetzungsstand Stand 2026-05-30:** Implementiert und geprüft. Die Host-Lobby rendert im spielerischen Quiz-Foyer eine eigene Arrival-Schicht (`foyer-entrance-layer`) für neue Teilnehmende, Teamkarten erhalten lokale Arrival-Overlays, große Join-Wellen werden gestaffelt bzw. unterdrückt, der Effekt ist auf `PLAYFUL`, Lobby/Connecting und `enableRewardEffects` gegatet, und `prefers-reduced-motion` nutzt reduzierte Animationen. Das Teilnehmendengerät zeigt nach erfolgreichem Join einen einmaligen, kleineren Ankunftsmoment.
  - **Verifizierung:** `npm run test -w @arsnova/frontend -- src/app/features/session/session-host/session-host.component.spec.ts src/app/features/session/session-vote/session-vote.component.spec.ts src/app/features/join/join.component.spec.ts` ✅.
...
`docs/EPIC6-AC-PRUEFUNG.md`). Story 6.5 ist technisch validiert (u. a.
#89–#106, #101, #104, #105, #116) und seit 2026-07-27 formal mit
AT-/Zoom-/OS-/Reader-Prüfungen nach WCAG 2.2 AA abgenommen. Story 6.6
(**qualitative UX-Testreihen** Thinking Aloud inkl. Umsetzung der Befunde)
ist abgeschlossen ✅. Damit ist Epic 6 geschlossen.
...
- **Story 6.4 (Mobile-First & Responsive Design):** 🔴 Als Teilnehmende:r möchte ich die App auf meinem Smartphone komfortabel bedienen können, da ich im Hörsaal primär mein Handy nutze.
  - **Akzeptanzkriterien:**
    ...
    - Touch-Targets (Buttons, Antwortoptionen) sind grundsätzlich mindestens
      44×44px groß (Projektstandard über dem WCAG-2.2-AA-Minimum aus 2.5.8;
      dokumentierte Ausnahmen bleiben möglich).
...
- **Story 6.5 (Barrierefreiheit / Accessibility):** 🔴 ✅ Als Nutzer mit Einschränkungen möchte ich die App vollständig per Tastatur, Screenreader und assistive Technologien bedienen können. _(Technische Gates und formale AT-/OS-/Zoom-/Reader-Abnahme nach WCAG 2.2 AA am 2026-07-27 abgeschlossen. Siehe [`Accessibility-Umsetzungsjournal`](docs/praktikum/ACCESSIBILITY-UMSETZUNGSJOURNAL.md) und [`Accessibility-Audit`](docs/praktikum/ACCESSIBILITY-AUDIT-WCAG-2.2-AA.md).)_
  - **Akzeptanzkriterien:**
  ...
  - **Stand (2026-07-27):** Technische Maßnahmen und automatische Gates sind auf `main` gemergt — u. a. Fokus/Overlays/Semantik (#89–#90), automatisierte Gates (#91), PDF/UA (#92), Restblocker inkl. persönlicher Timer/Host-Override/Punktvorschau/Session-Headings (#101, #104), Erklärung zur Barrierefreiheit unter `/legal/accessibility` (#105), News-Archiv-Tastatur-/Fokus-Härtung (#106), Feature-MOTD zur Initiative (#116). Die formale Prüfmatrix mit VoiceOver/Safari, NVDA/Firefox, 200-/400-%-Zoom, Windows High Contrast und PDF-Readern ist ohne offene A-/AA-Befunde abgeschlossen. Story 6.5 ist **fertig**.
...
- **Story 6.7 (Startseite: Hero-Chips & Kanal-Einstiege zuverlässig):** 🟢
  ...
  **Akzeptanzkriterien (abnahmefähig):**
  ...
  - [x] **Vollbild / immersive Host** (App-Chrome ausgeblendet): Pfad B bleibt nutzbar; Session-Ende ist **ohne** sichtbare Top-Toolbar möglich.
...
## Epic 8: Erweiterte Live-Kanäle (Q&A, Blitzlicht)

> **Verifizierung Epic 8 (2026-08-25):** Der Q&A-Kernumfang 8.1–8.4, die Host-Sortiermodi 8.6/8.7, Story 8.8 (Tempo-Blitzlicht als Host-Option) sowie 8.9a/8.9b sind umgesetzt. Vertrauenswürdige Tutor:innen oder Moderator:innen sollen diese Host-Funktionen künftig als Paired Hosts aus Story 2.10 nutzen. Details zu 8.6/8.7: [`docs/features/controversy-score.md`](docs/features/controversy-score.md). Zielmodell fuer 8.8: [`docs/architecture/decisions/0029-tempo-as-predefined-blitzlicht-template.md`](docs/architecture/decisions/0029-tempo-as-predefined-blitzlicht-template.md).
...
- **Story 8.6 (Q&A: Kontroversitäts-Score & Sortierung):** 🟡 Als Host einer Live-Veranstaltung möchte ich Fragen im Q&A nach Kontroversität (ausgeglichene Up- und Downvotes) sortieren können, damit polarisierende Themen sichtbar werden und nicht nur durch hohe Upvote-Zahlen dominieren.
  ...
  - **Produktabgleich Stand 2026-08-25:** Umsetzung als **optionaler Sortiermodus in der Host-Ansicht**, nicht als neue globale Standardsortierung; `PINNED` bleibt oberhalb jeder Score-Sortierung, Scoring wirkt nur innerhalb sichtbarer Statusgruppen. Vertrauenswürdige Tutor:innen oder Moderator:innen erhalten den Modus als Paired Hosts aus Story 2.10.
...
- **Story 8.8 (Tempo-Blitzlicht als Host-Option):** 🟡 Als Lehrperson möchte ich im bestehenden Blitzlicht ein vordefiniertes Tempo-Blitzlicht starten können, sowohl in der Session als auch im Standalone-Blitzlicht, damit Teilnehmende während eines Vortrags jederzeit anonym rückmelden können, ob sie folgen, ob es schneller gehen darf, ob es zu schnell ist oder ob sie abgehängt sind.
  - **Akzeptanzkriterien:**
...
- **Story 8.9a (Deterministischer Live-Moderationskompass):** 🟡 ✅ **Fertig.** Als Host möchte ich während einer Live-Session eine **regelbasierte, quellenbelegte Lageeinschätzung** aus bereits vorhandenen Session-Signalen erhalten, damit ich den nächsten Moderationsschritt bewusst wählen kann, ohne neue Inferenz- oder Datenschutzrisiken in den Live-Pfad zu ziehen.
  - **Produktziel / MVP:** Der erste Moderationskompass ist bewusst **ohne NLP-/LLM-Komponente** umsetzbar. Er liest nur vorhandene Host-Signale, macht Quellen sichtbar und schlägt vorsichtige, didaktische Handlungsoptionen vor. Er ist damit die robuste Fallback-Basis für spätere KI-/NLP-Erweiterungen.
  - **Akzeptanzkriterien:**
    ...
    - Vertrauenswürdige Tutor:innen oder Moderator:innen erhalten den Kompass ausschließlich als vollwertige Paired Hosts gemäß Story 2.10.
...
- **Story 8.9b (Optionale Q&A-NLP-Kaskade für Moderationssignale):** 🟡 Als Host möchte ich optional asynchron berechnete Q&A-Kategorien und Unsicherheitswerte im Moderationskompass sehen, damit Themenballungen schneller sichtbar werden, ohne dass das Einreichen oder Anzeigen von Q&A-Fragen von Inferenz abhängt.
  ...
  - **Akzeptanzkriterien:**
    ...
    - Teilnehmer-DTOs enthalten keine internen NLP-Ergebnisse, Modellversionen, Konfidenzen oder Moderationsartefakte. Sichtbarkeit ist hostseitig.
  ...
  - **Tests / Verifikation:**
    ...
    - DTO-Stripping-Test für Teilnehmer- und Host-Sicht.
  ...
  - **Abhängigkeiten:** Story 8.9a, Story 2.10 für vertrauenswürdige Co-Hosts, ADR-0013, ADR-0025, ADR-0026, ADR-0032.
- **Story 8.9c (Optionale generative Moderationszusammenfassung):** 🟢 Als Host möchte ich optional eine kurz formulierte, quellengebundene Zusammenfassung aus dem Moderationskompass erhalten, damit ich die Lage schneller verbalisieren kann, ohne dass ein LLM Aktionen ausführt oder ungeprüfte Aussagen als Wahrheit erscheinen.
  ...
  - **Stand (2026-08-20, Leitplanken 2026-08-22):** Slices 1–3 plus lokaler Loopback-Helfer und Snapshot-Ranking im Repo (Shared-Vertrag, Host-Kompass-Button, ephemere Queue, privater HTTP-Adapter, Near-Duplicate-Kanonisierung; PRs [#298](https://github.com/kqc-real/arsnova.eu/pull/298), [#301](https://github.com/kqc-real/arsnova.eu/pull/301)). Kill-Switch `QA_SUMMARY_ENABLED` bleibt default aus; ohne `QA_SUMMARY_INFERENCE_URL` kein Cloud-Fallback. Extraktive Kurzfassung lebt heute nur im Dev-Helfer; Slice 4 muss sie in die Backend-Queue ziehen. `QA_SUMMARY_INFERENCE_URL` wird nicht auf `llama-server` `/v1/chat/completions` gelegt. Echtes LLM folgt mit **Story 8.9d** / [ADR-0035](docs/architecture/decisions/0035-self-hosted-llm-runtime-llama-cpp-over-ollama.md) nach Prefill-Messung, nicht als DoD „gleich Gemini unter 8 s“. Kanonisch: [docs/features/qa-summary.md](docs/features/qa-summary.md). Die Story ist nicht fertig, solange Slice 4 (Modell) fehlt.
  - **Abgrenzung zu Story 1.14c:** Wenn Story 1.14c umgesetzt und freigegeben ist, darf 8.9c denselben abgesicherten Inferenzserver über einen getrennten schema- und quellengebundenen Auftrag wiederverwenden. Betrieb, Modelllebenszyklus und Servergrenze liegen bei **Story 8.9d** (Runtime: ADR-0035) im Rahmen von 1.14c; 8.9c besitzt nur den Moderationszusammenfassungsvertrag.
  - **Akzeptanzkriterien:**
  ...
  - **Abhängigkeiten:** Story 8.9a, optional Story 8.9b, Story 8.9d, Story 6.5, ADR-0025, ADR-0026, ADR-0032, ADR-0035.
- **Story 8.9d (Selbstgehosteter Open-Weight-LLM-Server: Runtime-Baustein für 1.14c/8.9c/#456):** 🟢 Als Lehrperson oder Moderator:in möchte ich, dass echte, generative Themenlabel (1.14c) sowie Moderations-Kurzfassungen (8.9c) und Lernzielvorschläge (#456) auf einem selbstgehosteten Open-Weight-Modell laufen können, damit ich von besseren KI-Vorschlägen profitiere, ohne dass Fragen- oder Antworttexte an einen kommerziellen SaaS-Anbieter gehen.
  - **Produktziel:** Diese Story liefert ausschließlich den **Runtime- und Betriebsbaustein** für den privaten LLM-Server, den Story 1.14c Stufe 2 (Labels), Story 8.9c Slice 4 (Summary) und Issue #456 Slice 5 (Lernzielableitung) über drei getrennte Aufträge auf derselben Serverrolle nutzen. R muss die versionierten Auftragsschemas einschließlich Lernziel-Ein-/Ausgabe und deren Adapterübersetzung bereits technisch unterstützen; fachlicher Lernzielprompt, Ableitungslogik und Host-UI folgen in #456 Slice 5. Sie ersetzt nicht das umfangreiche Mess-, Sicherheits-, Datenschutz-, FinOps- und ADR-Programm aus Story 1.14c „Zielarchitektur und Infrastrukturgrenze“/„Verbindliches Messprogramm“; diese Story liefert nur die konkrete Serving-Runtime, auf der jenes Programm aufsetzt. Kanonisch: [ADR-0035](docs/architecture/decisions/0035-self-hosted-llm-runtime-llama-cpp-over-ollama.md).
  - **Stand (2026-08-22):** ADR-0035 legt die Runtime fest: `llama.cpp`/`llama-server` (MIT), nicht Ollama, nicht vLLM (CPU-Rolle), nicht Hugging-Face-TGI. Modellwahl unverändert aus der Voranalyse: `Qwen3-4B-Instruct-2507` Q4_K_M (Apache 2.0), non-thinking. Leitplanken nach 1.14b/1.14c Stufe 1/8.9c Slices 1–3 nachgezogen (Topologie, ein Slot, kein OpenAI-Drop-in, Fallback in der App). Noch nicht implementiert.
  - **Ergänzung 2026-09-22 / [Issue #456](https://github.com/kqc-real/arsnova.eu/issues/456):** Runtime weiterhin **nicht implementiert**. Separater Runtime-PR **R** nach ADR-0035 ist verbindliche Voraussetzung vor #456 Slice 5 (Lernzielableitung); Slices 1–4 dürfen ohne Runtime umgesetzt werden. Implementierung: Grok 4.7 High Fast; Review: Codex. R umfasst Runtime, privaten Node-Adapter, gemeinsamen Slot-Schutz und realen lokalen Modellnachweis; die produktionsnahe Summary-Prefill-Abnahme bleibt zusätzlich erforderlich.
  - **Dritter Auftrag aus #456:** Explizit gestartete Lernzielableitung nutzt dieselbe Runtime mit eigenem Vertrag/Budget/Timeout. Slot belegt → manueller Retry, kein Modell-Queueing; Abbruch gibt den Slot frei. Keine automatische Präemption; Live-Label-/Summary-Aufträge nutzen bei belegtem Slot ihren Fallback. Manuelle/bestätigte Ziele bleiben erhalten. Details und Abnahme: [ADR-0035, Ergänzung zu #456](docs/architecture/decisions/0035-self-hosted-llm-runtime-llama-cpp-over-ollama.md#ergänzung-2026-09-22-runtime-voraussetzung-für-issue-456).
  - **Reihenfolge:** Runtime-Baustein dieser Story zuerst (Image, Profil, Flags, gemeinsames Inflight, Health, Tests ohne Modell-Download) → 1.14c Stufe 2 (kurze Labels) → 8.9c Slice 4 erst nach Prefill-Messung auf der echten 8-vCPU-Box. Für #456 gilt zusätzlich: Slices 1–4 → Abnahme R einschließlich Drei-Auftrags-Vertragstests → Slice 5. Ein kurzer Label-/Schemaauftrag belegt R lokal, ohne damit die fachlichen Label-/Summary-Stufen als fertig zu erklären.
  - **Akzeptanzkriterien:**
    - Der LLM-Server läuft als eigener, privater Prozess/Container ohne öffentlichen Port: Compose-Profil `llm`, eigenes Image, Image-Digest gepinnt. Kanonische Produktion ist der **zweite private Host** mit privatem HTTP plus Credential. Unix-Socket (`--host …sock`) nur Same-Host-Labor, nicht zusammen mit spaCy- und Encoder-Sidecar auf dem 16-GB-Live-Host. macOS-Dev spricht Loopback-HTTP (`127.0.0.1`), nicht den Docker-Volume-Socket.
    - URL-Ziele werden aufgelöst und danach nur als Loopback/RFC1918/ULA akzeptiert; öffentliche SaaS-Hosts bleiben gesperrt. Token nie in der URL. `QA_SUMMARY_INFERENCE_URL` zeigt **nicht** auf `llama-server` `/v1/chat/completions`.
    - Ein schmaler Node-Client übersetzt die versionierten App-Verträge nach Chat-Completions; `json_schema`/GBNF **pro Request und Auftragstyp** (Label ≠ Summary ≠ Lernzielableitung). R enthält Shared-Ein-/Ausgabeschemas, Auftragserkennung und validierte Adapterübersetzung für alle drei Typen; unbekannte Typen, schemainkompatible Antworten und unzulässige Quellenreferenzen werden abgewiesen. Kein globales `--json-schema` am Server.
    - Kill-Switch `OPEN_WEIGHT_LLM_ENABLED` (nur exakt `true`) ist unabhängig von `NLP_ENABLED`, `QA_NLP_ENABLED`, `WORD_CLOUD_SEMANTIC_ENABLED` und `QA_SUMMARY_ENABLED`. LLM aus lässt Stufe-1-Themen und 8.9a unberührt; ohne URL bleibt die 8.9c-Karte unsichtbar.
    - Modell ist ein gepinntes, digest-versioniertes GGUF-Artefakt als Volume, nicht CI-Image-Layer; kein automatischer Laufzeit-Pull gegen eine öffentliche Registry.
    - Hartflags mindestens: `--parallel 1` (nicht auto), `--ctx-size` 2048 oder 4096 nach KV-Messung (nie Modelldefault), `--n-predict` gedeckelt, `--no-webui`, `--slots`, `--api-key`, `--reasoning off`, `--n-gpu-layers 0`, `--slot-prompt-similarity 0`. Offizielles Docker-Beispiel `--host 0.0.0.0` wird nicht übernommen.
    - Ein **gemeinsames** Inflight über Label-, Summary- und Lernzieladapter. Belegter Slot: `GET /slots?fail_on_no_slot=1`; Label/Summary nutzen sofort ihren definierten Fallback, Lernzielableitung liefert einen typisierten Belegt-Zustand für manuellen Retry ohne neue Vorschläge. Keine interne llama.cpp-Warteschlange und keine automatische Präemption. Ohne `--slots` liefert der Server 501 statt 503; das App-Inflight bleibt die erste Sperre. Client-Abort gibt den Slot frei. Timeouts liegen unter den jeweiligen Backend-Budgets und sind nicht die Gemini-8-s-Kopie. Circuit Breaker analog Encoder-Client.
    - Slice-4-Timeout/Backpressure liefert extraktive Bullets **in der Backend-Queue**, nicht nur im Dev-Helfer. Stufe-2-Ausfall behält extraktive Labels (Clustering bleibt Stufe 1). Das LLM ändert keine Mitgliedschaft.
    - Slice 4 sendet eine gekürzte, schon gerankte Quellenliste; DoD ist nicht „gleich Gemini unter 8 s“.
    - `deploy.sh` startet den Server nicht; Aktivierung ist ein bewusster Betreiberschritt. Live-Hotpfade (`qa.submit`, Join, Vote, WebSocket) warten nie auf diesen Server.
  - **Tests / Verifikation:**
    - Sidecar-Tests ohne Modell-Download (Profil `llm`, kein öffentlicher Port, Ressourcenlimits, Health, Hartflags), analog `npm run test:spacy-sidecar` / `test:wordcloud-encoder`.
    - Hotpath-Isolation: Join/Vote/`qa.submit`/WS unverändert bei totem, langsamem oder überlastetem LLM-Server.
    - Drei-Auftrags-Isolation: jede Paarung Label/Summary, Label/Lernziel und Summary/Lernziel in beiden Belegungsreihenfolgen sowie drei gleichzeitig eintreffende Aufträge prüfen. Maximal ein Modellaufruf läuft; abgewiesene Live-Aufträge erhalten ihren Fallback, abgewiesene Lernzielaufträge einen Belegt-Zustand. Abbruch/Timeout jedes Auftragstyps gibt den Slot für jeden anderen Typ frei; kein gegenseitiges Timeout hinter einer Modell-Queue.
    - Vertragstests für alle drei Auftragsschemas: gültige Übersetzung, falscher Auftragstyp, schemainkompatible Antwort und unbekannte Referenzen. OpenAI-Rohantwort ohne Übersetzung wird vom 8.9c-Adapter abgelehnt; Summary bindet nur Snapshot-Quellen, Lernzielausgabe nur zulässige Aufgabenreferenzen. R darf dafür Lernziel-Fixtures verwenden; fachliche Ableitungsqualität wird erst in #456 Slice 5 abgenommen.
    - Allowlist-Test: öffentliche DNS-Namen und SaaS-Hosts abgelehnt; Loopback und nach Auflösung private Adressen erlaubt.
  - **Nicht-Ziele:**
    - Keine fachliche Promptoptimierung oder Host-UI in R: Labels bleiben fachlich bei 1.14c, Summary bei 8.9c, Lernzielableitung und deren UI bei #456 Slice 5. Die zur technischen Runtime-Abnahme erforderlichen versionierten Schemas und Adapterverträge für alle drei Aufträge sind ausdrücklich Bestandteil von R.
    - Kein Ersatz für die extraktiven Fallback-Pfade; der Gemini-Helfer bleibt nur Dev-Orakel.
    - Keine Bring-your-own-LLM-Option für Hosts (vom PO am 2026-08-22 verworfen).
    - Keine GPU-Beschaffung; bleibt CPU-only, solange keine gesonderte FinOps-/Security-Entscheidung vorliegt.
...
## Epic 11: Verlagszugänge & Redaktionsbackend (Westermann) — Angebotsoption / noch nicht beauftragt

> **Rolle:** Westermann-Redakteur:in.  
> **Status:** ⬜ Offen; derzeit **Angebots-/Planungsstand**, noch **nicht beauftragt**.  
> **Abgrenzung:** Dieses Epic beschreibt einen **separaten Redaktionsbereich** für die Westermann-gebrandete Quizoberfläche und ist **nicht** mit Epic 9 (technischer Plattform-Admin für Session-Inspektion, Löschung, Behördenexport) gleichzusetzen.  
> **Nicht Ziel dieses Epics:** signierte Quizdateien, serverseitige Allowlist-Prüfung einzelner Lehrkraft-Importe, gesonderte Freischaltcodes pro Quiz oder ein erweitertes Lizenzmodell über die Verlagsaccounts hinaus.
> **Implementierungsplan:** [`docs/implementation/EPIC-11-IMPLEMENTATION-PLAN.md`](docs/implementation/EPIC-11-IMPLEMENTATION-PLAN.md)

- **Story 11.1 (Verlagszugänge: personalisierte Redaktionsaccounts):** 🔴 Als Westermann-Redakteur:in möchte ich mich mit einem **persönlichen Verlagsaccount** in einen geschützten Redaktionsbereich einloggen können, damit nur autorisierte Westermann-Mitarbeitende Quizze der Westermann-Version erstellen und verwalten können.
  - **Akzeptanzkriterien:**
    - Es stehen initial **7 getrennte Verlagsaccounts** zur Verfügung.
    - Die Anmeldung erfolgt **serverseitig** über einen geschützten Login; ein gemeinsamer Sammelzugang ist nicht das Zielbild.
    - Nur angemeldete Verlagsaccounts erhalten Zugriff auf den Redaktionsbereich.
    - Lehrkräfte und Teilnehmende haben keinen Zugriff auf den Redaktionsbereich.
    - Aktionen im Redaktionsbereich sind mindestens fachlich einem konkreten Verlagsaccount zuordenbar.

- **Story 11.2 (Redaktionsbackend: Quizverwaltung):** 🔴 Als Westermann-Redakteur:in möchte ich Quizze im Redaktionsbereich **erstellen, bearbeiten, speichern, importieren und exportieren** können, damit ich Verlagsinhalte eigenständig pflegen und wiederverwenden kann.
  - **Akzeptanzkriterien:**
    - Quizze können neu angelegt, bearbeitet und gespeichert werden.
    - Quizze können importiert und exportiert werden.
    - Unterstützte Austauschformate werden vor Umsetzung festgelegt; die Angebotsanfrage nennt beispielhaft **Excel und/oder JSON**.
    - Der Editor unterstützt **Basis-Textformatierung**.
    - Der Editor unterstützt das **Einfügen von Bildern**.
    - **Audio- und Video-Inhalte** sind als optionale Erweiterung fachlich vorgesehen, aber nicht automatisch Pflichtumfang der ersten Ausbaustufe.

- **Story 11.3 (Redaktionsbackend: Veröffentlichung & Quizlink):** 🔴 Als Westermann-Redakteur:in möchte ich nach dem Speichern eines Quiz **direkt einen Quizlink** erhalten, damit das Quiz in den jeweiligen Verlagsprodukten verwendet werden kann.
  - **Akzeptanzkriterien:**
    - Nach dem Speichern wird ein direkter Quizlink bereitgestellt.
    - Der Link ist zur Verwendung in den jeweiligen Verlagsprodukten vorgesehen.
    - Die Quiz-Erstellung und Link-Bereitstellung bleiben auf Verlagsaccounts beschränkt.
    - Die fachliche Verknüpfung zwischen gespeichertem Quiz und ausgegebenem Link ist nachvollziehbar.

- **Story 11.4 (Redaktionsbackend: Passwort/Token-Schutz & accountbezogener Gesamtexport):** 🔴 Als Westermann-Redakteur:in möchte ich je Quiz einen **Zugriffsschutz für Lehrkräfte** konfigurieren und meine Quizdaten **accountbezogen gesammelt exportieren** können, damit ich Nutzung und Datenbestand meines Redaktionsaccounts steuern kann.
  - **Akzeptanzkriterien:**
    - Pro Quiz kann festgelegt werden, ob zum Start einer Quiz-Instanz ein **Passwort oder Token** erforderlich ist.
    - Die konkrete Produktausprägung des Startschutzes wird vor Umsetzung festgelegt und deckt die in der Angebotsanfrage beschriebenen Varianten fachlich ab (z. B. global gleiches Passwort oder quizindividuell festgelegtes Passwort/Token).
    - Der gewählte Zugriffsschutz wird zusammen mit dem Quiz gespeichert.
...
> **Rolle:** Betreiber/Product Owner lernt fortlaufend aus der realen Nutzung von arsnova.eu; Hosts und Teilnehmende können mit minimalem Aufwand verwertbares Produktfeedback geben.
>
...
- **Story 12.1 (Rollenspezifisches Zwei-Klick-Produktfeedback nach Sessionende):** ✅ Implementiert und am **2026-09-10 manuell abgenommen.** Als Host oder teilnehmende Person möchte ich nach einer tatsächlich genutzten Session mit zwei kurzen Auswahlen anonym mitteilen können, wie einfach und hilfreich arsnova.eu für meine jeweilige Aufgabe war, damit der Betreiber kontinuierlich vergleichbare Produktsignale erhält, ohne mich zu einer E-Mail oder längeren Texteingabe zu zwingen.
  ...
  - **Zweiter Klick – größter Einflussbereich:**
    ...
    - Nach **„Einfach“** oder **„Ja“** lautet sie **„Was hat heute besonders gut funktioniert?“**. Dadurch werden neben Problemen auch schützenswerte Produktstärken erfasst.
    - Teilnehmende erhalten die Bereiche **Session beitreten**, **Orientierung in der App**, **Antwort abgeben**, **Q&A oder Blitzlicht**, **Ergebnisse verstehen**, **Technik oder Verbindung**, **Barrierefreiheit**, **Etwas anderes**.
    - Hosts erhalten die Bereiche **Quiz vorbereiten**, **Session starten**, **Teilnehmende einladen**, **Live-Session steuern**, **Q&A oder Blitzlicht**, **Ergebnisse auswerten**, **PDF oder Export**, **Technik oder Verbindung**, **Barrierefreiheit**, **Etwas anderes**.
    - Mit Auswahl des Bereichs wird die strukturierte Rückmeldung sofort vollständig und atomar gespeichert. Ein Text ist nicht erforderlich.
  ...
  - **Teilnehmer-Trigger und Stichprobe:**
    ...
    - Nach Antwort, Schließen oder **„Für diese Befragung nicht mehr fragen“** gilt ein clientseitiger Cooldown. Standard: 30 Tage nach Antwort oder Schließen; dauerhafte Unterdrückung für die konkrete Befragungsversion nach expliziter Abwahl. Dafür wird ausschließlich versionierter lokaler Zustand verwendet, kein geräteübergreifendes Profil.
  - **Host-Trigger:**
    - Ein Host ist nur nach einer erfolgreich beendeten Session mit mindestens einer teilnehmenden Person und mindestens einer fachlichen Interaktion geeignet; abgebrochene oder reine Test-Sessions ohne Nutzung lösen keine Einladung aus.
  ...
  - **Persistenz, Bereinigung und Adminaggregation:**
    - Strukturierte Datensätze werden maximal 13 Monate aufbewahrt, damit saisonale beziehungsweise semesterbezogene Vergleiche möglich sind; optionale Freitexte werden nach spätestens 90 Tagen automatisch entfernt, sofern sie nicht zuvor datenschutzgerecht in ein bearbeitetes internes Arbeitselement überführt wurden.
    - Eine autorisierte Adminansicht zeigt Anzahl und Verteilung je Fragefamilie/-version, Rolle, Primärantwort, Bereich, Locale, App-Version, grober Sessiongröße und Zeitraum.
    - Zentrale Auswertungen sind mindestens: Anteil **Einfach / Mit kleinen Hürden / Schwierig**, Anteil **Ja / Teilweise / Nein**, häufigste positive Bereiche, häufigste Hürden sowie Antwortanzahl und Einladungs-/Abschlussquote.
    - Kleine Gruppen werden in Ausgaben geschützt: Kombinationen mit weniger als fünf Antworten werden nicht als fein segmentierte Statistik dargestellt.
  - **Fehler-, Offline- und Zustandsverhalten:**
    - Pending, Erfolg, typisierte Ablehnung, Tokenablauf, Timeout und erneuter Versuch besitzen eindeutige, lokalisierte Zustände.
...
- **Story 12.2 (Jederzeit erreichbares „arsnova.eu verbessern“ mit Admin-Triage):** ✅ Implementiert und am **2026-09-10 manuell abgenommen.** Als Nutzer:in möchte ich von jeder persönlichen arsnova.eu-Ansicht aus mit höchstens zwei kurzen Auswahlen ein Problem, eine Unklarheit, einen Wunsch oder eine Stärke melden können, damit meine Beobachtung ohne E-Mail, Login oder Kenntnis des GitHub-Repositories den Produktverantwortlichen mit dem notwendigen technischen Kontext erreicht.
  - **Auffindbarkeit und Informationsarchitektur:**
    ...
    - **„arsnova.eu verbessern“** steht in einer klar getrennten Utility-Zeile oberhalb beziehungsweise neben der dreiteiligen Footer-Navigation. Auf schmalen Viewports darf die Aktion in den regulär scrollbaren Footerbereich wechseln, bleibt aber außerhalb der drei primären Navigationseinträge und wird nicht als dauerhaft klebende Leiste über fachlichen Inhalten oder Sessionaktionen dargestellt.
    - Auf der ersten sichtbaren Ebene der bestehenden Hilfe unter **„So funktioniert’s“** (`/help`) wird die Aktion zusätzlich unmittelbar über oder neben den vorhandenen Rollenkarten angeboten; die Hilfeseite ist jedoch nicht der einzige oder primäre Einstieg.
  ...
  - **Situatives Angebot bei beobachtbaren Hürden:**
    - Zusätzlich zum jederzeit erreichbaren Einstieg wird **„Problem mit arsnova.eu melden“** als sekundäre Aktion angeboten, wenn eine konkrete technische oder fachliche Bedienhürde bereits sichtbar geworden ist, etwa nach einem fehlgeschlagenen oder abgelaufenen Vote-Submit, einem Timeout, einem Verbindungsabbruch, einer gescheiterten Hostaktion, einem erforderlichen Retry oder einem Exportfehler.
    - Die unmittelbar mögliche Problemlösung beziehungsweise Wiederholung bleibt immer die primäre Aktion. Das Produktfeedback-Angebot öffnet sich niemals automatisch, blockiert keine Bedienung und wird nicht allein aus Inaktivität, Verweildauer oder vermuteter Verwirrung abgeleitet.
    - Wiederholte Hinweise für dasselbe Fehlerereignis werden dedupliziert und mit einem kurzen gerätelokalen Cooldown begrenzt. Schließen oder Ignorieren verändert weder den fachlichen Fehlerzustand noch Session-, Vote-, Host- oder Realtime-Zustand.
    - Der typisierte Fehler-/Request-Kontext darf Feedbackart und betroffenen Bereich für die Reihenfolge der Auswahlmöglichkeiten vorschlagen, aber niemals vorauswählen, automatisch absenden oder die Nutzerentscheidung ersetzen.
    - Nach Schließen kehrt der Fokus zur sichtbaren Fehlermeldung beziehungsweise zur auslösenden Wiederholungs- oder Hilfeaktion zurück.
  - **Erster Klick – Art der Rückmeldung:**
    ...
    - Auswahl: **„Etwas funktioniert nicht“**, **„Etwas ist unverständlich“**, **„Mir fehlt eine Funktion“**, **„Etwas gefällt mir besonders gut“**.
    - Alle vier Wege sind gleichwertig erreichbar; positive Rückmeldungen werden bewusst angeboten, damit die Auswertung nicht ausschließlich problemgetrieben ist.
  - **Zweiter Klick – betroffener Produktbereich:**
    - Die Anschlussfrage lautet **„Worum geht es?“**. Der aus der aktuellen Route ableitbare Bereich wird an erster Stelle angeboten, aber nicht ohne Nutzerentscheidung vorausgewählt oder abgesendet.
    - Teilnehmendenbereiche: **Beitritt**, **Quizfrage oder Antwort**, **Q&A**, **Blitzlicht**, **Ergebnis oder Punkte**, **Darstellung oder Barrierefreiheit**, **Technik oder Verbindung**, **Etwas anderes**.
    - Hostbereiche: **Quiz-Sammlung oder Editor**, **Sessionstart und Einladung**, **Live-Steuerung**, **Q&A**, **Blitzlicht**, **Ergebnisse**, **PDF oder Export**, **Darstellung oder Barrierefreiheit**, **Technik oder Verbindung**, **Etwas anderes**.
    - Außerhalb einer bekannten Rolle werden nur zur Route passende allgemeine Bereiche angezeigt.
    - Der zweite Klick speichert bereits eine vollständige strukturierte Rückmeldung. Freitext, Kontaktangabe und weitere Formulare sind dafür nicht erforderlich.
  - **Optionale Konkretisierung und Auswirkung:**
  ...
  - **Verhalten in laufenden Sessions:**
    - Beim Öffnen aus einer aktiven persönlichen Session steht sichtbar **„Die Session läuft im Hintergrund weiter.“**
    - Timer, persönliche Fristen, Punkte, Vote-Entwürfe, Q&A-/Blitzlichtzustand, Hoststeuerung, Ergebnisfreigabe und Realtime-Verbindungen werden weder pausiert noch verändert.
    - Dialog/Sheet kann jederzeit geschlossen werden; danach kehrt der Fokus zuverlässig zur auslösenden Hilfeaktion zurück. Liveaktualisierungen ersetzen oder schließen den Dialog nicht.
    - Nach dem Senden bleibt der Nutzer auf derselben Route und im selben Kanal. Es erfolgt keine automatische Navigation.
  - **Offline-Postausgang und Wiederholung:**
    - Kann die Rückmeldung nicht gesendet werden, lautet der Status sinngemäß: **„Deine Rückmeldung ist auf diesem Gerät vorgemerkt und wird erneut gesendet, sobald arsnova.eu erreichbar ist.“**
  ...
  - **Admin-Postfach und Triage:**
    - Nur gültig authentifizierte Admins erhalten eine paginierte, filterbare Produktfeedback-Liste; Aufruf der Route allein verleiht keine Berechtigung.
    - Filter mindestens nach Zeitraum, Quelle, Rolle, Feedbackart, Bereich, Auswirkung, App-Version, Locale und Bearbeitungsstatus.
