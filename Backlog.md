...
>
> **Nächster Fokus (Auswahl offener Stories):** u. a. **2.9** (asynchrone Quiz-Modi, noch nicht beauftragt), **1.2ec–1.2ed** (Kurzantwort-Ausbau), **1.14b–1.14c** (spaCy-Glättung und Word Cloud 3.0), **8.5** (delegierbare Q&A-Moderation), **8.9a–8.9c** (didaktischer Live-Moderationskompass) — **Epic 6** ist einschließlich der formalen WCAG-2.2-AA-Abnahme von **6.5** und der abgeschlossenen UX-Testreihen **6.6** umgesetzt ✅. **Lehre:** Greenfield-Demo **1.7a** in **3×45 Min.** — [`docs/didaktik/greenfield-demo-1-7a-vorlesung.md`](docs/didaktik/greenfield-demo-1-7a-vorlesung.md).
>
...
| 1    | 1.14b | Word Cloud 2.6 (optionale spaCy-Glättung auf dem Single Host)                      | 🟡   | ⬜ Offen       |
...
> **Verifizierung Epic 1 (2026-03-09, ergänzt bis 2026-08-13):** Die umgesetzten Kern-Stories in Epic 1 sind auf **✅ Fertig** gesetzt — siehe Übersichtstabelle. **Offen** bleiben aktuell **1.2ec–1.2ed** und **1.14b–1.14c** (dort ⬜). **1.14a** ist mit den produktiven Ausbaustufen 2.1 bis 2.5 abgeschlossen; **1.14b** ist die kurzfristig geplante sprachliche Glättung auf dem Single Host; **1.14c** ist die getrennte semantische Ausbaustufe und das Studienobjekt des Cloud-Computing-Kurses. **1.6c** ist technisch fertig; der automatische UUID-only-Legacy-Cutoff am **1. Oktober 2026** ist nur noch ein betrieblicher Termin. **1.6d** ist mangels nachgewiesenem Performanceproblem geschlossen und nicht geplant. **Story 1.2f** ist aus A11y-Gründen **🚫 geschlossen**. **Story 1.2d**, **Story 1.2e**, **Story 1.2ea**, **Story 1.2eb**, **Story 1.2g**, **Story 1.2h**, **Story 1.2i**, **Story 1.2j**, **Story 1.7a** und **Story 1.7b** sind umgesetzt und auf **✅ Fertig** gesetzt.
...
- **Story 1.14a (Word Cloud 2.0 – echtes Layout + Premium-UX):** 🟡 ✅ **Fertig.** Als Lehrperson möchte ich Freitext-Antworten in einer echten, dichten und präsentationstauglichen Word-Cloud sehen, damit häufige Begriffe im Livebetrieb auf Smartphone, Host-Ansicht und Beamer ruhig, lesbar, interaktiv und exportierbar dargestellt werden.
  ...
  - **Akzeptanzkriterien:**
    ...
    - **Sprachliche Normalisierung entkoppelt:** Story 1.14a hält Cleaning, Tokenisierung, Stopwortlogik und Layout erweiterbar, implementiert aber keine neue NLP-Laufzeit. Die optionale spaCy-Glättung mit eigenem Fallback-, Ressourcen- und Qualitätsvertrag ist vollständig Story 1.14b.
  ...
  - **Performance-Hinweis:** Die Story ist nach [ADR-0025](docs/architecture/decisions/0025-treat-future-extensions-as-performance-critical-until-proven-otherwise.md) performance-kritisch, weil Live-Layout, Re-Rendering, Aggregation und Exportqualität im Presenter-/Host-Pfad liegen. Die zusätzlichen Ressourcen- und Degradationsverträge für spaCy und semantische Inferenz werden getrennt in Story 1.14b beziehungsweise Story 1.14c geführt.
...
- **Story 1.14b (Word Cloud 2.6 – optionale spaCy-Glättung auf dem Single Host):** 🟡 Als Lehrperson möchte ich in der Freitext- und Q&A-Wortwolke auf Wunsch unterschiedliche Sprachformen zu stabilen, lesbaren Begriffen zusammenführen, damit Flexionen die inhaltliche Verdichtung nicht künstlich aufsplitten und die bestehende lexikalische Wortwolke vollständig erklärbar bleibt.
  - **Umsetzungsabsicht:** Diese Story ist die nächste eigenständige Ausbaustufe der Wortwolke und soll kurzfristig umgesetzt werden. Sie konsolidiert die bisher in Story 1.14a sowie den spaCy-Konzept- und Planungsdokumenten verstreuten Anforderungen.
  ...
  - **Akzeptanzkriterien – Auslösung und Zustände:**
    ...
    - Die Analyse verarbeitet einen unveränderlichen Snapshot der aktuell sichtbaren Daten. Neue Fragen oder Antworten starten keine Neuberechnung, sondern markieren den geglätteten Stand als veraltet und bieten `Neu analysieren` an.
  ...
  - **Abhängigkeiten und kanonische Quellen:** Story 1.14/1.14a, Story 4.5, Story 6.5, ADR-0012, ADR-0013, ADR-0025, ADR-0026, [`WORD-CLOUD-SPACY-GLAETTUNG-ZIELBILD.md`](docs/implementation/WORD-CLOUD-SPACY-GLAETTUNG-ZIELBILD.md), [`WORD-CLOUD-SPACY-GLAETTUNG-IMPLEMENTATION-PLAN.md`](docs/implementation/WORD-CLOUD-SPACY-GLAETTUNG-IMPLEMENTATION-PLAN.md) und [`WORD-CLOUD-THESIS-REVIEW-2026-05.md`](docs/implementation/WORD-CLOUD-THESIS-REVIEW-2026-05.md).
