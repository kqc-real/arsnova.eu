...
test('real mode accepts the Slice-2D full-coverage flag on a complete baseline', () => {
  ...
  assert.equal(run.status, 0, run.stderr || run.stdout);
  assert.match(run.stdout, /Complete: 121/);
...
test('real router tree inventory follows mounted and nested routers exactly', async () => {
  ...
  assert.equal(procedures.length, 129);
  assert.equal(procedures.filter((procedure) => procedure.kind === 'query').length, 52);
  assert.equal(procedures.filter((procedure) => procedure.kind === 'mutation').length, 69);
...
test('real gate report is deterministic and complete coverage has no legacy debt', () => {
  ...
  try {
    ...
    assert.equal(report.summary.queriesMutations, 121);
    assert.equal(report.summary.complete, 121);
    ...
    assert.equal(queriesMutations.length, 121);
    assert.ok(
