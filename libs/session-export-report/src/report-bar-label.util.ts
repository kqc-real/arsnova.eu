...
function getEmojiSvgHtml(emoji: string): string | null {
  // Primary survey/quiz emojis mapped to crisp vector icons
  if (emoji === '😄') {
...
function getEmojiFontIndependentLabel(emoji: string): string {
  switch (emoji) {
    case '😄':
      return '[Stimmung: Sehr gut]';
    case '😃':
      return '[Stimmung: Glücklich]';
    case '😀':
      return '[Stimmung: Grinsend]';
    case '😊':
      return '[Stimmung: Zufrieden]';
    case '😐':
      return '[Stimmung: Neutral]';
    case '😢':
      return '[Stimmung: Traurig]';
    case '😭':
      return '[Stimmung: Weinend]';
    case '😡':
      return '[Stimmung: Wütend]';
    case '👍':
      return '[Auswahl: Ja]';
    case '👎':
      return '[Auswahl: Nein]';
    case '⭐':
      return '[Auswahl: Stern]';
    case '🚀':
      return '[Auswahl: Rakete]';
    default:
      return '[Option]';
  }
}
...
export function formatReportBarLabelHtml(
  ...
  const textFallback = getEmojiFontIndependentLabel(emoji);

  const emojiContent = svgHtml
    ? `<span class="report-emoji-svg-wrap" aria-hidden="true">${svgHtml}</span><span class="report-emoji-glyph" aria-hidden="true">${escapeHtml(emoji)}</span><span class="report-emoji-text-fallback">${escapeHtml(textFallback)}</span>`
    : `<span class="report-emoji-glyph report-emoji-glyph--unmapped" aria-hidden="false">${escapeHtml(emoji)}</span><span class="report-emoji-text-fallback">${escapeHtml(textFallback)}</span>`;
