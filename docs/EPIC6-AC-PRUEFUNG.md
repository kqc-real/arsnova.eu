...
Stand: Abschlussprüfung der Stories 6.1–6.4 gegen Codebase. Story 6.5 (Barrierefreiheit) bleibt als kontinuierlicher Qualitäts-Checkpoint (WCAG-Audit) bestehen. **Story 6.6** (UX-Testreihen _Thinking Aloud_ inkl. Umsetzung der Befunde) ist in [`Backlog.md`](../Backlog.md) beschrieben; der Nachweis erfolgt über **Dokumentation und Traceability** (siehe unten), nicht über eine feste Code-Matrix wie bei 6.1–6.4.
...
## Story 6.6 — UX-Testreihen (_Thinking Aloud_) & Umsetzung der Befunde

**Zweck:** Formative Nutzer:innen-Forschung mit der Methode **Thinking Aloud** (siehe ausführliche Beschreibung und Akzeptanzkriterien in [`Backlog.md`](../Backlog.md), Story **6.6**). Erkannte UI/UX-Probleme werden **priorisiert** und die für die Testreihe als **Must/Should** markierten Anpassungen **im Produkt umgesetzt** (Wording/i18n, Layout, Interaktion — konsistent mit ADR-0005, ADR-0008, ADR-0014).

**Abgrenzung zu 6.5:** 6.6 fokussiert **verständliche Bedienung und Informationsarchitektur** aus Nutzerperspektive; 6.5 bleibt der **systematische WCAG-/A11y-Checkpoint**. Befunde können sich überschneiden (z. B. fehlendes Label).

### Nachweis / Review-Checkliste (DoD der Story)

| Artefakt                   | Erwartung                                                                                                                  |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **Testplan**               | Schriftlich: Zielgruppe (mind. Host + Teilnehmende), Geräte (mind. Smartphone-Viewport), Kernflows/Tasks, Erfolgskriterien |
| **Durchführung**           | Mindestens eine Reihe, mindestens drei Testpersonen (oder gleichwertig dokumentiert); Einwilligung wenn Aufzeichnung       |
| **Protokoll & Auswertung** | Pro Sitzung Kurzprotokoll; daraus **Auswertungsdokument** mit **priorisierter Maßnahmenliste**                             |
| **Nachverfolgung**         | Jede Maßnahme: Status **umgesetzt** (PR/Commit-Referenz), **zurückgestellt** oder **abgelehnt** — mit kurzer Begründung    |
| **Umsetzung im Code**      | Für als Must/Should gewählte Punkte: tatsächliche Änderungen im Repo; bei Texten alle Locales (`de/en/fr/es/it`)           |

**Hinweis:** Konkreter Ablageort der Artefakte (z. B. `docs/ux/` oder verlinktes Wiki) beim ersten Testlauf festlegen und hier optional verlinken.

**Fazit (Platzhalter):** ⬜ Offen — nach erster abgeschlossener Testreihe und Umsetzung der priorisierten Maßnahmen dieses Abschnitt aktualisieren (Kurzfazit + Link zu Artefakten).
