...
## Bezug zu Story 8.9c (2026-08-20)
...
- Snapshot-Ranking: PINNED/PENDING vor Upvote, Near-Duplicate-Kanonisierung, 8.9b-`CLASSIFIED` nur als Tie-Break; Modell sieht nur `{ id, kind, text }`
- Quellenbindung `qa-question:{uuid}`; unsourced Aussagen werden verworfen; belegte Aussagen machen das Ergebnis `ready`
- Privater HTTP-Adapter; ohne URL oder bei SaaS-Host kein Cloud-Fallback
- Lokaler Loopback-Helfer `npm run qa-summary:dev` (nicht Produktion)
