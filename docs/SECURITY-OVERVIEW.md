...
- **Quiz-Sync-Zugriff:** Der Relay akzeptiert ausschließlich `quiz-library-room-<UUID>`. Share-Zugang läuft über signierte Query-Tokens (`?s=`, ADR-0033 / W3.4); die Rotations-Capability liegt nur auf dem Ursprungsgerät. Manuelles „Sync-Link ungültig machen“ erhöht die Generation. Bis `YJS_SHARE_LEGACY_UUID_CUTOFF_AT` (Default 2026-10-01 UTC) bleibt UUID-only als Grace erlaubt — danach ist die Raum-UUID allein kein Bearer mehr. Fehlendes `?s=` autorisiert kein Origin-Claim; Legacy-Räume werden bei Bedarf rekeyed. Nginx loggt mit `$uri` ohne Query/Referer; die App entfernt `s` nach Import aus der Browser-URL.
