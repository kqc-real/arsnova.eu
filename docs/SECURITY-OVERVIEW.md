...
- **Paired Host (Story 2.10 Slice 1):** Zusätzliche Host-Geräte entstehen nur über request-and-approve. Session-Code, QR oder Invite-Secret allein sind kein Host-Recht; auch der Privat-Pfad hat keinen silent grant. Nur der ursprüngliche Host darf einladen, freigeben, auflisten oder widerrufen. Ein `PAIRED_HOST` nutzt ein eigenes, gehashtes Token über dieselben `x-host-token`-Pfade (HTTP und WebSocket), darf aber nicht weiterkoppeln. Caps: max. 3 Paired Hosts, 1 aktive Einladung, 1 Pending; Rate-Limits nur auf Pairing-Endpunkten. Contracts: `@arsnova/shared-types` `host-pairing`.
