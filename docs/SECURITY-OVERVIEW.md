...
- **Quiz-Sync-Zugriff:** Der Relay akzeptiert ausschließlich `quiz-library-room-<UUID>`. Share-Zugang läuft über signierte Query-Tokens (`?s=` im WebSocket-Handshake, ADR-0033 / W3.4); Browser-Share-Links verwenden das nicht an HTTP-Server gesendete Fragment `#s=`. Die Rotations-Capability liegt nur auf dem Ursprungsgerät. Manuelles „Sync-Link ungültig machen“ erhöht die Generation und terminiert bereits verbundene Clients älterer Generationen sofort; eine zweite Autorisierung unmittelbar vor dem WebSocket-Attach schließt das Rotations-Race. Create/Rotate werden in Produktion erst nach bestätigtem Redis-AOF-Fsync (`WAITAOF`) beantwortet, sodass ein bestätigter Widerruf auch Redis-/Host-Crash überlebt. Bis `YJS_SHARE_LEGACY_UUID_CUTOFF_AT` (Default 2026-10-01 UTC) bleibt UUID-only als Grace erlaubt — danach ist die Raum-UUID allein kein Bearer mehr. Legacy-Upgrades schreiben keinen Redis-Zustand. Alte UUIDs lassen sich nicht nachträglich registrieren: Das Backend erzeugt beim Absichern immer einen neuen Raum; bestehende Origins starten diesen Rekey explizit. Persistierte Tokens werden vor dem Provider-Aufbau geladen; importierte Ersatz-Tokens werden generationsmonoton behandelt und erst nach erfolgreichem WebSocket-Sync persistiert, Rotationen über `storage` zwischen Tabs synchronisiert. Nginx loggt in HTTP und HTTPS mit `$uri` ohne Query/Referer; das Error-Log ist für den tokenführenden Yjs-Upgrade-Pfad deaktiviert und abgewiesene Relay-Upgrades werden aggregiert protokolliert.
...
- **Host / Present:** `session.create` liefert ein **Host-Token**; das Frontend speichert es pro Session-Code in `sessionStorage`. Für die Presenter-Ansicht in einem zweiten Tab schreibt das lazy Session-Feature denselben Token zusätzlich kurzlebig (TTL) in IndexedDB, weil Tablets `sessionStorage` bei `window.open` nicht klonen; nach erfolgreichem Restore wird der IndexedDB-Eintrag konsumiert. Das ist ein UX-Handoff, kein AuthService und keine zweite Autorisierung. Host-only-Prozeduren laufen zentral über **`hostProcedure`** und erwarten `x-host-token`. Die Routen `/session/:code/host` und `/session/:code/present` sind clientseitig tokengebunden; ohne Token Redirect auf Join oder Zugriff verweigert.
- **Paired Host (Story 2.10 Slices 1–5):** Zusätzliche Host-Geräte entstehen nur über request-and-approve. Session-Code, QR oder Invite-Secret allein sind kein Host-Recht; auch der Privat-Pfad hat keinen silent grant. Nur der ursprüngliche Host darf einladen, freigeben, auflisten oder widerrufen. Ein `PAIRED_HOST` nutzt ein eigenes, gehashtes Token über dieselben `x-host-token`-Pfade (HTTP und WebSocket), darf aber nicht weiterkoppeln. Caps: max. 3 Paired Hosts, 1 aktive Einladung, 1 Pending; Rate-Limits nur auf Pairing-Endpunkten. Widerruf beendet HTTP und bestehende Host-WebSockets sofort. DoD-Nachweise: `load:smoke:host-pairing-security`, Browser-Smoke `smoke:host-pairing-security`, Classroom-30/500-Cap-Last. Contracts: `@arsnova/shared-types` `host-pairing`.
...
- **Blitzlicht-Codeauflösung:** Der kombinierte Resolver prüft den Redis-Hauptdatensatz und den optionalen Session-Kontext gemeinsam. Ein gültiger Standalone-Code wird dadurch direkt aus Redis aufgelöst und löst keinen PostgreSQL-Session-Lookup aus. Das reduziert unnötige Datenbanklast und verhindert, dass automatische Poll-/Reconnect-Abfragen als explizite Codeeingaben bewertet werden (PRs [#161](https://github.com/kqc-real/arsnova.eu/pull/161) und [#164](https://github.com/kqc-real/arsnova.eu/pull/164)).
- **Teilnehmende:** Öffentliche Join-/Vote-Pfade mit Session-Code. Teilnehmerdaten sind auf Minimalzwecke geschnitten: Nickname-Kollisionen für Join, eigener Datensatz für Vote, keine öffentliche Voll-Liste. Pro Session gilt **höchstens eine Teilnahme je browserweiter `anonymousClientId`** (nur als Hash gespeichert): ohne gültiges Rejoin-Token erzeugt derselbe Client keine Zweitstimme und erhält auch **keine** neue Capability (`anonymousClientId` ist Bindungs-/Throttle-ID, kein Besitzbeweis). Ungültige Session-Codes werden nach anonymer Browser-Client-ID sowie Code-/Global-Soft-Cap geschützt; es gibt keinen Join-IP-Lock. Session-Erstellung verwendet ein grobes Shared-NAT-IP- plus Globalbudget, Vote-Submit wird pro Teilnehmenden-ID begrenzt. Restlücke ohne Login: neues Gerät, Incognito oder geleerter Storage.
- **Quiz-Sammlungs-Historie:** Endpunkte wie `session.getBonusTokensForQuiz`, `session.getLastSessionAnalysisForQuiz` und `session.getActiveQuizIds` verlangen zusätzlich einen **besitzgebundenen `accessProof`** zur hochgeladenen Quizkopie. Die Historie ist damit nicht mehr allein über `quizId` öffentlich enumerierbar. Nach erfolgreichem Bind (`historyScopeId`) ist die Capability die **Scope-UUID**; ein content-abgeleiteter Legacy-SHA-256-Hex öffnet Historien-Endpunkte nicht mehr. `bindQuizHistoryScope` akzeptiert Legacy auf bereits gebundenen Kopien nur bis `QUIZ_HISTORY_LEGACY_PROOF_CUTOFF_AT` (Default 2026-09-01 UTC), damit Clients den lokalen Proof einmalig auf die UUID migrieren. **Residualrisiko:** Ungebundene Uploads nutzen weiterhin den Content-Hash als einzige Capability — wer Inhalt+Namen kennt, kann bis zum Bind claimen. Nach Cutoff verlieren unmigrierte lokale Karten ohne UUID-Proof den Historienzugriff (accountfrei, keine Kontowiederherstellung).
...
Der optionale Word-Cloud-Encoder (Story 1.14c Stufe 1) folgt demselben Unix-Socket-Muster in
einem eigenen Python-Image (`docker/wordcloud-encoder/`), nicht im Node-App-Container und
nicht im Browser. Er hat keinen TCP-Port (`network_mode: none`), läuft capability-frei,
read-only mit begrenztem `/tmp` und ist auf 1 CPU / 2 GiB RAM / 64 PIDs limitiert.
Compose startet ihn nur über das Profil `encoder`; `deploy.sh` lässt ihn aus. Der
...
- **Härtungsplan (externes Audit + UX/NAT-Nachträge, Plan-Review geschärft 2026-07):** [SECURITY-HARDENING-PLAN.md](SECURITY-HARDENING-PLAN.md) — W0–W3 sind technisch umgesetzt; W3.7-Betriebsabnahme und S6.5-Formalabnahme bleiben offen
...
**Status-Hinweis:** W3.5 Astro 7, W3.6 Offsite-Backups und W3.7 Monitoring/Admin-Tab sind auf `main` gemergt. Die technische W3.7-Implementierung ist noch nicht operativ abgenommen; der offene, lokal validierte Dauerlast-Slice aus PR [#165](https://github.com/kqc-real/arsnova.eu/pull/165) ersetzt weder dessen Merge noch die S6.5-Zielhostabnahme.
