...
- **Quiz-Sync-Zugriff:** Der Relay akzeptiert ausschließlich `quiz-library-room-<UUID>`. Share-Zugang läuft über signierte Query-Tokens (`?s=`, ADR-0033 / W3.4); die Rotations-Capability liegt nur auf dem Ursprungsgerät. Manuelles „Sync-Link ungültig machen“ erhöht die Generation und terminiert bereits verbundene Clients älterer Generationen sofort. Bis `YJS_SHARE_LEGACY_UUID_CUTOFF_AT` (Default 2026-10-01 UTC) bleibt UUID-only als Grace erlaubt — danach ist die Raum-UUID allein kein Bearer mehr. Legacy-Upgrades schreiben keinen Redis-Zustand. Alte UUIDs lassen sich nicht nachträglich registrieren: Das Backend erzeugt beim Absichern immer einen neuen Raum; bestehende Origins starten diesen Rekey explizit. Persistierte Tokens werden vor dem Provider-Aufbau geladen; ein Ersatz-Token ersetzt den laufenden Provider. Nginx loggt in HTTP und HTTPS mit `$uri` ohne Query/Referer; die App entfernt `s` nach Import aus der Browser-URL.
