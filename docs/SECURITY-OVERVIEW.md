...
- **Verwaiste Quiz-Uploads:** Der stündliche Scheduler löscht nach **24 Stunden Grace Period** höchstens 13 atomare Batches zu je 100 Uploadkopien. Ein einzelnes SQL-CTE-Delete pro Batch schließt Quizzes und ganze Scopes mit Session-Historie bereits **vor** `LIMIT` und erneut beim Delete aus. `FOR UPDATE SKIP LOCKED` und serialisierbare Transaktionen verhindern Starvation und den Wettlauf mit `session.create`/Attach; aktive und historische Sessionkopien werden nicht gelöscht.
