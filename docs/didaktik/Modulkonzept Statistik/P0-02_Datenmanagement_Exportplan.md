...
- individuelle Leistungsprofile, Ranglisten mit Personenbezug oder ein Abgleich mit Kurs-, Prüfungs- oder LMS-Listen;
...
5. **Lokale Analyse:** JASP ist die verbindliche Analyseumgebung. LIVE-Daten werden ausschließlich in der lokal installierten JASP-Anwendung verarbeitet.
...
| **arsnova.eu/PostgreSQL**         | Session, temporäre Quizkopie, Teilnehmende, Votes, Q&A und optionale Bewertungen/Bonuscodes                                              | Host-PDF und Session-CSV nach `FINISHED`; Q&A nur teilweise im allgemeinen Export             | LIVE/RAW-RESTRICTED                                                         | Regulär werden beendete Sessions 24 Stunden nach `endedAt` für den stündlichen Cleanup fällig. Legal Hold sowie noch aufzubewahrende Bonus- oder Sessionfeedbackdaten können den technischen Purge verzögern. Der Kurs verlässt sich nie auf nachträgliche Verfügbarkeit. |
...
| **arsnova.eu-Sessionexport**      | Frageaggregate, Aggregationsrunde, Häufigkeiten, Schätzstatistik, Confidence, einfache Q&A-Daten; optional Teamwertung und Bonuscodes    | `arsnova-results-TITEL-SESSION.csv` und `.pdf`                                                | zunächst LIVE/RAW-RESTRICTED                                                | Fragewerte sind aggregiert und enthalten keine Personen-Rohwert- oder Paartabelle. Optional enthaltene Nicknames, Teamnamen oder Bonuscodes machen die gesamte Datei pseudonym und werden für diesen Kurs entfernt beziehungsweise gar nicht erzeugt.                     |
...
2. Für arsnova.eu sind anonymer Modus und systemerzeugte Kennungen gewählt. Eigene Nicknames, individuelle Ranglisten und Bonuscodes werden für den Statistikdatenfluss nicht verwendet. Teams werden nur didaktisch eingesetzt; Teamnamen enthalten keine Personen- oder Firmennamen.
...
5. Vor `FINISHED` wird Q&A geschlossen, die Fragenliste aktualisiert und der separate Q&A-CSV exportiert. Geprüft werden mindestens Dateigröße, Kopfzeile, Zahl der Fragen sowie die Spalten für positive Stimmen, negative Stimmen, Gesamtstimmen, Wilson-Score und Kontroversitäts-Score.
