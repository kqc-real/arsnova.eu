...
5. **UX-neutrale Fixes zuerst**, sichtbare Tradeoffs (Image-Proxy, Sync-Rotation, `accessProof`-Cutover, PDF-Queue) bewusst später und mit Migration/Fortschritt.
...
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **HIGH**        | Unsichere externe Bildverarbeitung beim PDF-Export (SSRF, Redirects, Size erst nach Vollladen, Asset-Pfad ohne Root-Containment; abgelehnte `<img src>` bleiben im HTML → Chromium kann sie bei `setContent` erneut laden; Chromium `--no-sandbox`; kein PDF-Parallelitätslimit) | Blind-SSRF, Speicher-/CPU-DoS, lokaler Dateizugriff; Fetch-Guard allein umgehbar | `apps/backend/src/lib/session-results-report-pdf.ts`, `libs/session-export-report/src/markdown-export-images.util.ts` (`inlineExportImagesInHtml`), `apps/backend/src/lib/session-export-asset-reader.ts` | UX-neutral: Stream-Cap, IP-Bind, Redirect-Policy, Path-Containment, **abgelehnte Srcs ersetzen oder Chromium-Netz blockieren**. Ziel: Image-Proxy; Platzhalter nur als fail-closed Interim |
...
| **MEDIUM**      | Legacy-`accessProof` = Content-Hash (kein Besitzbeweis); **Bind-Pfad existiert bereits**, Legacy bleibt danach akzeptiert                                                                                                                                                        | Historie/Bonus/PDF mit Quizinhalt+ID rekonstruierbar                             | `createLegacyQuizHistoryAccessProof`; `bindQuizHistoryScope` / Frontend-Bind; `assertQuizHistoryAccessAuthorized` akzeptiert Legacy auch nach `historyScopeId`                                            | **Offen: Cutover** (Legacy nach Bind/Fenster ablehnen) + Logging/Cutoff/Residuen; Bind nicht neu bauen                                                                                     |
...
- Quiz-Historie: `bindQuizHistoryScope` + Frontend-`bindLegacyQuizHistoryScope` (Claim → UUID-Capability) — **Cutover fehlt** (siehe W3.3)
...
| PDF-SSRF-Kern: TOCTOU-/DNS-Rebind, Stream-Cap, MIME+Magic, kein Header-Forward; **abgelehnte Srcs ersetzen oder Chromium-Netz blockieren** | Sonst umgeht Chromium den Fetch-Guard via `setContent`; Image-Proxy anstreben; Interim-Platzhalter ok |
...
| Legacy-`accessProof` abschalten (Cutover) | Unmigrierte Karten verlieren Historie | Bind-Pfad ist da; Fenster → Legacy-Ablehnung → Hilfetext; Residuen dokumentieren  |
...
| W1.2 | **PDF-SSRF-Kern** (TOCTOU-/DNS-Rebind-resistent): manuelle Redirects mit Hop-Limit; nach jeder Hop erneut auflösen; **alle** A/AAAA prüfen; `connect` an validierte IP binden; blockieren: private/loopback/link-local/multicast/Dokumentationsnetze/Cloud-Metadata; Antwort **streamend** begrenzen; **keine** Cookies/Auth/`Proxy-*`-Header weiterreichen; MIME + Magic-Bytes; Egress zu internen Netzen unterbinden; Asset-Pfad nach `resolve()` unter Asset-Root. **Chromium-Bypass schließen:** abgelehnte/übergröße/redirectete Bilder dürfen **nicht** als Original-`<img src>` in HTML bleiben (`inlineExportImagesInHtml` lässt sie heute stehen → `page.setContent` lädt unabhängig) — **ersetzt** durch Platzhalter/data-URL **oder** Chromium-Netzwerk für den PDF-Lauf **deaktivieren/intercepten**. Image-Proxy (W3.1) anstreben; **temporär** fail-closed Platzhalter erlaubt — **kein** absolutes Dauerabschalten | Unit-/Integrationstests: Blocklisten, Rebind, Size-Cap, Header-Strip, MIME; kein `file://` / RFC1918 / Metadata; **Nachweis, dass Chromium geblockte Ziele nie kontaktiert** |
...
| W3.3 | **`accessProof`-Cutover (kein Neu-Build der Bind-Pipeline):** Ist-Stand auf `main`: `session.bindQuizHistoryScope` akzeptiert Legacy, bindet an UUID-Capability; Frontend `bindLegacyQuizHistoryScope` speichert den neuen Proof. **Schwäche:** `assertQuizHistoryAccessAuthorized` akzeptiert den content-abgeleiteten Legacy-Proof **weiterhin nach Bind**. Offen: Legacy nach erfolgreichem Bind / nach Fenster **ablehnen**; Events loggen; optional zweiter Besitznachweis; **festes Cutoff-Datum**; **Residualrisiko** (Claim im Fenster) dokumentieren — Bind-/Claim-Schritte **nicht** duplizieren | Nach Cutover: Legacy-Proof allein reicht nicht mehr; migrierte Nutzer behalten Historie; Residuen in SECURITY-OVERVIEW |
| W3.4 | **Umgesetzt (Slice B):** signierte Share-Tokens (`?s=`), Redis-Generation + Rotations-Capability-Hash, UI „Sync-Link ungültig machen“, Legacy-UUID-Grace bis `YJS_SHARE_LEGACY_UUID_CUTOFF_AT` (Default 2026-10-01). Keine Kurz-TTL. Details: [W3.4-YJS-SHARE-TOKENS-ABNAHME.md](implementation/W3.4-YJS-SHARE-TOKENS-ABNAHME.md)                                                                                                                                                                                                                                                                          | Workflow bleibt; alte Links nach Rotation tot; Residuen/Grace dokumentiert                                             |
...
- [ ] **Keine offenen HIGH-Befunde** (Node, PDF-SSRF-Kern inkl. Rebind-Bind + Chromium ohne Re-Fetch abgelehnter Srcs, Public Creates)
...
**Woche 1 parallel:** Node 24 (oder 22 + Termin) · PDF-SSRF-Guardrails (Rebind/IP-Bind/Stream/MIME + Chromium ohne Re-Fetch abgelehnter Srcs) · Create-/Größenlimits · `resolveClientIp` · Client-ID Soft-Cap (kein Hard-Lock)
...
**Woche 3–4:** Image-Proxy + Cache-TTL/Size/Deletion · optional PDF-Queue · `accessProof`-**Cutover** (Legacy nach Bind ablehnen; Fenster/Residuen) · Sync-Rotation (Slice) · Astro 0.9 · Backups RPO/RTO/isolierter Restore · Monitoring-Alarme
...
| 2026-07-22 | Codex-PR-Review #124: W1.2 Chromium-Re-Fetch nach abgelehntem Fetch; W3.3 auf verbleibenden Legacy-Cutover (Bind existiert)                                                               |
