...
| **MEDIUM**      | Legacy-`accessProof` = Content-Hash (kein Besitzbeweis); **Bind-Pfad existiert**, Historien-Cutover umgesetzt                                                                                                                                                                    | Vor Cutover: Historie mit Quizinhalt+ID rekonstruierbar                          | `createLegacyQuizHistoryAccessProof`; `bindQuizHistoryScope` / Frontend-Bind; `assertQuizHistoryAccessAuthorized`                                                                                         | **W3.3:** Legacy nach Bind auf Historie abgelehnt; Bind-Upgrade bis Cutoff; Residuen dokumentiert                                                                                          |
...
- Quiz-Historie: `bindQuizHistoryScope` + Frontend-`bindLegacyQuizHistoryScope` (Claim → UUID-Capability) — **Cutover W3.3**
