...
| `QA_NLP_ENABLED`                                       | nein         | `false`                       | Kill-Switch für die optionale Q&A-NLP-Kaskade (Story 8.9b, Gatekeeper auf Seed-Set). Nur exakt `true` aktiviert den Pfad. Getrennt von `NLP_ENABLED` (spaCy 1.14b). Produktiv default aus. Produktdoku: [qa-nlp-moderation.md](features/qa-nlp-moderation.md)                                                                                                           |
...
| `QA_NLP_MIN_CONFIDENCE`                                | nein         | `0.55`                        | Unter dieser Gatekeeper-Konfidenz (0.2–0.95) wird `uncertain` persistiert (Best-Guess-Kategorie bleibt). Betriebsparameter, keine Architekturkonstante.                                                                                                                                                                                                                 |
...
Story 8.9b hat den asynchronen Vertrag, den Kill-Switch und einen hashed-n-Gramm-Naive-Bayes-Gatekeeper auf dem kuratierten Seed-Set (`gatekeeper-hash-nb-v1`). Standard bleibt `QA_NLP_ENABLED=false`. Das Flag ist **nicht** `NLP_ENABLED` (spaCy). Unter `QA_NLP_MIN_CONFIDENCE` (Default `0.55`) wird `uncertain` persistiert. Bei Timeout oder vollem Queue-Limit (`QA_NLP_QUEUE_LIMIT`) wird `failed` persistiert; `qa.submit` wartet die Inferenz nicht ab. Seed-Eval: `npm run eval:qa-nlp -w @arsnova/backend`. Altbestand einer Session nachklassifizieren: `npm run apply:qa-nlp -w @arsnova/backend -- --code ABC123`. Semantischer Fallback und Hörsaal-Lastmessung bleiben offen. Produktdoku: [qa-nlp-moderation.md](features/qa-nlp-moderation.md).
