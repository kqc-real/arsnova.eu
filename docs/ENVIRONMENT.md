...
| `NLP_ENABLED`                                          | nein         | `false`                       | Kill-Switch für die optionale spaCy-Glättung (Story 1.14b). Nur exakt `true` aktiviert den Sidecar-Pfad. Zusätzlich Compose-Profil `nlp` starten; `deploy.sh` startet den Sidecar nicht. Standard-Image nur MIT `de`/`en`                   |
