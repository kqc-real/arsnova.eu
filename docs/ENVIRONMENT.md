...
| `WORD_CLOUD_SEMANTIC_ENABLED`                          | nein         | `false`                               | Kill-Switch für den optionalen Q&A-Themen-Encoder (Story 1.14c Stufe 1). Nur exakt `true` aktiviert den Pfad. Getrennt von `NLP_ENABLED`, `QA_NLP_ENABLED` und `QA_SUMMARY_*`. Zusätzlich Compose-Profil `encoder` starten; `deploy.sh` startet den Sidecar nicht. Produktdoku: [word-cloud-semantic.md](features/word-cloud-semantic.md)                               |
| `WORD_CLOUD_ENCODER_SOCKET_PATH`                       | nein         | `/run/wordcloud-encoder/encoder.sock` | Interner Unix-Socket für App↔Encoder; kein TCP-Port und kein externer API-Endpunkt. Analog `NLP_SOCKET_PATH`. Host-`npm` auf macOS: Docker-Volume unsichtbar — Loopback `WORD_CLOUD_ENCODER_URL`                                                                                                                                                                        |
| `WORD_CLOUD_ENCODER_URL`                               | nein         | leer                                  | Optionaler privater `http`/`https`-Endpunkt statt Socket. Nur Loopback (`localhost`, `127.0.0.0/8`, `::1`) oder RFC1918/`fc00::/7`-Literale. Leer, öffentlicher DNS-Name oder öffentliche IP → kein Cloud-Fallback. Token nur über `WORD_CLOUD_ENCODER_TOKEN`. Lokal macOS: `http://127.0.0.1:8790/embed`                                                               |
| `WORD_CLOUD_ENCODER_TOKEN`                             | nein         | leer                                  | Optionaler Bearer-Token für den privaten Encoder; nicht in der URL hinterlegen                                                                                                                                                                                                                                                                                          |
| `WORD_CLOUD_ENCODER_TIMEOUT_MS`                        | nein         | `8000`                                | Hartes Encoder-Timeout (500–120.000 ms). CPU-e5 bei bis zu 500 Fragen braucht oft 30–120 s; danach `failed` plus 2.x-Phrasen. Live-Hotpath wartet nicht                                                                                                                                                                                                                 |
| `WORD_CLOUD_ENCODER_CACHE_TTL_SECONDS`                 | nein         | `1800`                                | Redis-TTL für SEMANTIC-Snapshots (60–28.800 s), getrennt von `NLP_CACHE_TTL_SECONDS`. Gecacht werden nur `ready`/`uncertain`; `disabled`/`failed`/`pending`/`fallback` nicht. Bei `WORD_CLOUD_SEMANTIC_ENABLED`≠`true` werden Cache-Hits nicht ausgeliefert                                                                                                             |
| `QA_NLP_ENABLED`                                       | nein         | `false`                               | Kill-Switch für die optionale Q&A-NLP-Kaskade (Story 8.9b, Gatekeeper plus k-NN-Fallback). Nur exakt `true` aktiviert den Pfad. Getrennt von `NLP_ENABLED` (spaCy 1.14b). Produktiv default aus; lokaler Hörsaallast 2026-08-19. Produktdoku: [qa-nlp-moderation.md](features/qa-nlp-moderation.md)                                                                     |
...
| `QA_NLP_MIN_CONFIDENCE`                                | nein         | `0.55`                                | Unter dieser Gatekeeper-Konfidenz (0.2–0.95) wird `uncertain` persistiert (Best-Guess-Kategorie bleibt). Betriebsparameter; kalibriert auf Seed-Eval, Default bleibt 0.55.                                                                                                                                                                                              |
| `QA_SUMMARY_ENABLED`                                   | nein         | `false`                               | Kill-Switch für die optionale Moderationszusammenfassung (Story 8.9c). Nur exakt `true` erlaubt die Host-Karte und `qa.requestSummary`, zusammen mit konfiguriertem privatem Endpunkt und mindestens drei sichtbaren Q&A-Beiträgen. Getrennt von `QA_NLP_ENABLED` und `NLP_ENABLED`. Produktiv default aus. Produktdoku: [qa-summary.md](features/qa-summary.md)        |
| `QA_SUMMARY_TIMEOUT_MS`                                | nein         | `8000`                                | Hartes Inferenz-Timeout je Zusammenfassungsjob (500–30.000 ms). Danach `failed`; der Kompass bleibt nutzbar                                                                                                                                                                                                                                                             |
| `QA_SUMMARY_QUEUE_LIMIT`                               | nein         | `8`                                   | Max. wartende plus laufende Summary-Jobs (1–32). Bei Überlast Skip, Status `failed` (`stub:queue-limit`)                                                                                                                                                                                                                                                                |
| `QA_SUMMARY_CONCURRENCY`                               | nein         | `1`                                   | Parallele Summary-Jobs im Backend-Prozess (1–2)                                                                                                                                                                                                                                                                                                                         |
| `QA_SUMMARY_COOLDOWN_MS`                               | nein         | `30000`                               | Kein zweiter Job für denselben Snapshot-Hash innerhalb der Frist (5.000–120.000 ms). `failed` (Timeout) darf sofort wiederholt werden                                                                                                                                                                                                                                   |
| `QA_SUMMARY_TTL_MS`                                    | nein         | `1800000`                             | Ephemeres Summary-Ergebnis pro Session (60.000–28.800.000 ms); keine Prisma-Spalte                                                                                                                                                                                                                                                                                      |
| `QA_SUMMARY_MAX_SOURCES`                               | nein         | `20`                                  | Max. sichtbare Q&A-Quellen im 8.9c-Snapshot nach Ranking und Near-Duplicate-Kanonisierung (3–40). Werte unter der 8.9c-Frageschwelle würden Inferenz sonst still verhindern. Das Modell erhält nur `{ id, kind, text }`                                                                                                                                                 |
| `QA_SUMMARY_INFERENCE_URL`                             | nein         | leer                                  | Privater `http`/`https`-Endpunkt. Leer oder öffentlicher SaaS-Host → kein Cloud-Fallback, `failed`. Token nur über `QA_SUMMARY_INFERENCE_TOKEN`. Lokal: `http://127.0.0.1:8787/summary` mit `npm run qa-summary:dev`                                                                                                                                                    |
| `QA_SUMMARY_INFERENCE_TOKEN`                           | nein         | leer                                  | Optionaler Bearer-Token für den privaten Inferenzserver; nicht in der URL hinterlegen                                                                                                                                                                                                                                                                                   |
...
| `WORD_CLOUD_ENCODER_IMAGE`                             | nein         | `arsnova-wordcloud-encoder:e5-small`  | Image-Referenz des optionalen Encoders (Compose-Profil `encoder`). Nicht Teil von `ARSNOVA_IMAGE` / `SPACY_IMAGE` / `deploy.sh`                                                                                                                                                                                                                                         |
...
| `RATE_LIMIT_SESSION_CODE_WINDOW_SECONDS`               | nein         | `60`                                  | Festes Fenster ausschließlich für nicht existente Session-Codes; nur nach unten konfigurierbar                                                                                                                                                                                                                                                                          |
| `RATE_LIMIT_SESSION_CODE_CLIENT_FAILURES_PER_WINDOW`   | nein         | `5`                                   | Ungültige Codes pro anonymer Browser-Client-ID; danach 429. Die ID wird nur gehasht im Redis-Key verwendet                                                                                                                                                                                                                                                              |
...
Danach in `.env.production` `WORD_CLOUD_SEMANTIC_ENABLED=true` setzen und die App neu starten. Rollback: `WORD_CLOUD_SEMANTIC_ENABLED=false` und `./scripts/prod-compose.sh stop wordcloud-encoder`. Encoder-Modell: [NOTICE](../NOTICE). Host-npm auf macOS: `WORD_CLOUD_ENCODER_URL=http://127.0.0.1:8790/embed` statt Docker-Volume-Socket.

### Optionale Q&A-NLP-Kaskade (Story 8.9b)

Story 8.9b hat den asynchronen Vertrag, den Kill-Switch, einen hashed-n-Gramm-Naive-Bayes-Gatekeeper (`gatekeeper-hash-nb-v1`), die Seed-Kalibrierung, einen In-Process-k-NN-Fallback (`fallback-knn-v1`) im selben Feature-Raum und einen lokalen k6/Artillery-Hörsaallast (2026-08-19). Standard bleibt `QA_NLP_ENABLED=false`. Das Flag ist **nicht** `NLP_ENABLED` (spaCy). Unter `QA_NLP_MIN_CONFIDENCE` (Default `0.55`, nach Kalibrierung beibehalten) wird ohne Early-Exit der Fallback geprüft oder `uncertain` persistiert. Bei Timeout oder vollem Queue-Limit (`QA_NLP_QUEUE_LIMIT`) wird `failed` persistiert; `qa.submit` wartet die Inferenz nicht ab. Host-Telemetrie: `qa.nlpRuntime` mit Fallback-/Early-Exit-/Unclassified-Raten. Seed-Eval inklusive Kurve und Kaskade: `npm run eval:qa-nlp -w @arsnova/backend`. Altbestand einer Session nachklassifizieren: `npm run apply:qa-nlp -w @arsnova/backend -- --code ABC123`. Produktdoku: [qa-nlp-moderation.md](features/qa-nlp-moderation.md).
...
- `RATE_LIMIT_SESSION_CODE_WINDOW_SECONDS=60`
- `RATE_LIMIT_SESSION_CODE_CLIENT_FAILURES_PER_WINDOW=5`
