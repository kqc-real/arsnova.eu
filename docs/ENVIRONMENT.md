...
| `QA_SUMMARY_INFERENCE_URL`                             | nein         | leer                          | Privater `http`/`https`-Endpunkt. Leer oder öffentlicher SaaS-Host → kein Cloud-Fallback, `failed`. Token nur über `QA_SUMMARY_INFERENCE_TOKEN`                                                                                                                                                                                                                         |
...
Slices 1–3 sind im Repo: Shared-Vertrag, Host-Button im Kompass, ephemere Queue, privater HTTP-Adapter. Standard bleibt `QA_SUMMARY_ENABLED=false`. Das Flag ist weder `QA_NLP_ENABLED` noch `NLP_ENABLED`. Ohne private `QA_SUMMARY_INFERENCE_URL` gibt es keinen SaaS-Fallback; `qa.requestSummary` awaitet die Inferenz nicht. Aussagen ohne Snapshot-Quelle werden verworfen. Echtes Modell folgt mit Story 1.14c. Produktdoku: [qa-summary.md](features/qa-summary.md).
