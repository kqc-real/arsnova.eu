...
| `NLP_ENABLED`                                          | nein         | `false`                       | Kill-Switch für die optionale spaCy-Glättung (Story 1.14b). Nur exakt `true` aktiviert den Sidecar-Pfad; ohne Sidecar bleibt `normalizationApplied` immer `NONE`                           |
...
| `NLP_TIMEOUT_MS`                                       | nein         | `5000`                        | Hartes Sidecar-Timeout (1.000–15.000 ms) für den Unix-Socket-Client. Ohne Sidecar (Phase 3) bleibt `normalizationApplied` `NONE`                                                           |
