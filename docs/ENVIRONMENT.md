...
| `WORD_CLOUD_ENCODER_URL`                               | nein         | leer                                  | Optionaler privater `http`/`https`-Endpunkt statt Socket. Leer oder öffentlicher SaaS-Host → kein Cloud-Fallback. Token nur über `WORD_CLOUD_ENCODER_TOKEN`. Lokal macOS: `http://127.0.0.1:8790/embed`                                                                                                                                                                 |
| `WORD_CLOUD_ENCODER_TOKEN`                             | nein         | leer                                  | Optionaler Bearer-Token für den privaten Encoder; nicht in der URL hinterlegen                                                                                                                                                                                                                                                                                          |
| `WORD_CLOUD_ENCODER_TIMEOUT_MS`                        | nein         | `8000`                                | Hartes Encoder-Timeout (500–30.000 ms). Danach `failed` plus 2.x-Phrasen; Live-Hotpath wartet nicht                                                                                                                                                                                                                                                                     |
| `WORD_CLOUD_ENCODER_CACHE_TTL_SECONDS`                 | nein         | `1800`                                | Redis-TTL für SEMANTIC-Snapshots (60–28.800 s). Gecacht werden nur `ready`/`uncertain`; `disabled`/`failed`/`pending`/`fallback` nicht                                                                                                                                                                                                                                  |
...
| `QA_SUMMARY_ENABLED`                                   | nein         | `false`                               | Kill-Switch für die optionale Moderationszusammenfassung (Story 8.9c). Nur exakt `true` aktiviert Host-Button und `qa.requestSummary`. Getrennt von `QA_NLP_ENABLED` und `NLP_ENABLED`. Produktiv default aus. Produktdoku: [qa-summary.md](features/qa-summary.md)                                                                                                     |
...
| `QA_SUMMARY_MAX_SOURCES`                               | nein         | `20`                                  | Max. sichtbare Q&A-Quellen im 8.9c-Snapshot nach Ranking und Near-Duplicate-Kanonisierung (1–40). Das Modell erhält nur `{ id, kind, text }`                                                                                                                                                                                                                            |
...
Stufe 1 ist im Repo; der Sidecar bleibt **betrieblich optional**. `deploy.sh` startet ihn nicht. Standard bleibt `WORD_CLOUD_SEMANTIC_ENABLED=false`. Das Flag ist weder `NLP_ENABLED` noch `QA_NLP_ENABLED` noch `QA_SUMMARY_ENABLED`. Ohne privaten Socket oder private URL gibt es keinen SaaS-Fallback. Produktdoku: [word-cloud-semantic.md](features/word-cloud-semantic.md). Bewusst einschalten nur mit eigenem Image und Compose-Profil `encoder`:
...
Slices 1–3 plus Loopback-Helfer und Snapshot-Ranking sind im Repo: Shared-Vertrag, Host-Button im Kompass, ephemere Queue, privater HTTP-Adapter. Standard bleibt `QA_SUMMARY_ENABLED=false`. Das Flag ist weder `QA_NLP_ENABLED` noch `NLP_ENABLED` noch `WORD_CLOUD_SEMANTIC_ENABLED`. Ohne private `QA_SUMMARY_INFERENCE_URL` gibt es keinen SaaS-Fallback; `qa.requestSummary` awaitet die Inferenz nicht. Der Snapshot rangiert sichtbare Fragen (PINNED/PENDING vor Upvote, Near-Duplicate-Kanonisierung, optionales 8.9b-Tie-Break) und sendet dem Modell nur `{ id, kind, text }`. Aussagen ohne Snapshot-Quelle werden verworfen. Lokal: `npm run qa-summary:dev` auf `http://127.0.0.1:8787/summary` (nicht Produktion; optional `GEMINI_API_KEY` nur im Helfer). Echtes Summary-LLM folgt als 8.9c Slice 4 nach 1.14c Stufe 1 auf derselben privaten Serverrolle, anderem Auftrag. Produktdoku: [qa-summary.md](features/qa-summary.md).
