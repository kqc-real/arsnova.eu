...
| `QA_NLP_ENABLED`                                       | nein         | `false`                       | Kill-Switch für die optionale Q&A-NLP-Kaskade (Story 8.9b, Gatekeeper und Seed-Kalibrierung). Nur exakt `true` aktiviert den Pfad. Getrennt von `NLP_ENABLED` (spaCy 1.14b). Produktiv default aus. Produktdoku: [qa-nlp-moderation.md](features/qa-nlp-moderation.md)                                                                                                  |
...
### Optionale Q&A-NLP-Kaskade (Story 8.9b, Gatekeeper)

Story 8.9b hat den asynchronen Vertrag, den Kill-Switch, einen hashed-n-Gramm-Naive-Bayes-Gatekeeper (`gatekeeper-hash-nb-v1`) und eine Kalibrierung auf erweitertem Seed-Eval. Standard bleibt `QA_NLP_ENABLED=false`. Das Flag ist **nicht** `NLP_ENABLED` (spaCy). Unter `QA_NLP_MIN_CONFIDENCE` (Default `0.55`, nach Kalibrierung beibehalten) wird `uncertain` persistiert. Bei Timeout oder vollem Queue-Limit (`QA_NLP_QUEUE_LIMIT`) wird `failed` persistiert; `qa.submit` wartet die Inferenz nicht ab. Seed-Eval inklusive Kurve: `npm run eval:qa-nlp -w @arsnova/backend`. Altbestand einer Session nachklassifizieren: `npm run apply:qa-nlp -w @arsnova/backend -- --code ABC123`. Semantischer Fallback und Hörsaal-Lastmessung bleiben offen. Produktdoku: [qa-nlp-moderation.md](features/qa-nlp-moderation.md).
