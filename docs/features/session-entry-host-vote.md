...
| Vierte Zeile                                                 | Bei `qaPendingQuestionCount` > 0: »In Moderation: {n}« (Host-Token); sonst »{n} Fragen« / »1 Frage« aus `qaQuestionCount`; bei null/0 keine Zeile                                                                                                                                                                                         | Code+Test | `hostSessionCtaQuestionDescription`        |
...
| Join mit anderem Nickname (Token wird nicht mitgesendet)   | **Keine** neue Teilnahme bei gebundener `anonymousClientId`; Join ohne gültiges Rejoin-Token wird abgelehnt (keine Capability-Rotation). Sonst neue Teilnahme | Nein bei gleichem Browser ohne Token-Diebstahl; Ja nur mit neuem Client/Gerät/geleertem Storage                     | Code+Test |
