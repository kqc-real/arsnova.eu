...
| Eine oder mehrere aktivierte Capabilities                  | Lookup bis 32 Codes; Sortierung offen zuerst (zuletzt gehostete, sonst frühestes Offen-bis), dann geschlossen (frühestes Zugang-bis); Anzeige höchstens 8; gefüllt nur das erste offene Forum | Code+Test | `loadHostSessionCtas`, Spec                |
...
| Vierte Zeile                                               | »{n} Fragen« bzw. »1 Frage«: nicht gelöschte Q&A-Fragen aus `getInfo.qaQuestionCount` (Live-Count, nicht der Seed-Zähler); ohne Zähler keine Zeile                                            | Code+Test | `hostSessionCtaQuestionDescription`        |
...
| ------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------- |
| **Quiz**                                          | Nur `/quiz` (Sammlung). Kein Session-Create.                                                              | Code      | `home.component.html`                             |
| **Neue Q&A-Session**                              | Immer Teilnahme-Dialog, dann `session.create`. Vorhandene Host-Sessions bleiben offen und als CTA stehen. | Code+Test | `openHeroHostTab('qa')`, `home.component.spec.ts` |
| **Q&A** Dialog abbrechen                          | Kein Create                                                                                               | Code+Test | Spec                                              |
| **Blitzlicht** mit Host-Token in Feld oder Recent | Wieder öffnen: `/session/{code}/host`                                                                     | Code+Test | `resolveHeroHostCode`                             |
| **Blitzlicht** ohne Host-Token                    | Sofort `session.create` (`quickFeedbackEnabled: true`), kein Teilnahme-Dialog                             | Code+Test | Spec                                              |
...
| Join mit anderem Nickname (Token wird nicht mitgesendet)   | Neue Teilnahme                            | Ja, zweite Stimme unter neuem Eintrag                                                                               | Code+Test |
...
| Quiz `FINISHED`, Q&A `OPEN`, vor `expiresAt` | Join      | Ja             | Q&A, kein End-Gate | **Nein** (FINISHED) | Ja                  | Q&A + Kanalwahl; weiteres Quiz/Blitzlicht öffnet neu (`canStartAnotherQuiz`) |
