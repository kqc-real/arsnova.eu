...
| Eimer-Icon rechts neben dem CTA                              | Bestätigung: Hauptaktion **Session löschen** (frisches `issueHostAccessToken`, `closeQaChannel`, `session.end`, lokales Vergessen); Ausnahme **Nur Schnellzugang entfernen**; CTA verschwindet sofort und bleibt nach veralteter `getInfo`-Antwort weg                                       | Code+Test | `removeHostSessionCta`, Spec               |
...
| **Q&A-Session** / **Neue Q&A-Session**            | Ohne Host-CTA **Q&A-Session**, mit mindestens einem Host-CTA **Neue Q&A-Session**. Immer Teilnahme-Dialog, dann `session.create`. Vorhandene Host-Sessions bleiben offen und als CTA stehen. | Code+Test | `openHeroHostTab('qa')`, `home.component.spec.ts` |
