...
> **Status:** Spezifikation / Backlog Epic 10 (noch nicht umgesetzt, sofern nicht anders im Backlog markiert)  
...
- **Konflikt mehrerer aktiver MOTDs:** Es wird **höchstens eine** Overlay-MOTD angezeigt; Auswahl nach **`priority` DESC**, dann sekundärer Sortierung (siehe ADR-0018).
...
- Archiv zeigt nur **freigegebene** Einträge; **paginierung** oder „load more“; **lazy load** beim Öffnen des Panels (Performance).
...
- `motd.getCurrent` — Input: `locale`; Output: aktive MOTD oder leer.
- `motd.listArchive` — Input: `locale`, Pagination; Output: nur freigegebene, vergangene/außerhalb Fenster.
- `motd.recordInteraction` (optional) — Input: `motdId`, `version`, `kind` (z. B. `thumb_up` | `thumb_down` | `ack` | `dismiss_close` | `dismiss_swipe`); streng rate-limited.
