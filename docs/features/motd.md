...
- **Kein Overlay-Stapel bei Reload:** Das Auto-Overlay erscheint **höchstens einmal pro Browsersitzung**. Nach Dismiss/Ack einer MOTD öffnet die Startseite **nicht** automatisch die nächstpriore andere Meldung (Reload, Rückkehr zur Startseite, neue Sitzung). Ungelesene Meldungen bleiben über **Badge** und **Archiv** erreichbar. Eine **höhere `contentVersion` derselben MOTD** darf erneut als Overlay erscheinen (inhaltliche Aktualisierung, ADR-0018).
