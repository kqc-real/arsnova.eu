...
Recherche basierend auf der **offiziellen Angular-Dokumentation** (angular.dev) und auf dem **aktuellen Repo-Stand** von `arsnova.eu`. Der beschriebene Ansatz gilt allgemein ab **Angular 17+**; im Repo läuft derzeit **Angular 21.2.x**.
...
| -------------------------------------------- | ------------ | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **Startseite / Code-Einstieg** `/`, `/join`  | –            | Formular (Session-Code) neu                                       | **Niedrig:** Nur eingegebener Code weg; Nutzer tippt neu.                                                              |
...
- **Deutsch (Quellsprache, kein `localize`):** **`npm run dev:de`** (Root) bzw. **`npm run dev:frontend:de`** / in `apps/frontend`: **`npm run start`** oder **`npm run start:de`**. Dann **`http://localhost:4200`** (Root) und Dev-Routen wie **`/session/:code/host`** ohne `/de` nutzen. i18n-Strings kommen direkt aus dem deutschen Quelltext.
...
- **Alle Locales / Produktionsnähe:** lokalisierten Build und Proxy wie unten (**„Lokalisierter Build lokal“**), z. B. `http://localhost:4200/en/`. Auf **macOS** für die spaCy-Wortwolke in allen Locales: `npm run spacy:macos-dev` (`build:prod` + `start:prod` + `serve:localize:api`, **kein** `ng serve`) — [word-cloud-spacy.md](features/word-cloud-spacy.md#lokale-prüfung-auf-macos-host-npm).
- **Stale PWA auf localhost:** Wenn nach einem lokalisierten PWA-Preview unter `localhost:4200/de/...` anschließend wieder `ng serve` läuft, kann Chrome noch eine alte Service-Worker-Registrierung für `/de/` verwenden. Dann einmal **`http://localhost:4200/`** öffnen und danach die Dev-Route ohne `/de` laden. Der Development-Build liefert zusätzlich einen Cleanup-Worker unter `/de/ngsw-worker.js`, damit alte lokale PWA-Registrierungen beim nächsten Dev-Server-Start entfernt werden.
...
**Quiz-Sync-Smoke:** `apps/frontend/scripts/check-quiz-sync-flow.mjs` läuft absichtlich gegen diese
...
| Aspekt                       | Stand                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Extrahierte Messages**     | 2351 trans-units in `messages.xlf`; alle vier Zielkataloge bestehen `npm run check:i18n -w @arsnova/frontend` mit 2351/2351 Einheiten.                                                                                                                                                                                                                                                                                                      |
| **Englisch (en)**            | Vollständig: Alle trans-units in `messages.en.xlf` haben ein `<target>`; Build ohne „No translation found“.                                                                                                                                                                                                                                                                                                                                 |
| **fr/it/es (Technikstatus)** | `messages.fr.xlf`, `messages.it.xlf`, `messages.es.xlf` sind vollständig gepflegt und in `angular.json` eingebunden; der lokalisierte Build erzeugt Ausgaben für alle fünf Sprachen.                                                                                                                                                                                                                                                        |
| **Legal-Seiten**             | Impressum und Datenschutz liegen für `de`, `en`, `fr`, `es` und `it` in `src/assets/legal/`.                                                                                                                                                                                                                                                                                                                                                |
...
_Stand: 2026-07-19. Repo-Abgleich mit Angular 21.2.x, fünf Locale-Dateien mit
je 2351 trans-units und Dokumentation unter
https://angular.dev/guide/i18n._
