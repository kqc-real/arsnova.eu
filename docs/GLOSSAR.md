...
- **Foyer-Einflug:** Animierte Einflug-Phase für Teilnehmenden-Chips im Preset **Spielerisch**; neue Joins fliegen zuerst ein und werden danach zum Teilnehmer-Badge **oben** in der Spalte bzw. Personenliste (neueste zuerst). Gilt in Host- und Presenter-Lobby. Vertiefung: Story 5.4a.
...
- **Moderationszusammenfassung:** Optionale, on-demand 2–4 quellengebundene Sätze über dem Kompass. Kill-Switch `QA_SUMMARY_ENABLED` default aus; Karte nur mit privatem Inferenz-Endpunkt und mindestens drei sichtbaren Q&A-Beiträgen; ohne Endpunkt kein Cloud-Fallback. Echtes Summary-LLM erst mit 8.9c Slice 4 auf der Runtime aus Story 8.9d / ADR-0035; Timeout fällt dann auf eine extraktive Kurzfassung in der App, nicht auf SaaS. Vertiefung: Story 8.9c, [qa-summary](features/qa-summary.md).
- **Open-Weight-LLM-Server:** Geplanter privater `llama.cpp`/`llama-server` (Story 8.9d) für Themenlabels und Kompass-Kurzfassung. Eigenes Image, Profil `llm`, Kill-Switch `OPEN_WEIGHT_LLM_ENABLED`. Nicht spaCy, nicht Encoder, kein öffentlicher Port, kein SaaS. Vertiefung: [ADR-0035](architecture/decisions/0035-self-hosted-llm-runtime-llama-cpp-over-ollama.md).
...
- **Presenter / Beamer:** Projektionsansicht für den Hörsaal ohne Host-Steuerung. Zentrierte Beamer-Bühne (Lobby max. 40rem, Quiz max. 76rem) mit hörsaaltauglicher Typografie; alle Vote-Frageinhalte außer Avatar, Eingaben und Teilnehmer-Chrome werden mitprojiziert, ohne die Fläche auf volle HDMI-Breite zu strecken. Zeigt Lobby (Code/QR), aktuelle Frage, Lesephase, Abstimmung, Ergebnisse, Wortwolke, Q&A und Blitzlicht. Die Lobby füllt den Viewport ohne Scroll: ohne Teams und ohne Motiv ein zentriertes Beitrittsplakat (Code/QR); mit Publikum auf Tablets (Hoch- und Querformat, inkl. kurzer Viewports wie Galaxy Tab S4 1138×712) zwei Spalten Beitritt|Publikum, auf Beamer drei Spalten; Team- und Personenbadges verdichten sich (bei vielen nur Icon/Sequenz), damit alle Teilnehmenden ohne Scroll sichtbar bleiben. Der Host-Button „Presenter-Ansicht“ erscheint auf Desktop und Tablets (iPad, Galaxy Tab), nicht auf schmalen Smartphones. Der neue Tab bekommt auf Desktop eine absolute Presenter-URL; auf Touch-Geräten `_blank` zur Startseite plus kurzlebige same-origin Token-Übergabe in `localStorage` (kein `sessionStorage`-Copy im neuen Tab, sonst bleibt Home stehen), weil Tablets `sessionStorage` nicht mit `window.open` klonen — Home übernimmt das Token und wechselt in die Presenter-Ansicht. Alternativ den Host-Tab duplizieren und `/host` durch `/present` ersetzen. Vorkommen: Route `session/.../present`. Vertiefung: Story 2.5.
...
- **Wortformen glätten / Glättung:** Führt Flexionsformen in Freitext- und Q&A-Wortwolke zusammen, ohne Semantik. Startet beim Anzeigen der Wolke (`Glättung ist an`); der Host kann sie ausschalten. In der Q&A-Wolke in **Wörter** und **Wörter & Phrasen** (dort nur die Einzelwörter; Phrasen bleiben ungeglättet). Neue Antworten/Fragen markieren den Snapshot als veraltet. Vorkommen: Host-Wortwolke. Vertiefung: Story 1.14b, [word-cloud-spacy](features/word-cloud-spacy.md).
...
- **Wortwolke / Word Cloud:** Live-Visualisierung häufiger Begriffe aus Freitext-Antworten und Q&A-Fragen; lexikalisch mit optionaler Sprachformen-Glättung. Q&A **Wörter & Phrasen** zeigt häufige Kurzgruppen, keine einzigartigen Vollfragen. Themenmodus: Host-Toggle **Themen** in Q&A (Story 1.14c Stufe 1, Encoder default aus) und Freitext (Stufe-0-Toggle mit 2.x-Fallback; Encoder-Clustering **Story 1.14d**, offen). Presenter bleibt auf dem lexikalischen Pfad. Vorkommen: Host, Presenter. Vertiefung: ADR-0012, Story 1.14 / 1.14a / 1.14b / 1.14c / 1.14d, [word-cloud-spacy](features/word-cloud-spacy.md), [word-cloud-semantic](features/word-cloud-semantic.md).
...
| **NLP**    | Natural Language Processing              | Getrennte optionale Pfade: Wortwolken-Glättung (1.14b, `NLP_ENABLED`), Q&A-Kategorien (8.9b, `QA_NLP_ENABLED`), semantische Themen (1.14c Stufe 1, `WORD_CLOUD_SEMANTIC_ENABLED`), geplanter Open-Weight-LLM-Server (8.9d, `OPEN_WEIGHT_LLM_ENABLED`) |
