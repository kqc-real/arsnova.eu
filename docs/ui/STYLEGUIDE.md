...
- **Button-Hierarchie:** Global bleibt **»Los geht’s«** der primaere Filled-CTA. **»Letztes Quiz starten«** sitzt auf der Live-Karte (volle Breite über den drei Kanalaktionen), nur wenn mindestens ein **eigenes** Quiz existiert (Demo zaehlt nicht). Die Vorbereiten-Karte hat keinen Filled-CTA: **»Neues Quiz erstellen«** bleibt tonal, **»Quiz-Sammlung öffnen«** outlined. Einsatzmöglichkeiten stehen als tertiaerer Textlink darunter. **Geteilte Sammlung nutzen:** Icon-Button `devices` **rechts außen** im Kartenkopf (`@@homeHostCard.syncPanelCta`). Das Panel nimmt nur einen **empfangenen** Sync-Link entgegen; den Link erzeugen bleibt in der Quiz-Sammlung (**»Sammlung teilen«**). Offen: Schließen per Icon, **Backdrop-Klick** oder **Escape**; Fokus zurück auf das Icon. Die offene Vorbereiten-Karte muss **über** dem Backdrop liegen (`z-index` höher als `.home-sync-backdrop`), auch gegen die spielerische Side-Act-Regel `z-index: 1`; im schmalen Desktop-Kartenslot stehen Eingabe und Submit untereinander.
