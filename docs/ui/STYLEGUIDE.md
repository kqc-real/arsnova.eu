...
- **`.l-page`:** Max-width 56 rem, zentriert, Padding 1,5 rem / 1 rem (responsive 1,5 rem / 2 rem). Fluchtlinie und Innenabstand wie auf der Startseite **unter 960px**.
...
- **Mitmachen-Karte (Spielerisch, ab 960px):** Statt flacher Graufläche: **mehrschichtiger** Kartenhintergrund (Primary-/Tertiary-Radials + diagonalen Verlauf), **ohne** linken Akzentstreifen – derselbe umlaufende Rand wie Blitzlicht und Veranstalten. **Kein Spotlight-Panel** um Session-Code oder „Letzte Sessions“. Code-Segmente **dieselbe Geometrie wie Serioes**: quadratisch, max. 20,5 rem, zentriert. **Ein Kartentitel** „Mitmachen“ (`@@homeJoinCard.title`); keine Eyebrow-Labels und keine zweite Zeile „Dabei sein“. Alle drei Karten: **Icon und h2 in einer Zeile** (`.home-card__heading`), Titel auf **derselben Hoehe**, **gleiche Innenabstaende** (Karten-Padding, Header/`content`/`actions` `padding-inline` 1 rem). Die Host-Karten-Header-Zeile hat **kein Flex-Gap**, damit die leere Material-`header-text`-Huelle das Icon nicht nach rechts schiebt. Ab **960px** haben **Mitmachen**, **Blitzlicht** und **Vorbereiten** dieselbe Kartenhoehe (`min-height` 25 rem). Die Codereihe sitzt **vertikal mittig** zwischen Kopf und CTA; **Los geht's**, der ABCD-Chip und **Einsatzmöglichkeiten** fluchten ab 960px (gleiche Hoehe, gleicher Abstand zum Kartenboden); auf **Quiz vorbereiten** sitzen die beiden CTAs **vertikal mittig** zwischen Kopf und Einsatzmoeglichkeiten-Link. Blitzlicht-Chips sind **zweispaltig** (Icons über dem Label, mehr Innenabstand, Inhalt bleibt in der Pill); unter den Chips bleibt Abstand zum Kartenboden.
- **Haupt-CTA:** Der gefuellte Button auf der **Mitmachen**-Karte hat im Spielerisch-Modus **keinen** Glow- oder Elevation-Glanz; deaktiviert ist die Flaeche matt und deckend.
...
- **Button-Hierarchie:** Auf der Quiz-vorbereiten-Karte ist **genau ein** gefuellter CTA: **„Letztes Quiz starten“**, wenn mindestens ein **eigenes** Quiz existiert (Demo zaehlt nicht); sonst **„Neues Quiz erstellen“**. Existiert ein letztes Quiz, bleibt **„Neues Quiz erstellen“** sichtbar, aber tonal (kein zweiter CTA). Darunter tonal **„Quiz-Sammlung öffnen“**. Hilfe als Text-Button ohne Umrandung. **Geteilte Sammlung nutzen:** Icon-Button `devices` **rechts außen** im Kartenkopf (`@@homeHostCard.syncPanelCta`). Das Panel nimmt nur einen **empfangenen** Sync-Link entgegen; den Link erzeugen bleibt in der Quiz-Sammlung (**„Sammlung teilen“**). Nicht als Text-CTA unter den Buttons. Offen: Schließen per Icon, **Backdrop-Klick** oder **Escape**; Fokus zurück auf das Icon. Die offene Vorbereiten-Karte muss **über** dem Backdrop liegen (`z-index` höher als `.home-sync-backdrop`), auch gegen die spielerische Side-Act-Regel `z-index: 1`.
...
- **Zustaende:** Leer (**2 px** `--mat-sys-outline`, beide Presets), Active (primary Border + Pulse-Animation), Filled (primary Border + Tint-Background), Valid (success-fg Border + Glow).
...
- **Fehleingabe:** Der Mitmachen-CTA bleibt **volle Kartenbreite** (Actions immer Spalte), **nowrap**, **padding-block 0,625 rem**, damit „Los geht's“ neben der Fehlermeldung nicht umbricht und das Button-Padding verliert. Icon und Text im Fehlerhinweis (`.home-error`) stehen **mittig** in der Alert-Zeile.
- **Host-CTAs:** Auf der Quiz-vorbereiten-Karte dürfen CTA-Labels umbrechen (`white-space: normal`), damit längere Übersetzungen in der Dreier-Spalte nicht aus der Karte laufen. Die Host-Buttons stehen in einer Spalte mit **1 rem Abstand** (auch unter 960 px).
...
- **Trust-Badges:** Staerkstes Argument zuerst in `.home-hero-usp--secondary`: „Kostenlos. Open Source. Ohne Anmeldung.“ (`@@homeHero.uspTrust`). Keine zusaetzliche Buehnen-Zeile nur fuer „ohne Anmeldung“.
- **Kein Onboarding-Banner:** Nach den Hero-Erweiterungen (USPs, Preset-Toggle) wird kein 3-Schritt-Banner, keine dekorative Chip-Sequenz („Code eingeben → Live dabei“) und keine Copy-Rotation (`.home-stage-rotator`) mehr angezeigt. Die Karten (Mitmachen/Veranstalten) und die Hilfe-Seite erklaeren den Ablauf.
