...
- **Spielerisch Light, Tonal-Flächen:** »Neues Quiz«, »Neue Q&A-Session« und die vier Blitzlicht-Vorlagen erhalten dieselbe aus `on-surface` (12 %) und `surface-container-high` gemischte neutrale Füllung. So trennen sie sich von den hellen Karten; Outlined-Aktionen bleiben getrennt und Dark/Seriös unverändert.
...
- **Kartenkopf-Icons:** Informative Glyphen in outlined Tiles (Quiz, Q&A und Blitzlicht als gefüllte Glyphen) (`meeting_room` Teilnehmen, `quiz` Quiz, `chat_bubble` Q&A, `bolt` Blitzlicht). Kein gefuellter Primary-Kreis; Filled bleibt Buttons, Badges und aktiven Zustaenden vorbehalten.
