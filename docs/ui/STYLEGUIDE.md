...
- **Startseite:** Bleibt Sonderfall mit eigenem Hero und nutzt `.l-page` ohne `.l-section`. Bis 1199 px stehen die Inhalte in **einer zentrierten Lesespalte mit maximal 40 rem**: Hero, Teilnahme, Rollenwechsel und danach die Aufgaben **Live**, **Direktes Feedback** und **Vorbereiten**. Ab 1200 px folgen Hero, Host-Einstieg und Host-Raster der **56-rem-Achse der Toolbar**; die drei Host-Karten stehen gleich breit und gleich hoch in einer Zeile. Die zentrierte Teilnahme-Karte behaelt mit maximal 36 rem ihre kompakte Eingabebreite.
...
- **Kontext:** Nach einer Auswahl unter »Mit einem Klick« auf der Startseite landet die veranstaltende Person auf der **Blitzlicht-Host-Ansicht** (`FeedbackHostComponent`, Route `feedback/:code`, **nicht** eingebettet in Session-Host).
...
- **Buehnen-Layout:** Hero + USP liegen in `.home-hero-band` mit dezentem Verlauf/Radial-Glow (nur bei `prefers-reduced-motion: no-preference`); der Glow-Layer darf **sehr langsam** driften (`home-hero-band-spotlight-drift`). **Keine Copy-Rotation** unter dem Hero: Der Trust-Satz sitzt in `.home-hero-usp--secondary`. Der Preset-Umschalter bleibt auf Mobile im globalen Hamburger-Menü, damit der Hero kompakt bleibt. Bis 1199 px folgt die einspaltige Aufgabenreihenfolge Teilnahme → Rollenwechsel → Live → Direktes Feedback → Vorbereiten; ab 1200 px stehen die drei Host-Aufgaben wie im serioesen Preset als gleich hohes Dreispaltenraster.
- **Karten:** Teilnahme (`.home-card--stage-main`) hat die staerkere Elevation (`level3` + `--app-shadow-card-playful`). Die drei Host-Aufgabenkarten (`.home-card--stage-side`) nutzen dieselbe bestehende Buehnen-Sprache mit Primary-/Tertiary-Radials, innerer Kante und weichem Farbschatten, aber zurueckhaltender. Direktes Feedback bleibt staerker Primary-, Vorbereiten staerker Tertiary-betont. Hover hebt Karten nur unter `prefers-reduced-motion: no-preference` leicht an.
...
- **Teilnahme-Karte:** Mehrschichtiger Primary-/Tertiary-Hintergrund, **ohne** linken Akzentstreifen. Rollen-Eyebrow »Für Teilnehmende«, darunter genau ein `h2` »An einer Session teilnehmen«. **Kein Spotlight-Panel** um Session-Code oder »Letzte Sessions«. Die sechs Code-Segmente behalten in beiden Presets dieselbe Geometrie.
...
- **Aufgabenwahl:** Unter »Was möchtest du tun?« stehen drei getrennte Karten: **Session starten** (Quiz, Q&A, Blitzlicht), **Mit einem Klick** (Standalone-Feedbackformate) und **Quiz erstellen**. Bis 1199 px werden sie untereinander angeordnet; ab 1200 px bilden sie drei gleich breite und gleich hohe Spalten.
...
- **Zweizeilige Aktionsbuttons:** Hauptlabel mit `label-large`, Kurzbeschreibung mit mindestens `body-small`; beide verwenden unveraendert die vom Material-Button geerbte On-Container-Farbe. Keine reduzierte Opacity oder gemischte Textfarbe. Mindesthoehe 3,75 rem; auf Mobile volle Breite. Die Host-CTAs duerfen eine dritte `body-small`-Zeile (Q&A-Offen-Frist) und eine vierte (Fragenzahl) tragen. Live: zuerst eine eigene Reihe (`.home-host-session-cta-row`, eine Spalte, 600–1199 px zwei Spalten, Desktop wieder eine Spalte) fuer jede in diesem Browser noch zugangliche Host-Session; offen zuerst (zuletzt gehostete, sonst frühestes Offen-bis), dann geschlossen (frühestes Zugang-bis), Anzeige höchstens 8; gefüllt nur das erste offene Forum, der Rest outlined wie **»Quiz-Sammlung öffnen«**; darunter die drei tonalen Kanalaktionen (Quiz, **Q&A-Session** ohne Host-CTA bzw. **Neue Q&A-Session** sobald mindestens ein Host-CTA steht, Blitzlicht) in einer Spalte, von 600 bis 1199 px drei Spalten, im aeusseren Desktop-Dreispaltenraster wieder eine Spalte. Vorbereiten: eine Spalte, von 480 bis 1199 px zwei Spalten fuer Erstellen/Sammlung, im aeusseren Desktop-Raster wieder eine Spalte. Die Host-Karten tragen keine `.home-card__description`; der erklaerende Introtext unter »Was möchtest du tun?« wechselt im Desktop-Raster von `body-small` auf `body-medium`.
- **Direktes Feedback:** Die drei Host-Karten haben keine zusaetzliche Kartenbeschreibung mehr; Eyebrow, Titel und Aktionen reichen. Die kompakte Auswahl auf der Startseite enthaelt genau **Tempo**, **Stimmungsbild**, **Ja · Nein · Vielleicht** und **Sterne**. Weitere Blitzlichtformate bleiben in den dafuer vorgesehenen Detailansichten verfuegbar. Im aeusseren Desktop-Dreispaltenraster wird die dritte sichtbare Beschriftung platzsparend als **»Ja · Nein · ?«** dargestellt; Mobile/Tablet und der zugaengliche Buttonname behalten **»Ja · Nein · Vielleicht«**. Dort sitzt die Vierergruppe vertikal zentriert im freien Kartenraum; `0,75 rem` Rasterabstand (Desktop `1 rem`) und mindestens `4,25 rem` Kachelhoehe sorgen fuer Luft. Session-starten- und Vorbereiten-Karte zentrieren ihre Aktionen ebenso, auch wenn das Sync-Panel die dritte Spalte streckt.
- **Button-Hierarchie:** Global bleibt **»Los geht’s«** der primaere Filled-CTA. Auf der Quiz-vorbereiten-Karte ist **»Letztes Quiz starten«** nur dann ein zusaetzlicher, kontextbezogener Filled-CTA, wenn mindestens ein **eigenes** Quiz existiert (Demo zaehlt nicht). **»Neues Quiz erstellen«** bleibt unabhaengig davon immer tonal; ohne eigenes Quiz gibt es in dieser Karte keinen Filled-CTA. **»Quiz-Sammlung öffnen«** ist outlined. Einsatzmöglichkeiten stehen als tertiaerer Textlink darunter. **Geteilte Sammlung nutzen:** Icon-Button `devices` **rechts außen** im Kartenkopf (`@@homeHostCard.syncPanelCta`). Das Panel nimmt nur einen **empfangenen** Sync-Link entgegen; den Link erzeugen bleibt in der Quiz-Sammlung (**»Sammlung teilen«**). Offen: Schließen per Icon, **Backdrop-Klick** oder **Escape**; Fokus zurück auf das Icon. Die offene Vorbereiten-Karte muss **über** dem Backdrop liegen (`z-index` höher als `.home-sync-backdrop`), auch gegen die spielerische Side-Act-Regel `z-index: 1`; im schmalen Desktop-Kartenslot stehen Eingabe und Submit untereinander.
...
- **Abstaende:** Nebeneinander liegende Aktionsraster der Host-Karten nutzen 0,75 rem. Gestapelte Buttons (eine Spalte, inkl. Desktop-Dreispaltenraster) und die CTA-Saeule der Vorbereiten-Karte nutzen 1 rem Zeilenabstand; die Desktop-Vierergruppe fuer direktes Feedback ebenfalls 1 rem. Card-Header, Inhalt und Actions behalten 1 rem Inline-Padding. Im breiten Desktop-Raster erhalten alle Home-Karten 1,25 rem Abstand zum unteren Kartenrand.
...
- **Zustaende:** Leer (**2 px** `--mat-sys-outline`, beide Presets), Active (Primary-Border + Pulse-Animation), Filled (Primary-Border + Tint-Background).
- **Micro-Interactions:** Segment-Pulse auf aktiver Box (1 s), CTA-Pulse beim 6. Zeichen (Scale 1 → 1,04 → 1, 350 ms), Shake bei ungueltigem Submit (400 ms, Error-Border). Alle Animationen in `@media (prefers-reduced-motion: no-preference)`.
- **Beschriftung:** Keine sichtbare Wiederholung **»Session-Code«** unter den Segmenten; **»Code eingeben«** benennt die Aufgabe bereits. Für Screenreader bleibt das `aria-label` am Eingabefeld (»Session-Code, 6 Zeichen«).
...
- **Dedizierter Einstieg:** Der codefreie Pfad `/join` verwendet dieselbe Startseiten-Komponente und darf die Code-Eingabe auf Geraeten ohne groben Primaerzeiger fokussieren. Bei primaerer Touch-Eingabe bleibt der Fokus unveraendert, damit ein direkter Link nicht ungefragt die Bildschirmtastatur oeffnet; ein bewusster Klick auf »Code eingeben« fokussiert weiterhin.

## Startseite: Hero und Aufgabenfuehrung

- **Hero-Text:** Kompaktes Nutzenversprechen mit den drei Kanaelen Quiz, Q&A und Blitzlicht. Der Hero und der Live-Button verwenden getrennte i18n-IDs (`@@homeHero.channelBlitzlicht` / `@@homeLiveCard.quickFeedbackLabel`), damit Englisch im Marketingtext **live polls**, am benannten Feature aber **Pulse Check** anzeigen kann.
- **Host-Zugang wiederherstellen:** Die Live-Karte lädt bis zu 32 gespeicherte Host-Capabilities und zeigt höchstens 8: offene Foren zuerst (zuletzt gehostete, sonst frühestes Offen-bis), danach geschlossene (frühestes Zugang-bis). Gefüllt ist nur das erste offene Forum; der Rest ist outlined wie **»Quiz-Sammlung öffnen«** (inkl. spielerisch-hellem Kontrast). Ohne offenes Forum bleibt die ganze Reihe outlined. Jeder Eintrag öffnet die Host-Session direkt im Q&A-Kanal (`?tab=qa`) und heißt **Q&A-Session** plus der sechsstellige Sessioncode, nicht »offen« und nicht »Host-Session«. Die zweite Zeile zeigt die Host-Zugangsfrist (`postProcessingEndsAt`, sonst `expiresAt`) als »Zugang bis …« mit lokalem Datum und Uhrzeit; ohne geladene Frist bleibt `@@homeLiveCard.recoveryDescription`. Die dritte Zeile zeigt die Q&A-Offen-Frist (`qaClosesAt`, sonst `expiresAt`) als »Offen bis …«; nach Ablauf oder geschlossenem Kanal steht dort »Forum geschlossen«. Die vierte Zeile zeigt die teilnehmendensichtbaren Q&A-Fragen (`ACTIVE`/`PINNED`/`ARCHIVED`) als »1 Frage« oder »{n} Fragen«. Rechts neben jedem CTA sitzt ein `delete_outline`-Icon (44 px, `@@homeLiveCard.removeCtaAria`). Die Host-CTAs sind abgerundete Rechtecke (`--mat-sys-corner-medium`, linksbündige Mehrzeilen), keine Pills. Die Abfrage bietet **Session löschen** als Hauptaktion (`session.end` für alle Kanäle) und **Nur Schnellzugang entfernen** als Ausnahme; sie nennt Offen-Status, Fragenzahl, Zugangsfrist und den Wiederherstellungshinweis. Nur ein Wiederherstellungskandidat ohne aktivierte Capability zeigt keinen CTA. Die Wiederherstellungsseite bleibt unter `/<locale>/host-recovery` erreichbar, hat auf der Live-Karte aber keinen Textlink. Die tonalen Kanalaktionen stehen von 600 bis 1199 px in drei Spalten, darunter oder darüber die CTA-Reihe (600–1199 px zwei Spalten, sonst eine).
- **Alleinstellungsmerkmale (USP):** Zielgruppe/Gruppengroesse in Primary-Farbe; darunter als Trust-Zeile »Kostenlos. Open Source. Ohne Anmeldung.« (`@@homeHero.uspTrust`) in on-surface-variant.
...
- **Kein Onboarding-Banner:** Kein 3-Schritt-Banner, keine dekorative Chip-Sequenz und keine Copy-Rotation (`.home-stage-rotator`). Rollenlabels, Aufgabenüberschrift und Karten erklaeren den Ablauf.
...
- **Session-Code (Startseite):** Sichtbar benennt »Code eingeben« die Aufgabe; »Session-Code, 6 Zeichen« bleibt das `aria-label` des Eingabe-Overlays.
