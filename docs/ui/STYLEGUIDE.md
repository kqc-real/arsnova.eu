...
- **Outlined-Kontrast:** Auf der spielerischen Light-Verlaufsfläche verwendet **»Quiz-Sammlung öffnen«** für Label und Rand `on-surface` bzw. `on-surface-variant`; die normalen Outlined-Tokens wären dort zu kontrastarm. Spielerisch Dark und beide seriösen Themes behalten die Material-Tokens.
...
- **Host-Zugang wiederherstellen:** Der gefüllte Primary-CTA der Live-Karte (`@@homeLiveCard.recoveryLabel` / `@@homeLiveCard.recoveryDescription`) erscheint nur bei genau einer bevorzugten aktivierten Host-Capability (zuletzt gehostete Session, sonst die einzige gespeicherte). Er öffnet diese Host-Session direkt im Q&A-Kanal (`?tab=qa`) und heißt **Q&A-Session** plus der sechsstellige Sessioncode, nicht »offen« und nicht »Host-Session«. Die zweite Zeile zeigt die Host-Zugangsfrist (`postProcessingEndsAt`, sonst `expiresAt`) als »Zugang bis …« mit lokalem Datum und Uhrzeit; ohne geladene Frist bleibt `@@homeLiveCard.recoveryDescription`. Mehrere Capabilities ohne Last-Hosted-Marker oder nur ein Wiederherstellungskandidat zeigen keinen CTA. Die Wiederherstellungsseite bleibt unter `/<locale>/host-recovery` erreichbar, hat auf der Live-Karte aber keinen Textlink. Ohne CTA stehen Quiz, Q&A und Blitzlicht von 600 bis 1199 px wieder nebeneinander in drei Spalten. Mit CTA stehen die vier Live-Aktionen im Tablet-Portrait (600–839 px) im 2×2-Raster; unter 600 px untereinander, ab 1200 px wieder einspaltig in der schmalen Host-Karte.
