...
- **Spielerisch Light, Tonal-Flächen:** »Neues Quiz«, »Neue Q&A-Session« und die vier Blitzlicht-Vorlagen erhalten dieselbe aus `on-surface` (12 %) und `surface-container-high` gemischte neutrale Füllung. Das gilt bei explizitem Light und bei System mit hellem Betriebssystem. So trennen sie sich von den hellen Karten; Outlined-Aktionen bleiben getrennt und Dark/Seriös unverändert.
...
- **Desktop-Hero-Abstand:** Ab 1200 px verwenden Seriös und Spielerisch 3 rem oberen Abstand und identisches Hero-Padding (0,75 / 1 / 0,9 rem); so bleibt der Titel unter der App-Bar und die Position beim Presetwechsel stabil.
