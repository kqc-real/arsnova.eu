...
- **Host-Zugang wiederherstellen:** Der Einstieg sitzt nur dann als erster, gefüllter Primary-CTA der Live-Karte (`@@homeLiveCard.recoveryLabel` / `@@homeLiveCard.recoveryDescription`), wenn dieser Browser eine Host-Capability oder einen Wiederherstellungskandidaten gespeichert hat. Eine aktivierte Capability öffnet die Host-Session direkt; nur ein Kandidat ohne Capability führt auf `/host-recovery`. Ohne gespeicherte Capability bleibt der CTA unsichtbar, und Quiz, Q&A und Blitzlicht stehen von 600 bis 1199 px wieder nebeneinander in drei Spalten. Mit CTA stehen die vier Live-Aktionen im Tablet-Portrait (600–839 px) im 2×2-Raster; unter 600 px untereinander, ab 1200 px wieder einspaltig in der schmalen Host-Karte.
