...
- **Host-Zugang wiederherstellen:** Der Textlink (`@@hostRecovery.title`) bleibt auf der Live-Karte nach den Start-Aktionen immer erreichbar (mindestens 44 px Zielhöhe, hinter dem Live-Raster in der Tastaturreihenfolge). Der gefüllte Primary-CTA der Live-Karte (`@@homeLiveCard.recoveryLabel` / `@@homeLiveCard.recoveryDescription`) erscheint nur bei genau einer bevorzugten aktivierten Host-Capability (zuletzt gehostete Session, sonst die einzige gespeicherte). Er öffnet diese Host-Session direkt und heißt **Host-Session**, nicht »offen«. Mehrere Capabilities ohne Last-Hosted-Marker oder nur ein Wiederherstellungskandidat zeigen keinen CTA; der Einstieg bleibt der Wiederherstellungslink. Ohne CTA stehen Quiz, Q&A und Blitzlicht von 600 bis 1199 px wieder nebeneinander in drei Spalten. Mit CTA stehen die vier Live-Aktionen im Tablet-Portrait (600–839 px) im 2×2-Raster; unter 600 px untereinander, ab 1200 px wieder einspaltig in der schmalen Host-Karte.
