...
- **1.9b:** Prompt-Template (wartbares Artefakt), Einbindung der aktuellen Preset-/Zielgruppen-/Schwierigkeitswerte, Platzierung in der UI **nach** Durchklicken der Vorgaben; Feintuning des Prompt-Textes für Schema-Treue. Kanonische Prompt-Spezifikation: **ADR-0007** (Promptarchitektur für KI-generierte Quizzes).
...
| **Eingabe** | Thema/Dokument in Mentimeter einfügen | PDF/URL/Thema in Kahoot hochgeben | Thema / Empfehlungen in Slido | **Prompt kopieren** → Nutzer lädt Präsentation/Skript per **Kontext-Upload (RAG)** im Chatbot hoch; optional Preset, Zielgruppe, Schwierigkeit |
...
4. **Steuerung per Prompt:** Preset, Zielgruppe (Nickname-Auswahl), Schwierigkeitsgrad können im Prompt abgefragt werden – das generierte JSON ist damit bereits passend konfiguriert (analog zu „Tuning“ in der Mitbewerber-App).
...
**Zusätzliche Steuerung im Prompt:** Der Prompt kann den Dozenten optional fragen bzw. das LLM anweisen, das generierte Quiz anhand von **Preset** (Seriös/Spielerisch), **Zielgruppe** (z. B. Nickname-Auswahl: Kita, Grundschule, Oberstufe) und **Schwierigkeitsgrad** (EASY/MEDIUM/HARD) auszurichten – analog zu „Tuning in der App“ bei Mitbewerbern, nur über die Prompts und das generierte JSON.

---

## 4. Einordnung in die Produktstrategie

- **Epic 1 (Quiz-Verwaltung):** Story 1.9a (Import + Zod) und 1.9b (Prompt) erweitern 1.9 um den KI-Workflow. **1.9b** bündelt das Feintuning des Prompts und die Platzierung in der UI (nach Preset/Optionen); **1.9a** bleibt auf Import und Validierung fokussiert.
- **Positionierung:** arsnova.eu bietet „KI-gestützten Quiz-Import“ mit **externem LLM**, **kontextbasiertem Prompt** (Preset, Zielgruppe, Schwierigkeit) und **strikter Zod-Validierung** – mit Fokus auf Datensparsamkeit und Schema-Treue.
...
- **Implementierung:** Zuerst 1.9b (Prompt-Template, Platzierung, Kontext-Einbindung, Feintuning für Schema), dann oder parallel 1.9a (Import-Dialog, Zod, Fehlerbehandlung, Yjs).
