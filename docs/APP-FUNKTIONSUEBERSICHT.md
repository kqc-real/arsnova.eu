...
- Die Host-Leiste **Präsentation starten** öffnet zuerst den Dialog **Präsentation starten**: Primäraktion **Präsentation im Vollbild starten**; optional **Mit Smartphone steuern** / **Smartphone verbinden**; die Verbindung wird immer auf dem Host-Bildschirm bestätigt; nach Kopplung startet die Presenter-Ansicht ohne zweiten Laptop-Klick. Co-Host über **Weiteres Gerät verbinden**. Freigabe als Ja/Nein (**Ja, Gerät verbinden** / **Nein, ablehnen**) mit Zeichenabgleich. Der Original-Host verwaltet Geräte kompakt (Status, **Verbindung trennen**); Widerruf beendet Host-Rechte sofort. Das getrennte Gerät kann die Veranstaltung verlassen, ohne sie für alle zu beenden. Pairing liegt nur in diesem Dialog, nicht in der Host-Leiste. QR, Freigabe, Geräteverwaltung und Secrets bleiben auf `/host`, nie auf `/present`. Das gekoppelte Gerät steuert die laufende Veranstaltung (Fragen, Ergebnisse, Q&A, Blitzlicht, Session beenden), nicht die Quiz-Sammlung — und gibt der Lehrkraft volle Bewegungsfreiheit im Raum.
...
- in der Host-Ansicht als Moderations- und Steuerbereich inklusive Sortiermodi `Meist unterstuetzt` / `Beste Fragen` / `Umstritten`, Bewertungsmetriken (`Zustimmung` bzw. `Geteilte Reaktionen`) und sortierabhaengiger Q&A-Wortwolke
...
- **Als gelesen markieren** pro ungelesener Archiv-Meldung (Zähler sinkt um 1); gelesene Einträge bleiben als Status **Gelesen** sichtbar

### 9.2 Interaktionen

Für MOTDs existieren getrennte Interaktionsarten:

- Kenntnisnahme (Overlay) bzw. **Als gelesen markieren** (Archiv, einzeln) / **Alles als gelesen markieren**
- Daumen hoch
- Daumen runter
