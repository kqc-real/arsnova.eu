...
- Story 2.10 Slices 1–5: `isHostSessionTokenValid` accepts the original host token or a hashed `PAIRED_HOST` token. Pairing admin (`createHostPairingInvite`, approve/reject/revoke/list) requires `originalHostProcedure`. Revoke deletes the token lookup immediately and ends existing host subscriptions via `waitWhileHostTokenValid`; a revoked token cannot regain rights on reconnect. Session code, QR, or invite secret alone never grants host rights; no silent grant. DoD: `load:smoke:host-pairing-security`, `smoke:host-pairing-security`, classroom-30/500 cap load.
