...
- Stufe 2 LLM labels, Presenter topics, and 8.9c Slice 4 remain out of 1.14c. Runtime is ADR-0035 (`llama-server`); Stufe 2 may only replace extractive labels — clustering stays Stufe 1. `OPEN_WEIGHT_LLM_ENABLED` off must not disable `WORD_CLOUD_SEMANTIC_ENABLED`. Host-Freitext encoder clustering is Story 1.14d (open): same kill-switch and sidecar, lift `channel: FREETEXT` hard fallback; no second public service.
