...
- Max one inflight job per session; new data → stale + “Neu analysieren”. Circuit breaker 3 failures / 30 s. Live hotpath (`qa.submit`, Join, Vote, WS) never awaits inference.
- `SEMANTIC + LEMMA` is `MODE_UNSUPPORTED`. Smoothing stays off in Themenmodus (Q&A like Freitext).
- CI fixtures: geometric unit vectors, no model download. Three Kapitel-4 exam paraphrases cluster; Folien vs Beamer do not.
- Stufe 2 LLM labels, Presenter topics, Freitext encoder clustering, and 8.9c Slice 4 are out of scope.
