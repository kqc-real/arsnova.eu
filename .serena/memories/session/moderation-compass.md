...
- 8.9a is done: host-only rule-based compass cards from already loaded signals. No auto pin/archive/phase actions. Cards appear only with evidence; the button stays visible. Present/Vote/FINISHED have no compass. Trusted tutors or moderators may use it only as full Paired Hosts under story 2.10. First-user Host-UI: next-step banner first, cards Tempo→Reibung→Klärung→Themen, tappable source rows (3 visible), optional 8.9c summary collapsed with the button at the top, prominent return-to-compass, one-time sessionStorage first-open hint.
