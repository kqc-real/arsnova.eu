...
- Story 1.14b is done (analysis version `1.14b.13`). Canonical doc: `docs/features/word-cloud-spacy.md`. Sentence-initial capitalized finite verbs (`Zählt`, `Läuft der`, `Macht das`) stay VERB via STTS verb tags or a known finite-surface list, not a generic PROPN/X neighbor heuristic (that would drop `Angular Framework`). Bare numbers (`10`) and punctuation (`..`) are not unigrams; participles (`Gelernt`) stay VERB. THEME/Q&A **Wörter & Phrasen** never uses unique full questions as cloud labels; only short shared anchors (2–3 tokens) plus lemma unigrams when smoothing is on.
...
- Q&A: separate MatDialog. Smoothing on `LEXICAL` and `THEME` (does not force `LEXICAL`). THEME + smoothing keeps THEME phrases and replaces unigrams with lemma nouns; `THEME + LEMMA` stays `MODE_UNSUPPORTED`. Sort TOP/BEST/CONTROVERSIAL with active lemma re-smooths. THEME sort change stays `normalization: NONE` for phrases and re-runs lemma for unigrams. Cloud size `Häufigkeit` (`TIME`) analyzes all eligible questions with equal weight 1; the question list stays `Zeit` (`createdAt`). Votes do not drive word size.
...
- Host-npm `npm run dev` starts the sidecar (`scripts/spacy-dev-sidecar.sh`, socket `/tmp/arsnova-nlp.sock`) and sets `NLP_ENABLED=true` only in the backend process. First run creates `docker/spacy/.venv` and downloads de/en/fr/es. Python 3.10+ required. `npm run dev:backend` stays API-only without sidecar.
- macOS locale helper: `npm run spacy:macos-dev` (`scripts/macos-spacy-wordcloud-dev.sh`). Not `ng serve`. After Postgres/Redis it runs `prisma:migrate` (otherwise `public.Quiz` is missing). Localized dist via `serve:localize:api` on `http://localhost:4200/de/` … `/it/` plus `start:prod` on 3000. Smoothing de/en/fr/es; Italian UI needs an explicit cloud language. Docker volume socket is invisible to host Node; do not expect `docker:up:nlp`. Canonical: `docs/features/word-cloud-spacy.md` sections “Lokaler Dev-Server” and “Lokale Prüfung auf macOS”.
