/**
 * Tests für HomeComponent + PresetToastComponent.
 * Sichert die Logik nach dem Refactoring ab.
 */
...
// ---------------------------------------------------------------------------
// PresetToastComponent Tests (extrahierte Preset-Logik)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// HomeComponent Tests (Orchestrator nach Refactoring)
// ---------------------------------------------------------------------------
...
describe('HomeComponent', () => {
  ...
  describe('setLanguage', () => {
    it('setzt Sprache und speichert in localStorage', () => {
      const comp = createComponent();
      comp.setLanguage('fr');
      expect(comp.language()).toBe('fr');
      expect(localStorage.getItem('home-language')).toBe('fr');
    });
  });

  describe('Controls-Menü', () => {
    it('toggleControlsMenu wechselt den Zustand', () => {
      const comp = createComponent();
      expect(comp.controlsMenuOpen()).toBe(false);
      comp.toggleControlsMenu();
      expect(comp.controlsMenuOpen()).toBe(true);
      comp.toggleControlsMenu();
      expect(comp.controlsMenuOpen()).toBe(false);
    });

    it('closeControlsMenu schließt das Menü', () => {
      const comp = createComponent();
      comp.controlsMenuOpen.set(true);
      comp.closeControlsMenu();
      expect(comp.controlsMenuOpen()).toBe(false);
    });

    it('onEscapePressed schließt das Menü wenn offen', () => {
      const comp = createComponent();
      comp.controlsMenuOpen.set(true);
      comp.onEscapePressed();
      expect(comp.controlsMenuOpen()).toBe(false);
    });

    it('onEscapePressed tut nichts wenn Menü geschlossen', () => {
      const comp = createComponent();
      comp.controlsMenuOpen.set(false);
      comp.onEscapePressed();
      expect(comp.controlsMenuOpen()).toBe(false);
    });
  });
  ...
  describe('Preset-Toast Integration', () => {
    ...
    it('presetToastVisible ist initial false', () => {
      const comp = createComponent();
      expect(comp.presetToastVisible()).toBe(false);
    });
  });
});
