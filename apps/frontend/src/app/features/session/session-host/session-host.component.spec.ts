...
function qaHostSnapshot(
  ...
  const rankingRevision = options.rankingRevision ?? 1;
  const pendingCount = questions.filter((question) => question.status === 'PENDING').length;
  return {
    questions,
    state: 'ACTIVE',
    sessionLifecycleRevision: options.sessionLifecycleRevision ?? 1,
    ...
    rankingRevision: `${rankingRevision}:TOP:`,
    nextCursor: null,
    totalCount: questions.length,
    pendingCount,
    sessionPendingCount: options.sessionPendingCount ?? pendingCount,
  };
}
...
describe('SessionHostComponent', { timeout: 60_000 }, () => {
  ...
  it('lädt nach Auswahl einer Teilnahme im Verzeichnis deren Q&A-Fragen', async () => {
    ...
    expect(identityButtons[1]?.textContent ?? '').toContain('Team');
    expect(identityButtons[1]?.textContent ?? '').toContain('🍎');
    ...
    expect(fixture.nativeElement.textContent ?? '').toContain('Roter Drache 2');
    expect(
  ...
  it('zeigt die Teamzugehörigkeit an Host-Fragen auch ohne authorTeamName aus dem Verzeichnis', async () => {
    const dragonQuestion = {
      id: 'question-dragon',
      text: 'Kannst du das Beispiel noch einmal erklären?',
      upvoteCount: 5,
      ...
      participants: [
        {
          ...
          teamId: 'team-apple',
          teamName: 'Team 🍎',
          joinedAt: '2026-09-15T08:01:00.000Z',
        },
      ],
      nextCursor: null,
      participantCount: 1,
    });
    qaListQueryMock.mockResolvedValue([dragonQuestion]);
    ...
    await fixture.whenStable();
    fixture.componentInstance.activeChannel.set('qa');
    fixture.componentInstance.qaQuestions.set([dragonQuestion]);
    fixture.detectChanges();

    expect(fixture.componentInstance.qaQuestionAuthorTeamName(dragonQuestion)).toBe('Team 🍎');
    expect(
      fixture.nativeElement.querySelector('.session-qa-card__author-team')?.textContent,
    ).toContain('Team');
    fixture.destroy();
  ...
  it('zeigt den Kanalwahlschalter nach Quiz-FINISHED, solange Q&A offen ist', () => {
    const fixture = setup();
    fixture.componentInstance.session.set({
      ...defaultSession,
      status: 'FINISHED',
  ...
  it('öffnet Blitzlicht nach Quiz-FINISHED für die anwesenden Teilnahmen', async () => {
    const finishedOpenQa = {
      ...defaultSession,
      status: 'FINISHED' as const,
      qaClosesAt: '2026-09-20T06:07:00.000Z',
      channels: {
    ...
    const fixture = setup();
    fixture.componentInstance.session.set(finishedOpenQa);
    fixture.componentInstance.activeChannel.set('qa');
    fixture.detectChanges();
    reopenQuickFeedbackChannelMutateMock.mockResolvedValue({
      quiz: { enabled: true },
    ...
    getInfoQueryMock.mockResolvedValue({
      ...
      status: 'LOBBY',
      channels: {
        ...finishedOpenQa.channels,
        quickFeedback: { enabled: true, open: true },
      },
    });

    expect(fixture.componentInstance.effectiveStatus()).toBe('FINISHED');
    ...
    expect(fixture.componentInstance.activeChannel()).toBe('quickFeedback');
    expect(fixture.componentInstance.effectiveStatus()).toBe('LOBBY');
    expect(fixture.componentInstance.canStartAnotherQuiz()).toBe(false);
    fixture.destroy();
  });
  ...
  it('zeigt Q&A nach Session-Ende nur lesbar und blockiert direkte Änderungsaufrufe', async () => {
    ...
    getInfoQueryMock.mockResolvedValue(finishedClosedQa);
    const fixture = setup();
    await fixture.componentInstance.ngOnInit();
    fixture.componentInstance.session.set(finishedClosedQa);
  ...
  it('lässt Host-Moderation nach Quiz-FINISHED zu, solange Q&A offen ist', async () => {
    const fixture = setup();
    fixture.componentInstance.session.set({
      ...defaultSession,
      status: 'FINISHED',
    ...
    const host = fixture.nativeElement as HTMLElement;
    expect(fixture.componentInstance.qaHostWritesAllowed()).toBe(true);
    expect(host.textContent).not.toContain('ausschließlich lesen und exportieren');
    expect(host.querySelector('.session-qa-moderation-toggle')).not.toBeNull();
    expect(
      fixture.componentInstance.canModerateQaQuestion(
        fixture.componentInstance.qaQuestions()[0]!,
        'APPROVE',
      ),
    ).toBe(true);
  ...
  it('zeigt die Servermeldung, wenn das Anhängen des Quiz scheitert', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      quizName: null,
      preferredChannel: 'qa',
  ...
  it('beendet eine reine Q&A-Session nicht beim Verlassen der Host-Ansicht', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      qaEnabled: true,
  ...
  it('startet eine reine Q&A-Lobby beim Verlassen, ohne die Session zu beenden', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'LOBBY',
      qaEnabled: true,
  ...
  it('beendet ein offenes Q&A nicht, wenn Quiz oder Blitzlicht aktiv sind', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
  ...
  it('zeigt fuer Freitext-Fragen die Word-Cloud-Aktion mit Live-Hinweis', async () => {
    ...
    expect(text).toContain('Häufige Wörter und kurze Wortgruppen.');
  ...
  it('oeffnet die Freitext-Wortwolke ohne Vollbild mit Analyse-Toggle und Freeze-Steuerung', async () => {
    ...
    try {
      ...
      expect(
        wordCloudAnalyzeQueryMock.mock.calls.every(
          (call) => (call[0] as { normalization?: string }).normalization === 'LEMMA',
        ),
      ).toBe(true);
  ...
  it('startet Freitext-Glaettung beim Anzeigen als LEXICAL+LEMMA', async () => {
    getInfoQueryMock.mockResolvedValue({ ...defaultSession, status: 'ACTIVE' });
    getCurrentQuestionForHostQueryMock.mockResolvedValue({
      questionId: '11111111-1111-4111-8111-111111111111',
      order: 5,
    ...
    expect(activeSmoothButton?.textContent).toContain('Glättung ist an');
    expect(component.freetextWordCloudMode()).toBe('PHRASES');
  ...
  it('zeigt einen Fehlerhinweis, wenn die Freitext-Glaettung fehlschlaegt', async () => {
    getCurrentQuestionForHostQueryMock.mockResolvedValue({
      questionId: '11111111-1111-4111-8111-111111111111',
      order: 5,
      text: 'Warum bleibt ein Satellit im Orbit?',
  ...
  it('verwendet im Fragen-Tab keinen sichtbaren Seitenausschnitt als Wortwolkenkorpus', async () => {
    ...
    const text = fixture.nativeElement.textContent ?? '';
    expect(text).toContain('Wortwolke anzeigen');
    expect(text).toContain('2 sichtbare Fragen');
  ...
  it('verlangt bei geändertem Pending-Stand eine neue Bestätigung', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      channels: {
        quiz: { enabled: true },
    ...
    const firstQuestion: QaQuestionDTO = {
      id: '44444444-4444-4444-8444-444444444444',
      text: 'Erste wartende Frage',
      upvoteCount: 0,
      status: 'PENDING',
      createdAt: '2026-03-13T12:00:00.000Z',
      myVote: null,
    ...
    const secondQuestion: QaQuestionDTO = {
      ...firstQuestion,
      id: '55555555-5555-4555-8555-555555555555',
      text: 'Neu hinzugekommene Frage',
    };
    ...
    snapshot = qaHostSnapshot([firstQuestion, secondQuestion], {
      rankingRevision: 2,
      sessionLifecycleRevision: 2,
    });
    ...
    expect(repeatedConfig.data).toMatchObject({
      title: '2 Fragen freigeben?',
      message: '2 Fragen in Moderation werden für Teilnehmende sichtbar.',
      confirmLabel: '2 Fragen freigeben',
    });
  ...
  it('oeffnet die Q&A-Wortwolke ohne Vollbild mit Sortierzustand des Hosts', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
  ...
  it('zeigt ohne gewählte Analysesprache keinen lokalen Q&A-Seitenausschnitt', async () => {
    ...
    await vi.waitUntil(
      () =>
        wordCloudAnalyzeQueryMock.mock.calls.some(
          (call) => (call[0] as { mode?: string }).mode === 'THEME',
        ) && lemmaAnalyzeCalls().length >= 1,
      { timeout: 5000, interval: 25 },
    );
    expect(wordCloudAnalyzeQueryMock).toHaveBeenCalledWith(
  ...
  it('zeigt Theme-Entries statt lokaler Terme, sobald die Analyse belastbar ist', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
    ...
    expect(data.themeFallbackHint()).toBeNull();
    expect(data.terms()).toBeNull();
    expect(data.analysisEntries()?.[0]?.label).toBe('gewinnt');
    fixture.destroy();
  });
  ...
  it('markiert Q&A-Themen bei neuen Fragen als veraltet und analysiert erst nach Neu analysieren', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
    ...
    await fixture.componentInstance.openQaWordCloudDialog();
    fixture.componentInstance.setQaWordCloudAnalysisVariant('SEMANTIC');
    expect(fixture.componentInstance.qaWordCloudThemeAnalysisPending()).toBe(true);
    expect(fixture.componentInstance.qaWordCloudSemanticWaitHint()).toBeNull();
    await vi.waitUntil(
      () => fixture.componentInstance.qaWordCloudThemeAnalysisResult()?.status === 'ready',
      { timeout: 5000, interval: 25 },
    );
    const analyzeCallsAfterOpen = wordCloudAnalyzeQueryMock.mock.calls.length;
    ...
    expect(wordCloudAnalyzeQueryMock.mock.calls.length).toBe(analyzeCallsAfterOpen);
    ...
    await fixture.componentInstance.toggleQaWordCloudSmoothing();
    await vi.waitUntil(() => wordCloudAnalyzeQueryMock.mock.calls.length > analyzeCallsAfterOpen, {
      timeout: 5000,
      interval: 25,
    });
  ...
  function lemmaAnalyzeCalls(): unknown[] {
    return wordCloudAnalyzeQueryMock.mock.calls.filter(
      (call) => (call[0] as { normalization?: string }).normalization === 'LEMMA',
    );
  }

  it('startet Sprachformen-Glaettung beim Oeffnen als LEXICAL+LEMMA', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
    ...
    await vi.waitUntil(
      () =>
        wordCloudAnalyzeQueryMock.mock.calls.some(
          (call) =>
            (call[0] as { mode?: string; normalization?: string }).mode === 'THEME' &&
            (call[0] as { normalization?: string }).normalization === 'NONE',
        ) && lemmaAnalyzeCalls().length >= 1,
      { timeout: 5000, interval: 25 },
    );
    await vi.waitUntil(() => fixture.componentInstance.qaWordCloudSmoothingStatus() === 'active', {
    ...
    expect(fixture.componentInstance.qaWordCloudSmoothingLabel()).toBe('Glättung ist an');
    expect(fixture.componentInstance.qaWordCloudVisibleTerms()).toBeNull();
    expect(fixture.componentInstance.qaWordCloudAnalysisEntries()).toMatchObject([
      { label: 'Kapitel 4' },
      { label: 'Klausur' },
  ...
  it('glaettet Q&A-Woerter & Phrasen nur bei den Einzelwoertern und laesst Phrasen stehen', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
    ...
    expect(fixture.componentInstance.qaWordCloudSmoothingLabel()).toBe('Glättung ist an');
    expect(
      fixture.componentInstance.qaWordCloudAnalysisEntries()?.map((entry) => entry.label),
    ).toEqual(['Klausur']);
    fixture.destroy();
  });
  ...
  it('behaelt die Q&A-Glaettung auf Woerter & Phrasen, solange Themen vorbereitet werden', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
    ...
    let releaseSemantic: ((value: unknown) => void) | undefined;
    const semanticGate = new Promise((resolve) => {
    ...
    await vi.waitUntil(
      () =>
        fixture.componentInstance.qaWordCloudThemeAnalysisResult()?.entries[0]?.label ===
        'Kapitel 4',
      { timeout: 5000, interval: 25 },
    );
    expect(
    ...
    fixture.componentInstance.setQaWordCloudAnalysisVariant('SEMANTIC');
    fixture.detectChanges();
    await vi.waitUntil(() => fixture.componentInstance.qaWordCloudThemeAnalysisPending(), {
      timeout: 5000,
      interval: 25,
    });
    ...
    expect(fixture.componentInstance.qaWordCloudAnalysisEntries()?.[0]?.label).toBe(
  ...
  it('faellt nach Sortierungswechsel auf die lexikalische Wolke zurueck, wenn die Glaettung fehlschlaegt', async () => {
    ...
    expect(fixture.componentInstance.qaWordCloudSmoothingStatus()).toBe('idle');
    expect(fixture.componentInstance.qaWordCloudLemmaSnapshotVisible()).toBe(false);
    expect(fixture.componentInstance.qaWordCloudAnalysisEntries() ?? []).toEqual([]);
  ...
  it('laesst die lexikalische Wolke waehrend der Glaettung sichtbar', async () => {
    ...
    let releaseLemma: ((value: unknown) => void) | undefined;
    const lemmaGate = new Promise((resolve) => {
      releaseLemma = resolve;
    });
    wordCloudAnalyzeQueryMock.mockImplementation(
      async (input: { mode?: string; normalization?: string }) => {
        if (input.normalization === 'LEMMA') {
    ...
    await vi.waitUntil(() => fixture.componentInstance.qaWordCloudSmoothingStatus() === 'pending', {
      timeout: 5000,
      interval: 25,
    });
    await vi.waitUntil(
      () => (fixture.componentInstance.qaWordCloudThemeAnalysisResult()?.entries.length ?? 0) > 0,
      { timeout: 5000, interval: 25 },
    );
    expect(fixture.componentInstance.qaWordCloudLemmaSnapshotVisible()).toBe(false);
    expect(fixture.componentInstance.qaWordCloudAnalysisEntries()).toMatchObject([
  ...
  it('markiert geglaettete Q&A-Snapshots bei neuen Fragen als veraltet ohne automatisch neu zu rechnen', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
    ...
    fixture.componentInstance.qaQuestions.update((questions) => [
      ...
      {
        id: '33333333-3333-4333-8333-333333333333',
        text: 'Gibt es Klausuraufgaben zu Kapitel 5?',
        upvoteCount: 2,
        status: 'ACTIVE',
        createdAt: '2026-03-13T12:05:00.000Z',
        myVote: null,
    ...
    expect(fixture.componentInstance.qaWordCloudSmoothingLabel()).toBe('Neu analysieren');
    expect(fixture.componentInstance.qaWordCloudSmoothingHint()).toBe(
      'Neue Fragen seit letzter Glättung',
    );
    expect(fixture.componentInstance.qaWordCloudAnalysisEntries()).toEqual(
      expect.arrayContaining([expect.objectContaining({ label: 'Klausur' })]),
    );
  ...
  it('laesst den Themenmodus bei NONE, wenn die Glaettung fehlschlaegt oder abgeschaltet ist', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
    ...
    await fixture.componentInstance.openQaWordCloudDialog();
    await vi.waitUntil(
      () => fixture.componentInstance.qaWordCloudSmoothingHint() === 'Glättung nicht verfügbar',
      { timeout: 5000, interval: 25 },
    );
    await vi.waitUntil(
      () => (fixture.componentInstance.qaWordCloudThemeAnalysisResult()?.entries.length ?? 0) > 0,
    ...
    const themeCalls = wordCloudAnalyzeQueryMock.mock.calls.filter(
      (call) => (call[0] as { mode?: string }).mode === 'THEME',
    );
    expect(themeCalls.length).toBeGreaterThan(0);
    expect(
      themeCalls.every((call) => (call[0] as { normalization?: string }).normalization === 'NONE'),
    ).toBe(true);
    fixture.destroy();
  });

  it('zeigt einen Fehlerhinweis, wenn die Glaettung per Timeout fehlschlaegt', async () => {
    getInfoQueryMock.mockResolvedValue({
      ...defaultSession,
      status: 'ACTIVE',
      channels: {
  ...
  it('blendet Quiz-Navigationsbuttons in Q&A und Blitzlicht aus', async () => {
    ...
    expect(fixture.componentInstance.activeChannel()).toBe('qa');
    expect(buttonTexts).toContain('Zur Startseite');
  ...
  it('zeigt in der Join-Kapsel weder Sessionende noch Löschtermin anzeigen', async () => {
    ...
    getInfoQueryMock.mockResolvedValue({
      ...
      channels: {
        ...
        qa: {
          ...
          moderationMode: false,
          state: 'OPEN' as const,
          closesAt: defaultLifecycle.qaClosesAt,
        },
        quickFeedback: { enabled: false, open: false },
      },
    });
    ...
    const quizBanner = fixture.nativeElement.querySelector(
      '.session-host__live-code-block',
    ) as HTMLElement | null;
    ...
    const qaBanner = fixture.nativeElement.querySelector(
      '.session-host__live-code-block',
    ) as HTMLElement | null;
  ...
  describe('Host-Steering-Callout bei Störfällen', () => {
    ...
    it('öffnet den Q&A-Callout nicht bei stillen Hintergrund-Aktualisierungen', async () => {
      getInfoQueryMock.mockResolvedValue({
        ...defaultSession,
        channels: {
          quiz: { enabled: true },
      ...
      qaListQueryMock.mockResolvedValueOnce([]);
      ...
      await (
        fixture.componentInstance as unknown as {
          refreshQaQuestions(options?: { silent?: boolean }): Promise<void>;
        }
      ).refreshQaQuestions({ silent: true });
      fixture.detectChanges();
    ...
    it('exportiert alle Q&A-Fragen als CSV mit den in der Host-UI sichtbaren Attributen', async () => {
      ...
      expect(exportedCsv).toContain(
        'Nr.;Frage-ID;Status;Statusbezeichnung;Autor;Team;Frage;Score;Positive Stimmen;Negative Stimmen;Stimmen gesamt;Wilson-Score;Kontroverse-Score;Umstritten;Hervorgehoben;Erstellt am',
      );
      expect(exportedCsv).toContain(
  ...
  describe('Moderationskompass', () => {
    it('zeigt den Button Moderation neben der Live-Leiste', async () => {
      ...
      expect(button?.getAttribute('aria-label')).toBe('Moderationskompass öffnen');
      fixture.destroy();
    });

    it('bleibt in Quiz, Q&A und Blitzlicht sichtbar', async () => {
      getInfoQueryMock.mockResolvedValue({
        ...defaultSession,
        status: 'ACTIVE',
        channels: {
      ...
      for (const channel of ['quiz', 'qa', 'quickFeedback'] as const) {
    ...
    it('öffnet den großen Dialog mit Moderationskarten', async () => {
      getInfoQueryMock.mockResolvedValue({
        ...defaultSession,
        channels: {
          quiz: { enabled: true },
    ...
    it('zeigt Reibung auch ohne CONTROVERSIAL-Sortierung', async () => {
      const fixture = setup();
      fixture.detectChanges();
      await fixture.whenStable();
      await flushComponentAfterStable(fixture, 50);
      ...
      const friction = component.moderationCompassCards().find((card) => card.kind === 'friction');
      expect(friction?.sources[0]?.label).toContain('5 dafür, 5 dagegen');
      expect(friction?.sources[0]?.label).toContain('Ist die Klausur open book?');
      fixture.destroy();
    });

    it('nimmt Quiz-Fakten erst nach der Ergebnisfreigabe auf', async () => {
      const fixture = setup();
      fixture.detectChanges();
      await fixture.whenStable();
      await flushComponentAfterStable(fixture, 50);
      ...
      expect(
        component
          ...
          .some((card) => card.sources.some((source) => source.kind === 'quiz-result')),
      ).toBe(false);
      ...
      const clarification = component
        .moderationCompassCards()
        .find((card) => card.kind === 'clarification');
      expect(clarification?.sources.map((source) => source.label)).toEqual(
        expect.arrayContaining([
          '22 von 30 liegen daneben · Was ist 2+2?',
          'Häufigste andere Antwort: 5 · Was ist 2+2?',
    ...
    it('behält Quiz-Fakten der letzten Ergebnisfreigabe', async () => {
      const fixture = setup();
      fixture.detectChanges();
      await fixture.whenStable();
      await flushComponentAfterStable(fixture, 50);
      ...
      component.session.update((session) =>
        session ? { ...session, status: 'ACTIVE', currentQuestion: 1 } : session,
      );
      fixture.detectChanges();
      ...
      expect(
        component
          .moderationCompassCards()
          .some((card) =>
            card.sources.some((source) => source.label.includes('22 von 30 liegen daneben')),
          ),
      ).toBe(true);
      fixture.destroy();
    });

    it('springt von einer Q&A-Quelle zur Frage', async () => {
      getInfoQueryMock.mockResolvedValue({
        ...defaultSession,
        status: 'ACTIVE',
        channels: {
      ...
      await component.followModerationCompassSource(source!);
      fixture.detectChanges();
      await fixture.whenStable();
      fixture.detectChanges();
      ...
      expect(component.moderationCompassReturn()?.channel).toBe('quiz');

      dialogOpenMock.mockClear();
      await component.returnToModerationCompass();
      fixture.detectChanges();

      expect(component.activeChannel()).toBe('quiz');
      expect(component.moderationCompassReturn()).toBeNull();
      expect(component.isQaCompassFocused('11111111-1111-4111-8111-111111111111')).toBe(false);
      expect(
        fixture.nativeElement.querySelector('[data-testid="host-moderation-compass-return"]'),
      ).toBeNull();
    ...
    it('öffnet die Q&A-Wortwolke für einen Themenbegriff', async () => {
      getInfoQueryMock.mockResolvedValue({
        ...defaultSession,
        status: 'ACTIVE',
        channels: {
      ...
      expect(component.activeChannel()).toBe('qa');
      expect(component.qaSortMode()).toBe('TOP');
      expect(component.qaWordCloudAnalysisVariant()).toBe('LEXICAL');
      expect(component.moderationCompassFocusedTerm()).toBe('Übung');
      expect(component.isQaCompassFocused('11111111-1111-4111-8111-111111111111')).toBe(true);
      expect(component.isQaCompassFocused('22222222-2222-4222-8222-222222222222')).toBe(true);
      expect(component.isQaCompassFocusPrimary('11111111-1111-4111-8111-111111111111')).toBe(true);
      expect(component.isQaCompassFocusPrimary('22222222-2222-4222-8222-222222222222')).toBe(false);
      const [, config] = dialogOpenMock.mock.calls.at(-1) as [unknown, Record<string, unknown>];
      expect(config['panelClass']).toBe('word-cloud-dialog-panel');
      fixture.destroy();
    });
    ...
    it('zeigt negatives Blitzlicht neben Tempo', async () => {
      ...
      component.quickFeedbackResult.set({
        ...
        locked: false,
        totalVotes: 10,
        distribution: { NEGATIVE: 8, POSITIVE: 2 },
      });
      fixture.detectChanges();
