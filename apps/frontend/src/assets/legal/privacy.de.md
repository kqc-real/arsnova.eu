# Datenschutzerklärung

Informationen gemäß Art. 13 und 14 DSGVO

## 1. Verantwortlicher

Prof. Dr.-Ing. habil. Klaus Quibeldey-Cirkel  
Juri-Gagarin-Ring 152  
99084 Erfurt  
Deutschland

E-Mail: [klaus.quibeldey-cirkel@iu.org](mailto:klaus.quibeldey-cirkel@iu.org)

## 2. Geltungsbereich

Für https://arsnova.eu (Informations- und Marketingseite) und – soweit im Angebot darauf verwiesen wird – für die Nutzung der Anwendung arsnova.eu (Live-Quiz & Abstimmung) gelten die folgenden Hinweise zum Datenschutz. Bei Self-Hosting der Anwendung kann der Verantwortliche ein anderer sein (Ihre Institution); dann gelten die Datenschutzhinweise, die dort bereitgestellt werden.

## 3. Daten beim Besuch von arsnova.eu

Wenn Sie arsnova.eu aufrufen, verarbeitet der betreibende Server bzw. der Hosting-Anbieter in der Regel automatisch Zugriffsdaten (z. B. IP-Adresse, Datum/Uhrzeit, aufgerufene Seite, übertragene Datenmenge, Browsertyp) in Server-Logs. Rechtsgrundlage ist Art. 6 Abs. 1 lit. f DSGVO (berechtigtes Interesse am Betrieb und an der Sicherheit). Die Speicherdauer richtet sich nach den Vorgaben des Hosting-Anbieters; typischerweise werden Logs nach kurzer Frist gelöscht oder anonymisiert.

Wir setzen **keine Cookies** (auch keine Drittanbieter-Cookies) und führen keine Analyse-Tools (z. B. Google Analytics) aus. Für die Darstellung nutzen wir **System-Schriftarten**; **Material Icons** liegen auf unserem Server (**kein** Google Fonts, kein Verbindungsaufbau zu Google). Sofern nicht ausdrücklich anders gekennzeichnet, binden wir keine eingebetteten Inhalte Dritter (z. B. Karten, Videos) ein. Links zu Drittanbietern sind in Abschnitt 4 beschrieben.

**Progressive Web App (PWA):** Wenn Sie arsnova.eu installieren oder die Offline-Funktion nutzen, kann Ihr Browser ausgewählte Inhalte lokal zwischenspeichern. Das verkürzt Ladezeiten und kann Inhalte ohne Netzverbindung anzeigen. **Es werden keine Cookies** für Werbe- oder Tracking-Zwecke gesetzt.

## 4. Verlinkung zu Drittanbietern

Auf arsnova.eu finden Sie Links zu externen Diensten (z. B. GitHub). Beim Klick gelangen Sie auf die Seiten der jeweiligen Anbieter. Dort gelten deren Datenschutzerklärungen und Nutzungsbedingungen. Wir haben keinen Einfluss auf deren Datenverarbeitung.

## 5. Hosting

arsnova.eu wird von der Hetzner Online GmbH in Deutschland gehostet. Hetzner lässt sich jährlich durch den TÜV Rheinland prüfen, um die Einhaltung der Sicherheitsstandards nach Art. 32 DSGVO zu gewährleisten.

Die Website und die Anwendung unter https://arsnova.eu werden bei Hetzner betrieben. Wird die Software **selbst oder durch Dritte** (z. B. Ihre Institution) betrieben, können Zugriffsdaten (z. B. IP-Adresse, Zugriffszeit) auf Servern des jeweiligen Anbieters anfallen. Wird in Drittländern (z. B. USA) gehostet, kann eine Übermittlung dorthin erfolgen; dann werden geeignete Garantien (z. B. Standardvertragsklauseln nach Art. 46 DSGVO) vereinbart. Eine **separate** Projekt-Landingpage (Demo/Marketing) kann zusätzlich über GitHub Pages unter einer anderen Domain bereitgestellt werden – nicht die Hauptseite arsnova.eu; Informationen: [GitHub Privacy Statement](https://docs.github.com/en/site-policy/privacy-policies/github-privacy-statement). Der konkrete Anbieter und Speicherort ergeben sich aus dem jeweiligen Angebot.

## 6. Ihre Rechte

Sie haben gegenüber dem Verantwortlichen u. a. folgende Rechte:

- **Auskunft** (Art. 15 DSGVO) über Ihre verarbeiteten personenbezogenen Daten
- **Berichtigung** (Art. 16 DSGVO) unrichtiger Daten
- **Löschung** (Art. 17 DSGVO) oder **Einschränkung der Verarbeitung** (Art. 18 DSGVO)
- **Widerspruch** (Art. 21 DSGVO) gegen die Verarbeitung, soweit sie auf Art. 6 Abs. 1 lit. f DSGVO gestützt wird
- **Datenübertragbarkeit** (Art. 20 DSGVO), soweit anwendbar
- **Widerruf** einer erteilten Einwilligung (Art. 7 Abs. 3 DSGVO) mit Wirkung für die Zukunft
- **Beschwerde** bei einer Aufsichtsbehörde (Art. 77 DSGVO)

Für Auskünfte und die Ausübung Ihrer Rechte wenden Sie sich bitte an: [klaus.quibeldey-cirkel@iu.org](mailto:klaus.quibeldey-cirkel@iu.org).

## 7. Bereitstellungspflicht

Für einen reinen Informationsbesuch ohne Anmeldung sind Sie weder gesetzlich noch vertraglich verpflichtet, personenbezogene Daten anzugeben. Sie können arsnova.eu ohne solche Angaben nutzen; wir können Ihnen die Inhalte anzeigen und technisch ausliefern. Für eine Kontaktaufnahme per E-Mail benötigen wir eine E-Mail-Adresse von Ihnen.

## 8. Hinweis zur Anwendung arsnova.eu

Wenn Sie arsnova.eu für Live-Quiz, Q&A oder Abstimmungen nutzen, ist die Nutzung datenschutzfreundlich gestaltet:

- **Local-First:** Quiz-Inhalte (Fragen, Antworten) speichern Sie als Ersteller im Browser lokal (IndexedDB/Yjs). Eine dauerhafte serverseitige Speicherung von Quiz-Inhalten erfolgt nicht; beim Start einer Live-Session werden die Daten nur temporär für die Dauer der Session an den Server übertragen.
- **Speicherung im Browser:** Local Storage und IndexedDB können u. a. Quiz-Daten, Einstellungen, zuletzt genutzte Session-Codes und – bei Feedback-Abstimmungen – eine lokale Kennung zur einmaligen Stimmabgabe enthalten. **Es werden keine Cookies** für Tracking oder Analyse gesetzt.
- **Betrieb der Live-Session durch den unter Abschnitt 1 genannten Verantwortlichen:** Während einer Session werden u. a. Session-Code, pseudonyme Nicknames (oder im anonymen Modus nur automatisch vergebene Teilnehmenden-IDs), Abstimmungen und Punkte verarbeitet. Rechtsgrundlage ist Art. 6 Abs. 1 lit. f DSGVO (berechtigtes Interesse am Betrieb der Echtzeit-Anwendung). Es werden keine personenbezogenen Daten ohne Zweckbindung gespeichert.
- **Session-Lebenszyklus:** Sessions enden standardmäßig 24 Stunden nach ihrer Erstellung; der Host kann innerhalb der vom Betreiber gesetzten und technisch auf 30 Tage begrenzten Obergrenze eine frühere oder spätere absolute Frist wählen. Dafür speichern wir den Erstellungszeitpunkt, das globale Sessionende, gegebenenfalls einen getrennten Q&A-Schluss und den Zeitpunkt des ersten erfolgreichen Beitritts. Der Marker des ersten Beitritts enthält keinen Nickname und bleibt bis zur Löschung des Sessiondatensatzes bestehen, damit eine Frist nicht nachträglich unbemerkt neu konfiguriert wird. Das Erreichen der Frist beendet Zugriffe und Änderungen, ist aber nicht selbst die physische Löschung; dafür gilt die nachfolgende Bereinigungsregel.
- **Nachbereitung und Bereinigung:** Nach dem Sessionende können berechtigte Hosts vorhandene Q&A-Inhalte noch 14 Tage lang ausschließlich lesen und exportieren; Teilnehmende erhalten keinen nachträglichen Zugriff. Anschließend werden der Sessionkern und seine abhängigen Inhalte technisch gelöscht. Ein eng begrenzter gesetzlicher oder zweckgebundener **Legal Hold** kann diese technische Löschung verzögern, verlängert aber keinen Host- oder Teilnehmerzugriff. Bonusnachweise und Sessionbewertungen werden getrennt höchstens 90 Tage, minimierte administrative Prüfnachweise höchstens 365 Tage aufbewahrt; beim Löschen des Sessionkerns werden direkte Session- und Teilnehmendenbezüge daraus entfernt. Aggregierte Statistiken (z. B. Teilnehmendenzahlen) enthalten keine Rückschlüsse auf Einzelpersonen.
- **Teilnahmeprofil und Anonymmodus:** Vor dem ersten Beitritt legt der Host für die gesamte Session fest, ob vorgegebene Pseudonyme, selbst gewählte Nicknames oder der Anonymmodus verwendet werden. Danach ist die Wahl gesperrt. Im Anonymmodus erscheinen Beiträge ohne sichtbare Verfasserangabe; technische Session-, Sicherheits- und Missbrauchsschutzdaten können weiterhin verarbeitet werden, sodass dies keine vollständige Anonymisierung bedeutet.
- **Produktfeedback (getrennt von Session-Bewertung und Blitzlicht):** Nach genutzten Sessions können wir Sie optional um zwei kurze Auswahlen zur Bedienbarkeit und zum Nutzen von arsnova.eu bitten; außerdem können Sie jederzeit über „arsnova.eu verbessern“ Feedback geben. Die Teilnahme ist freiwillig und erfordert weder Login noch E-Mail-Adresse. Gespeichert werden strukturierte Antworten sowie optional ein kurzer Freitext. Bei Feedback nach einer Session kann zusätzlich deren grobe Größenklasse verarbeitet werden. Beim In-App-Feedback umfasst der grobe technische Kontext höchstens Sprache, App-Version, parameterlose Routengruppe, Sessionphase, aktiven Kanal, Geräteklasse, Browserfamilie mit Hauptversion, Betriebssystemfamilie, Onlinezustand und eine vorhandene Fehler-/Request-ID. **Nicht** übertragen werden vollständige URLs, Sessioncodes/-IDs, Nicknames, Quiz-, Frage-, Antwort- oder Q&A-Inhalte, Bonus-Codes, Local-Storage-Inhalte, Zwischenablage oder Screenshots. IP-Adressen können technisch in kurzfristigen Server-Logs und Rate-Limits anfallen, werden aber nicht als Feedbackattribut gespeichert. Nur Plattform-Admins sehen die Meldungen, nicht Hosts oder die Presenteransicht; Freitext wird nicht automatisch veröffentlicht. Strukturierte Daten werden höchstens ca. 13 Monate und Freitexte höchstens 90 Tage aufbewahrt. Ihr Browser kann lokal Cooldown- und Warteschlangenstände speichern; ungesendete Entwürfe bleiben dort höchstens 7 Tage, und es entsteht kein geräteübergreifendes Profil. Ein öffentlicher Lösungslink erscheint nur, wenn ein Admin ihn freigibt. Berechtigte Admins können eine Markdown-Auswertung mit festem Prompt erzeugen; freiwillige Freitexte sind darin nur enthalten, wenn sie das ausdrücklich wählen. Berechtigte Admins können gespeicherte Rückmeldungen vorzeitig vollständig oder bis zu einem gewählten Datum endgültig löschen.

Bei Self-Hosting der Anwendung kann der Verantwortliche ein anderer sein (Ihre Institution). Dann gelten die Datenschutzhinweise, die dort bereitgestellt werden.

## 9. Änderungen

Wir passen die Datenschutzhinweise an, wenn sich die Rechtslage oder unsere Datenverarbeitung ändert. Die jeweils aktuelle Fassung finden Sie jederzeit hier auf arsnova.eu.
