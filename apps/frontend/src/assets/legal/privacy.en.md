...
- **Product feedback (separate from session ratings and quick feedback):** After a used session or at any time through “Improve arsnova.eu”, you can voluntarily provide feedback with two short selections, without login or email. We store structured answers and an optional short free-text note. In-app feedback may include only coarse context: language, app version, route group without parameters, session phase, active channel, device class, browser family and major version, operating-system family, online state, and an existing error/request ID. **Not** transmitted are full URLs, session codes/IDs, nicknames, quiz, question, answer or Q&A content, bonus codes, Local Storage content, clipboard content, or screenshots. IP addresses may occur technically in short-lived server logs and rate limits, but are not stored as a feedback attribute. Only platform admins see reports; free text is never published automatically. Structured data is retained for at most about 13 months, free text for at most 90 days, and local unsent drafts for at most 7 days. A public resolution link is shown only after admin approval.
