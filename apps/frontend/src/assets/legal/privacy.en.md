...
- **Product feedback (separate from session ratings and quick feedback):** After a used session we may optionally ask you, with two short selections, how easy and helpful arsnova.eu was. This is voluntary, without login or email. We store structured answers and an optional short free-text note plus coarse technical context only (e.g. language, app version, device class, session size) — **no** session codes, nicknames, questions, answers, quiz content, or Q&A content. Results are visible only to platform admins, not to hosts and not on the presenter view. Structured data is kept for at most about 13 months; optional free text for at most 90 days. Your browser may store local cooldown/outbox state (no cross-device profile).
